...
namespace sw
{
	...
	CXChildVisitResult AstVisitor::visitCursor( CXCursor cursor, CXCursor, CXClientData clientData )
	{
		...
		if ( kind == CXCursor_FieldDecl )
		{
			if ( hasAnnotateAttrPrefix( cursor, annotationConstants::kPropertyPrefix ) ||
				 sourceHasPrimaryAnnotation( cursor, annotationConstants::kPropertyPrefix ) )
			{
	...
	void AstVisitor::onEnumDecl( CXCursor cursor )
	{
		...
		BLOCK( "Parse ENUM annotation (Alias / …)" )
		{
			...
			string enumSpelling = sourceExtractMacroAnnotation( cursor, annotationConstants::kEnumMacroOpen, annotationConstants::kEnumPrefix );
	...
	string AstVisitor::buildFullyQualifiedName( CXCursor cursor )
	{
		...
		while ( true )
		{
			...
			const string spelling = cxStringToStd( clang_getCursorSpelling( current ) );
