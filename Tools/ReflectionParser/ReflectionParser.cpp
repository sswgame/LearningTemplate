...
namespace
{
	...
	void processInputFile( const std::string& inputFile, const CommandLineArgs& args, std::atomic<int32>& errorCount )
	{
		...
		{
			const std::string existingGen = sw::tool::readTextFile( genPath );
			const bool		  bPlaceholder =
				existingGen.find( "AUTO-GENERATED placeholder" ) != std::string::npos ||
				( existingGen.find( "regenerated by ReflectionParser" ) != std::string::npos &&
				  existingGen.find( "RegisterType" ) == std::string::npos );

			if ( bPlaceholder == false &&
				 sw::FileUtil::getFileTimestamp( genPath ) >= sw::FileUtil::getFileTimestamp( inputFile ) )
			{
