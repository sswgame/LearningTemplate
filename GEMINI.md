...
| 대상 | 규칙 | 예시 |
| :--- | :--- | :--- |
| 리소스 디렉터리 | 완전 소문자 (`lowercase`) | `Resource/game/empty/prefabs/`, `Resource/engine/shaders/` |
...
1. 비교할 때는 bool 자료형이고 true가 아니면 명시적으로 !로 비교하지말고, nullptr/false/값비교를 명시한다..
    ...
    if (_bValid == false) 
      return;
    ```
2. 비트 필드(bit field) 플래그(예: `uint8 _bFlag : 1;`)는 `true`/`false` 대신 `SW_TRUE`(1) / `SW_FALSE`(0)를 사용하여 대입 및 비교한다.
    ```cpp
    //BAD
    _bFlag = true;
    if (_bFlag == true)
...
3. if문 안의 코드가 1줄인 경우 괄호를 생략한다.
   ...
   - `switch` 의 `case` / `default` 는 본문이 **두 문장 이상이면 괄호를 씌우고**, 한 문장이면 씌우지 않는다.
     `break;` 도 한 문장으로 세므로 문장 하나 + `break;` 에는 괄호가 붙고, `case A: return X;` 는 그대로 둔다.
     `break;` 는 괄호 **안**에 둔다. 본문에 전처리기 지시문이 끼어 있으면 건드리지 않는다 — 여는 괄호와
     닫는 괄호가 `#if` 의 반대편에 놓여 한쪽 플랫폼에서만 짝이 맞는 일이 생긴다.
    ```cpp
    //BAD
    if (pOwner == nullptr)
    {
