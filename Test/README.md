...
| **`TestFramework`** | 테스트 공통 프레임워크 | 구글 테스트(GTest)를 래핑하거나 엔진 전용 환경 초기화 등 테스트 실행에 필요한 공통 기반 코드를 제공합니다. |

## ❌ CompileFail 테스트 (컴파일 실패 검증)

일부 테스트는 **의도적으로 컴파일 에러가 발생해야 정상**인 상황을 검증합니다.
- 예: `CompileFail_ComponentVariable` 테스트는 `Component`를 상속받은 클래스가 멤버 변수를 가지고 있음에도 `REFLECT_SCRIPT()` 대신 `REFLECT()`를 사용했을 때 `static_assert`가 올바르게 작동하는지 확인합니다.
- 이러한 테스트들은 CMake의 `WILL_FAIL` 속성을 사용하여, 빌드가 실패하면 테스트 성공으로 간주하고 빌드가 성공하면 테스트 실패로 간주하도록 설정되어 있습니다.
