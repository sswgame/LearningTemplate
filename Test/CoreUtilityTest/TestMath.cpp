...
SW_TEST_CASE( Utility_Math, Float2FullTest )
{
...
SW_TEST_CASE( Utility_Math, Float3FullTest )
{
...
SW_TEST_CASE( Utility_Math, Float4FullTest )
{
	sw::float4 v0;
	SW_EXPECT_NEAR_EQUAL( 0.0f, v0._x, 1e-4f );
	...
	SW_EXPECT_NEAR_EQUAL( std::sqrt( 30.0f ), vComp.getLength(), 1e-4f );
...
SW_TEST_CASE( Utility_Math, Float4x4FullTest )
{
...
SW_TEST_CASE( Utility_Math, QuaternionFullTest )
{
...
SW_TEST_CASE( Utility_Math, MathUtilFunctionsFull )
{
	float clamped = sw::MathUtil::clamp( 15.0f, 0.0f, 10.0f );
	SW_EXPECT_NEAR_EQUAL( 10.0f, clamped, 1e-4f );

	float saturated = sw::MathUtil::saturate( 1.5f );
	SW_EXPECT_NEAR_EQUAL( 1.0f, saturated, 1e-4f );

	float lerped = sw::MathUtil::lerp( 0.0f, 100.0f, 0.5f );
	SW_EXPECT_NEAR_EQUAL( 50.0f, lerped, 1e-4f );

	float invLerp = sw::MathUtil::inverse_lerp( 10.0f, 20.0f, 15.0f );
	SW_EXPECT_NEAR_EQUAL( 0.5f, invLerp, 1e-4f );

	float smoothstepVal = sw::MathUtil::smoothstep( 0.0f, 10.0f, 5.0f );
	SW_EXPECT_NEAR_EQUAL( 0.5f, smoothstepVal, 1e-4f );

	float sq = sw::MathUtil::sqaure( 4.0f );
	SW_EXPECT_NEAR_EQUAL( 16.0f, sq, 1e-4f );

	float p4 = sw::MathUtil::pow4( 2.0f );
	SW_EXPECT_NEAR_EQUAL( 16.0f, p4, 1e-4f );

	float fractional = sw::MathUtil::frac( 3.75f );
	SW_EXPECT_NEAR_EQUAL( 0.75f, fractional, 1e-4f );

	uint32 aligned = sw::MathUtil::align( 13u, 8u );
	SW_EXPECT_EQUAL( 16u, aligned );

	float rad = sw::MathUtil::toRadian( 180.0f );
	SW_EXPECT_NEAR_EQUAL( sw::MathUtil::kPI, rad, 1e-4f );

	float deg = sw::MathUtil::toDegree( sw::MathUtil::kPI );
	SW_EXPECT_NEAR_EQUAL( 180.0f, deg, 1e-4f );

	float rndVal = sw::MathUtil::getRandomRange( 1.0f, 5.0f );
