...
/**
 * @brief [ArchiveTest] 모든 원시 자료형(bool, u8~u64, i8~i64, f32, f64, string) 직렬화 라운드트립
 */
SW_TEST_CASE( ArchiveTest, ArchiveAllPrimitiveTypesStreaming )
{
...
/**
 * @brief [ArchiveTest] 수학 벡터/행렬(float2, float3, float4, float4x4) 직렬화 검증
 */
SW_TEST_CASE( ArchiveTest, ArchiveMathTypesStreaming )
{
    sw::Archive writeArch;
...
/**
 * @brief [ArchiveTest] 디스크 파일 저장/로드(saveFile 및 Archive(file, true)) 라운드트립
 */
...
/**
 * @brief [ArchiveTest] CRC32 체크섬 계산 및 데이터 변조(Corruption) 감지 검증
 */
SW_TEST_CASE( ArchiveTest, ArchiveChecksumAndCorruptionDetection )
{
    sw::Archive writeArch;
...
/**
 * @brief [ArchiveTest] 읽기 범위 초과(Out of Bounds) 및 에러 경계 검증
 */
...
/**
 * @brief [ArchiveTest] Archive 객체 TLV 직렬화
 */
SW_TEST_CASE( ArchiveTest, ArchiveObjectTLVSerialization )
{
    struct DummyStruct
    {
        int32 _valA = 123;
...
/**
 * @brief [ArchiveTest] SceneDocument 및 PrefabAsset 바이너리 아카이브 라운드트립 검증
 */
...
/**
 * @brief [ArchiveTest] Archive 에러 추적 및 안전성 헬퍼 (isOk, isError, hasBytesAvailable, getRemainingBytes)
 */
SW_TEST_CASE( ArchiveTest, ArchiveErrorTrackingAndSafetyHelpers )
{
    sw::Archive writeArch;
...
/**
 * @brief [ArchiveTest] Archive 섹션 및 서브 아카이브 분할 슬라이싱 검증
 */
SW_TEST_CASE( ArchiveTest, ArchiveSubArchiveAndSectionSlice )
{
    sw::Archive writeArch;
...
/**
 * @brief [ArchiveTest] Archive 바이트 벡터(vector<uint8>) 스트리밍 및 템플릿 직렬화 검증
 */
SW_TEST_CASE( ArchiveTest, ArchiveByteVectorAndTemplateObjectSerialization )
{
    sw::Archive writeArch;
...
/**
 * @brief [ArchiveTest] Archive 압축 섹션 및 압축 리플렉션 객체 직렬화 검증
 */
SW_TEST_CASE( ArchiveTest, ArchiveCompressedObjectSerialization )
{
    // 1) 압축 섹션 I/O
    sw::Archive       writeArch;
...
/**
 * @brief [ArchiveTest] 단일 Archive 내 복합 포맷(바이너리 + 압축 + JSON 메타 + XML 설정) 하이브리드 번들링 검증
 */
SW_TEST_CASE( ArchiveTest, ArchiveMultiFormatHybridBundle )
{
...
/**
 * @brief [ArchiveTest] 가변 길이 정수(VarInt / ZigZag) 경계값 및 압축율 전수 검증
 */
SW_TEST_CASE( ArchiveTest, VarIntAndZigZagBoundaryEncoding )
{
    sw::Archive writeArch;
...
/**
 * @brief [ArchiveTest] StringPool 문자열 인터닝 및 중복 제거 대규모 압축률 검증
 */
SW_TEST_CASE( ArchiveTest, StringPoolInterningAndDeduplication )
{
...
/**
 * @brief [ArchiveTest] PredefinedNameType 표준 사전 기반 0-바이트 스트링 풀 연동 검증
 */
SW_TEST_CASE( ArchiveTest, PredefinedTypesStringPoolIntegration )
{
    sw::StringPool pool;
...
/**
 * @brief [ArchiveTest] Archive 컴팩트 직렬화(Compact Object) 대규모 스트레스 테스트 (1,000회)
 */
SW_TEST_CASE( ArchiveTest, ArchiveCompactObjectStress1000 )
{
...
/**
 * @brief [ArchiveTest] 멀티플레이어 넷코드 시뮬레이션: 50개 액터 상태 델타 패킷 압축 및 MTU 한도 내 전송 검증
 */
SW_TEST_CASE( ArchiveTest, SimulatedNetworkPacketDeltaReplication )
{
...
/**
 * @brief [ArchiveTest] 손상된 버퍼 및 경계 초과 결함 주입(Fault Injection) 내결함성 검증
 */
SW_TEST_CASE( ArchiveTest, CorruptedBufferFaultInjectionAndGracefulHandling )
{
...
/**
 * @brief [ArchiveTest] 복합 중복 제거 파이프라인 대규모 엔드-투-엔드 스트레스 테스트 (5,000회)
 */
SW_TEST_CASE( ArchiveTest, MassiveEndToEndDeduplicationPipelineStress5000 )
{
...
SW_TEST_CASE( ArchiveTest, SerializerUtilTranscodingAndScopedScratch )
{
...
SW_TEST_CASE( ArchiveTest, MalformedStreamOversizedAllocationFaultInjection )
{
...
SW_TEST_CASE( ArchiveTest, TranscodingFailureCasesAndInvalidInputs )
{
    const sw::TypeInfo* pPlayerType = TestReflectedPlayer::StaticType();
    SW_EXPECT_NOT_NULL( pPlayerType );
