...
**증거는 린트가 직접 든다.** 각 린트 모듈이 `kSelfTestCases` 에 "이건 반드시 잡아야 한다" 는 조각을
적어 두고, 이 장치가 임시 트리에 그것을 써서 린트를 돌린다. 0 이 아닌 종료 코드가 나와야 통과다.
조각 표를 여기 모아 두지 않는 이유는 하나다 — **표가 둘이면 언제든 어긋난다.**

린트가 `kSelfTestCases` 를 선언하지 않으면 이 검사가 "덮이지 않은 린트" 로 실패한다.
정말 조각을 만들 수 없는 린트는 `kSelfTestSkipReason` 에 이유를 적는다(이유 없는 예외는 없다).
...
def main(argv: list[str] | None = None) -> int:
    ...
    for moduleName in gateLints:
        ...
        cases = getattr(module, "kSelfTestCases", None)
        skipReason = getattr(module, "kSelfTestSkipReason", "")

        if not cases:
            if skipReason:
                if args.verbose:
                    print(f"  [{moduleName}] 건너뜀 — {skipReason}")
                continue
            errors.append(f"{moduleName}: `kSelfTestCases` 가 없습니다 — 이 린트가 죽어도 아무도 모릅니다 "
                          f"(정말 못 만들면 `kSelfTestSkipReason` 에 이유를 적으세요)")
            continue
