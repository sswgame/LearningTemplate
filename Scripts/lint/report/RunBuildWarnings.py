...
[빌드 로그의 경고는 sccache 가 **재생한 옛 경고**일 수 있다]
이 저장소는 sccache 를 컴파일러 런처로 쓴다(`build.ninja` 의 `LAUNCHER`). 캐시가 맞으면 오브젝트만
돌려주는 것이 아니라 **그때 찍혔던 stderr 까지 그대로 다시 출력한다.** 그래서 빌드 로그의 경고가
**지금 파일의 내용과 맞지 않을 수 있다.**

알아보는 법은 하나다 — clang 이 경고와 함께 찍어 주는 **소스 줄의 본문이 그 줄 번호의 실제 내용과
다르면** 그것은 재생이다. 실제로 겪었다(2026-09-19): `ReflectionContainers.h(87,13): duplicated
command '@brief'` 가 빌드마다 나왔는데 87 줄에는 `@brief` 가 없었고, 찍힌 본문은 그 파일 **다른
줄**의 것이었다. 처음엔 PCH 가 낡은 줄 알고 `pch.h` 를 건드려 봤지만 그것이 아니었다.

확인과 해소는 `SCCACHE_RECACHE=1` 이다 — 걸고 그 TU 를 다시 빌드하면 캐시를 읽지 않고 **진짜로
컴파일**하며, 캐시 항목도 그때 갱신된다. 그리고 **이 스크립트는 애초에 캐시를 지나지 않는다**:
컴파일 DB 에는 런처가 없어 컴파일러를 직접 부른다. 즉 **빌드 로그와 이 스크립트가 다르면 이
스크립트가 맞다.**
