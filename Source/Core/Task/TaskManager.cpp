...
namespace sw
{
    ...
    namespace
    {
        ...
        /** @brief 태스크 본문 호출 — 콜러블 variant 의 갈래마다 하나. */
...
namespace sw
{
    ...
    void TaskManager::drainQueueItem( uintptr_t item )
    {
        ...
        if ( TaskQueueItem::isTicket( item ) )
            closeGroupTicket( TaskQueueItem::toGroup( item ), false ); // 돌리지 않고 닫는다 — 부모는 준비되지 않은 채 핸들이 놓일 때 돌아간다
        else
            TaskQueueItem::toNode( item )->release();
    }

    // ------------------------------------------------------------------------------
    // 2) 스레드 정체 — 메인 · 워커 · 스크래치/대기자 슬롯
    // ------------------------------------------------------------------------------
    ...
    TaskNode* TaskManager::createTaskNode( string_view name, TaskThreadAffinity affinity )
    {
        ...
        // 태스크 본문 안에서 만든 태스크는 그 본문의 자식이다 — 부모는 자식이 다 끝나야 완료된다.
    ...
    void TaskManager::runParallel( uint32 count, uint32 serialThreshold, const ParallelBlockDelegate& body )
    {
        ...
        // 워커가 깨기를 기다리지 않는다 — 이 스레드가 첫 청크부터 집는다. 남은 티켓은 이 스레드가 닿을 수 있는
        // 큐(자기 덱 · 전역)에만 있으므로 조인이 돕다 보면 스스로 집는다.
    ...
    TaskStageHandle TaskManager::getOrCreateStage( string_view stageName )
    {
        ...
        if ( pStage == nullptr )
        {
            ...
            pStage->_name = stageName;
            _listAllStage.push_back( pStage ); // 목록이 핸들 하나 몫(풀이 준 첫 참조)을 쥔다 — clear 가 놓는다
        }
        pStage->retain();
        return TaskStageHandle{ pStage };
    }
    ...
    // ------------------------------------------------------------------------------
    // 6) 기다리기 — 돕다가 자기 워드에 잠들기, 방송, 메인 스레드 깨우기
    // ------------------------------------------------------------------------------
    ...
    bool TaskManager::waitAll( uint32 timeoutMs )
    {
        ...
        while ( _activeTaskCount.load( std::memory_order_acquire ) > 0 )
        {
            ...
            // 대상이 전체라 대기자 하나를 적을 곳이 없다 — 방송에 잠든다(활성 0 · 메인 일감 · clear 가 울린다).
    ...
    {
        // 워커는 자기 덱(LIFO — 방금 만든 것이 캐시에 뜨겁다), 밖의 스레드(게임·렌더)는 전역 큐. 덱이 차면 전역으로 흘린다.
    ...
    bool TaskManager::tryTakeItem( int32 workerId, uintptr_t& outItem )
    {
        ...
        // 워커는 자기 다음 워커부터 한 바퀴(자기 덱은 방금 비웠다), 밖의 스레드는 전부.
    ...
    void TaskManager::workerLoop( uint32 workerId )
    {
        ...
        while ( _bStop.load( std::memory_order_relaxed ) == false )
        {
            ...
            // (읽은 뒤에 들어온 것은 세대가 바뀌고, 읽기 전에 들어온 것은 바로 아래 tryTakeItem 이 본다.)
