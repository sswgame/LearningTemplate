...
namespace sw
{
    ...
    {
        return MathUtil::sqrt( getDistanceSquared( from, to ) );
    }
    ...
    {
        return float2{ MathUtil::min( lhs._x, rhs._x ), MathUtil::min( lhs._y, rhs._y ) };
    }

    float2 float2::max( const float2& lhs, const float2& rhs ) noexcept
    {
        return float2{ MathUtil::max( lhs._x, rhs._x ), MathUtil::max( lhs._y, rhs._y ) };
    }
    ...
    {
        return MathUtil::sqrt( getLengthSquared() );
    }
    ...
    bool float2::operator==( const float2& other ) const noexcept { return MathUtil::nearEqual( _x, other._x ) && MathUtil::nearEqual( _y, other._y ); }

    bool float2::operator!=( const float2& other ) const noexcept { return !( *this == other ); }
    ...
    {
        return float3{ MathUtil::min( lhs._x, rhs._x ), MathUtil::min( lhs._y, rhs._y ), MathUtil::min( lhs._z, rhs._z ) };
    }

    float3 float3::max( const float3& lhs, const float3& rhs ) noexcept
    {
        return float3{ MathUtil::max( lhs._x, rhs._x ), MathUtil::max( lhs._y, rhs._y ), MathUtil::max( lhs._z, rhs._z ) };
    }
    ...
    bool float3::operator!=( const float3& other ) const noexcept { return !( *this == other ); }
    ...
    {
        return float4{ MathUtil::min( lhs._x, rhs._x ), MathUtil::min( lhs._y, rhs._y ), MathUtil::min( lhs._z, rhs._z ), MathUtil::min( lhs._w, rhs._w ) };
    }

    float4 float4::max( const float4& lhs, const float4& rhs ) noexcept
    {
        return float4{ MathUtil::max( lhs._x, rhs._x ), MathUtil::max( lhs._y, rhs._y ), MathUtil::max( lhs._z, rhs._z ), MathUtil::max( lhs._w, rhs._w ) };
    }
    ...
    float4 float4::transform( const float4& v, const quaternion& rotation ) noexcept
    {
        const float4x4 rotMat = rotation.toMatrix();
        return transform( v, rotMat );
    }

    float4 float4::transform( const float4& v, const float4x4& matrix ) noexcept
    {
        return float4{
            v._x * matrix._11 + v._y * matrix._21 + v._z * matrix._31 + v._w * matrix._41,
            v._x * matrix._12 + v._y * matrix._22 + v._z * matrix._32 + v._w * matrix._42,
            v._x * matrix._13 + v._y * matrix._23 + v._z * matrix._33 + v._w * matrix._43,
    ...
    float4 float4::transformNormal( const float4& v, const float4x4& matrix ) noexcept
    {
        return float4{
            ...
            v._x * matrix._12 + v._y * matrix._22 + v._z * matrix._32,
            v._x * matrix._13 + v._y * matrix._23 + v._z * matrix._33,
            0.0f };
    }
    ...
    bool float4::operator==( const float4& other ) const noexcept { return MathUtil::nearEqual( _x, other._x ) && MathUtil::nearEqual( _y, other._y ) && MathUtil::nearEqual( _z, other._z ) && MathUtil::nearEqual( _w, other._w ); }

    bool float4::operator!=( const float4& other ) const noexcept { return !( *this == other ); }
    ...
    {
        return double3{ MathUtil::min( lhs._x, rhs._x ), MathUtil::min( lhs._y, rhs._y ), MathUtil::min( lhs._z, rhs._z ) };
    }

    double3 double3::max( const double3& lhs, const double3& rhs ) noexcept
    {
        return double3{ MathUtil::max( lhs._x, rhs._x ), MathUtil::max( lhs._y, rhs._y ), MathUtil::max( lhs._z, rhs._z ) };
    }
    ...
    double3& double3::normalize() noexcept
    {
        ...
        if ( lenSq > 0.0 )
        {
            const float64 invLen = 1.0 / MathUtil::sqrt( lenSq );
            ( *this ) *= invLen;
        }
        else
            ( *this ) = Zero;
    ...
    bool double3::operator==( const double3& other ) const noexcept { return MathUtil::nearEqual( _x, other._x ) && MathUtil::nearEqual( _y, other._y ) && MathUtil::nearEqual( _z, other._z ); }

    bool double3::operator!=( const double3& other ) const noexcept { return !( *this == other ); }
