 ...
 * @brief Per-module GVM / Type / Enum / ComponentFactory registrar list heads (hot-reload safe)
 *
 * Core.dll keeps GlobalVariableRegistrar::getHead() / TypeRegistrar::getHead() /
 * ComponentFactoryRegistrar::getHead(). Other modules declare heads in a header and
 * define them in exactly one .cpp (avoids -Wunique-object-duplication from inline
 * static locals in shared libs).
 */
