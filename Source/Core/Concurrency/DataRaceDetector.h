 ...
 * @brief 컨테이너 래퍼에 붙이는 런타임 데이터 레이스 탐지기입니다(디버그 빌드 전용).
 */
#pragma once
...
namespace sw
{
    ...
    public:
        ...
        /** @brief 카운터를 공유하지 않고, 0 으로 초기화된 새 컨텍스트로 복사합니다. */
        constexpr RaceDetectContext( const RaceDetectContext& ) noexcept
            : _state{ 0 } {}
        /** @brief 카운터를 가져오지 않고 0 으로 되돌립니다. */
        RaceDetectContext& operator=( const RaceDetectContext& )
        {
            _state.store( 0 );
            return *this;
        }
        /** @brief 카운터를 가져오지 않고, 0 으로 초기화된 새 컨텍스트로 이동합니다. */
        ...
        /** @brief 읽기 수를 올리고, 쓰는 중인 스레드가 있으면 레이스로 보고합니다. */
        void enterRead();
        /** @brief 읽기 수를 내립니다. */
        void exitRead();

        /** @brief 쓰기 수를 올리고, 다른 스레드가 접근 중이면 레이스로 보고합니다. */
        void enterWrite();
        /** @brief 쓰기 수를 내립니다. */
        void exitWrite();
#else
    ...
    private:
#if defined( SW_DEBUG )
        /** @brief 레이스 메시지와 콜 스택을 Fatal 로그로 남깁니다. */
        void triggerDataRace( const utf8* pMessage );

        /** @brief 지금 들어와 있는 것이 현재 스레드 자신인지 확인합니다. 그렇다면 재진입이지 레이스가 아닙니다. */
        bool isOwnedByCurrentThread() const;

        // 하위 16비트: 읽기 수(최대 65535)
        // 상위 16비트: 쓰기 수(최대 65535)
        atomic<uint32> _state{ 0 };

        /**
         * @brief 지금 이 컨텍스트에 처음 들어온 스레드의 id 해시입니다. 아무도 없으면 0 입니다.
         *
         * @details 데이터 레이스는 정의상 두 스레드가 있어야 생깁니다. 그런데 이 탐지기에는 스레드 개념이 없어서, 한 스레드가
         *          가드를 잡은 채 **같은 객체의 다른 가드 메서드를 부르기만 해도** 레이스로 보고했습니다. 예를 들어
         *          `sw::set::operator=(initializer_list)` 는 쓰기 가드를 잡고 `clear()` · `insert()` 를 부르는데, 그 둘도 가드를 잡습니다.
         *          그래서 Debug 빌드에서는 `sw::set` 에 초기화 리스트를 대입하기만 해도 있지도 않은 레이스가 보고됐습니다.
         *
         *          `map` 에도 같은 곳이 있고, 쓰기 가드 안에서 const 메서드를 부르는 경우도 모두 같은 모양입니다. 즉 "가드 메서드가
         *          가드 메서드를 부르지 않게 조심한다" 로는 막을 수 없는 **구조적인** 오탐이었습니다. 그래서 주인 스레드를 기억해
         *          재진입을 구분합니다.
         *
...
// ------------------------------------------------------------------------------
// 3) 컨테이너 훅 — Release · C++17 에서 멤버와 가드가 바이너리에 남지 않게 한다
