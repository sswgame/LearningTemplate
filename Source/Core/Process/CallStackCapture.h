...
{
    // ------------------------------------------------------------------------------
    // 1-1) 심볼화의 **공통 관문** — 플랫폼 구현이 시작 전에 같이 지나는 두 자리
    // ------------------------------------------------------------------------------
    /**
     * @brief 프레임이 하나도 없을 때 돌려주는 문자열입니다.
     * @details 플랫폼마다 리터럴을 따로 적으면 로그를 grep 하는 쪽이 두 철자를 알아야 한다.
     */
    inline constexpr auto kEmptyCallStackText = "[Empty CallStack]";

    /**
     * @brief 심볼을 못 쓸 때 **주소만** 찍은 스택 문자열을 만듭니다.
     * @details 심볼화는 전역 락을 잡는데, 이 함수는 **크래시 경로에서도 불린다** — 다른 스레드가
     *          이미 심볼화 중이면 기다리다 교착하는 대신 주소만 남기고 빠져나와야 한다(주소는 맵
     *          파일로 나중에 풀 수 있다). 그 규칙이 플랫폼마다 한 벌씩 적혀 있었고, 본체(DbgHelp vs
     *          backtrace_symbols)는 정말 다르지만 **이 관문만은 같아야 한다.**
     * @note 일부러 인라인 함수다 — 크래시 경로에 간접 호출(델리게이트·함수 포인터)을 끼워 넣지 않는다.
     */
