...
namespace sw
{
    ...
    bool Process::launch( string_view command, const ProcessOptions& options )
    {
        ...
        {
            // ---- 자식 ----
            // 여기서부터 exec 까지는 **async-signal-safe 한 함수만** 부른다. fork 는 부모의 스레드를 하나만 데려오지만
            // 뮤텍스는 통째로 복사되므로, 다른 스레드가 잡고 있던 락(로거의 락이 대표적이다)은 영원히 잠긴 채로 남는다.
            ...
            _exit( 127 ); // exec 가 실패했을 때만 여기에 온다
        }
    ...
    {
        // POSIX 는 자식의 종료 코드를 정할 수 없다. 시그널로 죽이고, `waitForExit` 은 128 + 시그널 번호를 준다.
        (void)exitCode;
        ...
        // 그룹 전체에 보낸다. `sh -c "…"` 가 명령을 직접 exec 해 버리는 경우(대부분의 셸이 그렇게 최적화한다)에도 그룹에는
        // 자식 하나뿐이라 결과가 같고, 손자 프로세스를 띄웠다면 그것까지 죽는다.
    ...
    bool Process::isRunning() const
    {
        ...
        // 핵심은 `WNOWAIT` 이다. **거두지 않고** 묻는다. `waitpid(WNOHANG)` 로 물으면 끝난 자식을 그 자리에서 거둬
        // 버려서, 이어서 부르는 `waitForExit` 이 종료 코드를 영영 받지 못한다.
        siginfo_t info{};
        info.si_pid = 0;
        if ( waitid( P_PID, static_cast<id_t>( childPid ), &info, WEXITED | WNOHANG | WNOWAIT ) != 0 )
            return false; // 물을 수 없다. 이미 거뒀거나 이 프로세스의 자식이 아니다
