	...
	public:
		...
		ObjectHandle insert( T value )
		{
			...
			if ( _listFree.empty() == false )
			{
				...
				_listFree.pop_back();
				gen							 = _listSlots[index]._generation;
				_listSlots[index]._value	 = std::move( value );
				_listSlots[index]._bOccupied = true;
			}
			else
			{
				index = static_cast<uint32>( _listSlots.size() );
				_listSlots.push_back( Slot{ std::move( value ), 1u, true } );
				gen = 1;
			}
		...
		const T* get( ObjectHandle handle ) const
		{
			...
			if ( index >= _listSlots.size() || _listSlots[index]._bOccupied == false || _listSlots[index]._generation != handle.generation() )
				return nullptr;
			return std::addressof( _listSlots[index]._value );
		}
		...
		void erase( ObjectHandle handle )
		{
			...
			if ( pSlot == nullptr )
				return;
			const uint32 index			  = handle.index();
			_listSlots[index]._value	  = T{};
			_listSlots[index]._bOccupied  = false;
			_listSlots[index]._generation = _listSlots[index]._generation + 1u;
			if ( _listSlots[index]._generation == 0 )
				_listSlots[index]._generation = 1;
		...
		{
			for ( uint32 slotIndex = 0; slotIndex < static_cast<uint32>( _listSlots.size() ); ++slotIndex )
			{
				Slot& slot = _listSlots[slotIndex];
				if ( slot._bOccupied )
		...
		void forEachHandle( Fn&& fn ) const
		{
			for ( uint32 slotIndex = 0; slotIndex < static_cast<uint32>( _listSlots.size() ); ++slotIndex )
			{
				const Slot& slot = _listSlots[slotIndex];
				if ( slot._bOccupied )
					fn( ObjectHandle::make( slotIndex, slot._generation ), slot._value );
			}
		}
