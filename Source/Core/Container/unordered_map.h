...
namespace sw
{
	...
	private:
		...
		vector<size_t> _listBuckets;   ///< 버킷 헤드 인덱스 테이블
	...
	public:
		...
		T& operator[]( const Key& key )
		{
			...
			_listDenseData.push_back( { std::make_pair( key, T{} ), _listBuckets[bucketIdx] } );
		...
		T& operator[]( Key&& key )
		{
			...
			const size_t newIdx = _listDenseData.size();
			_listDenseData.push_back( { std::make_pair( std::move( key ), T{} ), _listBuckets[bucketIdx] } );
			_listBuckets[bucketIdx] = newIdx;
			return _listDenseData[newIdx].kv.second;
		}
		...
		std::pair<iterator, bool> insert( const value_type& value )
		{
			...
			const size_t newIdx = _listDenseData.size();
			_listDenseData.push_back( { std::make_pair( value.first, value.second ), _listBuckets[bucketIdx] } );
			_listBuckets[bucketIdx] = newIdx;
		...
		std::pair<iterator, bool> insert_or_assign( const Key& key, M&& obj )
		{
			...
			const size_t newIdx = _listDenseData.size();
			_listDenseData.push_back( { std::make_pair( key, std::forward<M>( obj ) ), _listBuckets[bucketIdx] } );
			_listBuckets[bucketIdx] = newIdx;
			return { iterator( this, newIdx ), true };
		}
		...
		std::pair<iterator, bool> insert_or_assign( Key&& key, M&& obj )
		{
			...
			const size_t newIdx = _listDenseData.size();
			_listDenseData.push_back( { std::make_pair( std::move( key ), std::forward<M>( obj ) ), _listBuckets[bucketIdx] } );
			_listBuckets[bucketIdx] = newIdx;
			return { iterator( this, newIdx ), true };
		}
