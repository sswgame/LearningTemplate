 ...
 *
 * @note **볼륨과 음소거는 이 인터페이스가 들고 있다.** 값 범위(0~1 클램프)와 "음소거는 마스터
 *       한 자리에만 건다" 는 규칙은 백엔드마다 다시 적을 것이 아니라 여기 한 번만 적는다.
 *       백엔드는 값이 바뀐 뒤 `applyVolume()` 으로 통보만 받아 실제 보이스에 반영한다.
 */
#pragma once
...
namespace sw
{
    /**
     * @class IAudioSystem
     * @brief 오디오 재생 및 관리를 위한 추상 인터페이스
     */
    ...
    public:
         ...
         * @details 요청 시점에 정해지므로 비동기 디코드가 끝나기 전에도 이미 이 값이다.
         *          **사본을 돌려준다** — 백엔드가 이 문자열을 다른 스레드에서 바꾸므로
         *          안을 가리키는 뷰를 내주면 잠금 밖에서 흔들린다.
         */
         ...
         * @details 음소거는 **마스터 볼륨 한 자리에만** 건다. 음악·효과음 보이스는 자기 볼륨만
         *          들고 있으므로, 음소거 중에 볼륨을 바꿔도 음소거를 풀면 그대로 돌아온다.
