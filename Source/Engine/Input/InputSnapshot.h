...
namespace sw
{
    ...
    struct SW_API InputSnapshot
    {
        ...
        uint64  _buttonMask{ 0 };          ///< 최대 64개 액션/버튼 눌림 비트마스크. 불리언이 아니라 마스크다.
        ...
        /**
         * @brief 직렬화된 스냅샷 한 개의 바이트 수입니다. **`sizeof(InputSnapshot)` 과 다르다.**
         * @details 구조체에는 `_tickNumber` 뒤에 정렬 패딩 4바이트가 있다. 예전에는 구조체를
         *          통째로 `memcpy` 했기 때문에 **그 패딩까지 파일·네트워크로 나갔다** — 패딩은
         *          아무도 쓰지 않으므로 값이 정해져 있지 않고, 그래서 같은 입력을 두 번 저장해도
         *          바이트가 달라질 수 있었다(체크섬·비교·중복 제거가 성립하지 않는다).
         *          지금은 필드를 순서대로 적는다. 컴파일러·아키텍처가 달라도 같은 바이트가 나온다.
         */
        static constexpr uint32 kSerializedSize = sizeof( uint32 ) + sizeof( uint64 ) + ( 4 * sizeof( float32 ) ) + ( 2 * sizeof( float32 ) );

        /** @brief 바이너리 버퍼에 직렬화합니다 (성공 시 기록된 바이트 수 = `kSerializedSize`). */
