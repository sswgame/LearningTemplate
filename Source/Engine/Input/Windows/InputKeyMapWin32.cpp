    ...
    namespace
    {
        struct InputKeyMapWin32Internal
        {
            static Key resolveShiftKey( intptr_t lParam )
            {
                if ( lParam != 0 )
                {
                    const uint32 scancode = ( static_cast<uint32>( lParam ) & 0x00FF0000 ) >> 16;
                    const uint32 mappedVk = MapVirtualKeyW( scancode, MAPVK_VSC_TO_VK_EX );
                    if ( mappedVk == VK_RSHIFT )
                        return Key::RightShift;
                    if ( mappedVk == VK_LSHIFT )
            ...
            static Key resolveControlKey( intptr_t lParam )
            {
                if ( lParam != 0 )
                {
            ...
            static Key resolveAltKey( intptr_t lParam )
            {
                if ( lParam != 0 )
                {
                    if ( ( static_cast<uint32>( lParam ) & 0x01000000 ) != 0 )
...
namespace sw
{
    Key InputKeyMap::mapWin32VirtualKey( uintptr_t vk, intptr_t lParam )
    {
        if ( 'A' <= vk && vk <= 'Z' )
        ...
        switch ( vk )
        {
            ...
            case VK_RSHIFT:
                return Key::RightShift;
            case VK_SHIFT:
                return InputKeyMapWin32Internal::resolveShiftKey( lParam );
            case VK_LCONTROL:
                return Key::LeftControl;
            case VK_RCONTROL:
                return Key::RightControl;
            case VK_CONTROL:
                return InputKeyMapWin32Internal::resolveControlKey( lParam );
            case VK_LMENU:
                return Key::LeftAlt;
            case VK_RMENU:
                return Key::RightAlt;
            case VK_MENU:
                return InputKeyMapWin32Internal::resolveAltKey( lParam );
            case VK_LWIN:
...
namespace sw
{
    Key InputKeyMap::mapWin32VirtualKey( uintptr_t, intptr_t )
    {
        return Key::Unknown;
    }

    Key InputKeyMap::mapScanCodeToKey( uint32, bool )
    {
        return Key::Unknown;
    }
