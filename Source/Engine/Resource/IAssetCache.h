/**
 * @file IAssetCache.h
 * @brief 경로를 키로 에셋을 들고 있는 캐시의 공통 계약입니다. 에셋 종류를 늘리는 자리입니다.
 *
 * [왜 필요한가]
 * 캐시가 셋(Material · Texture · Prefab)인데 그것을 **아는 코드가 이름으로 셋을 적고 있었습니다.**
 * `ResourceManager::shutdown` 은 머티리얼과 텍스처만 비우고 프리팹을 잊었고(재초기화 뒤에도 옛
 * 프리팹이 남았습니다), 새 에셋 종류를 하나 더하려면 캐시 클래스 · 매니저 멤버 · 게터 · 종료 경로를
 * 손으로 같이 고쳐야 했습니다. **목록이 여럿이면 한쪽만 늘어납니다.** 이 저장소가 린트 · 픽서 · 백엔드에서
 * 이미 같은 결론에 도달한 자리입니다.
 *
 * 상용 엔진도 같은 모양입니다. 언리얼의 `FStreamableManager` · 고도의 `ResourceFormatLoader` · 유니티의
 * `AssetDatabase` 는 모두 "종류마다 등록하고, 공통 동작은 등록부를 훑는다" 입니다. 여기서는 그 최소한만
...
namespace sw
{
    ...
    {
    public:
        /** @brief 빈 인터페이스로 만듭니다. 상태는 구현이 갖습니다. */
        IAssetCache() = default;
        /** @brief 파생 캐시를 포인터로 들고 지울 수 있어야 합니다. */
        virtual ~IAssetCache() = default;

        /** @brief 캐시는 복사하지 않습니다. 등록부가 포인터로 듭니다. */
        ...
        /**
         * @brief 캐시에 있으면 디스크에서 다시 읽습니다(에디터 핫 리로드).
         * @param pDevice GPU 자원을 다시 올려야 하는 캐시가 씁니다. 필요 없는 캐시는 무시합니다.
         * @details **디바이스를 인자로 받습니다.** 캐시가 마지막으로 본 디바이스를 들고 있으면
         *          백엔드를 바꾼 뒤 그 포인터가 사라진 디바이스를 가리킵니다. `MaterialCache` 가
         *          실제로 그렇게 들고 있었습니다.
         */
