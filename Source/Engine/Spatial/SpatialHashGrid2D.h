...
{
    /**
     * @brief 많은 2D/2.5D 객체를 위한 O(1) 공간 분할 해시 그리드입니다.
     */
    ...
    public:
         ...
         * @details 셀 범위는 **입력 좌표에서** 나옵니다. 큰 AABB 하나가 수천만 개의 셀을 요구할 수
         *          있고, 이 모듈이 스스로 제공하는 `AABB2D::infinite()` 가 바로 그런 값입니다. 상한을
         *          넘는 핸들은 셀에 흩뿌리는 대신 목록 하나에 모아 두고, 그리드를 보는 질의가 그
         *          목록을 **항상 함께** 봅니다. 그런 핸들은 수가 적고 어차피 거의 모든 질의에 걸리므로
         *          셀에 흩어 두는 것이 이득이 되지 않습니다.
         * @note `PhysicsWorld::kMaxBodyCellCount` 와 값이 같지만 다른 질문입니다(그쪽은 물리 바디,
         *       이쪽은 범용 질의 색인). 한쪽 사정으로 값을 바꿀 수 있어야 하므로 별칭을 두지 않습니다.
         */
        static constexpr int64 kMaxHandleCellCount = 1024;

        /**
         * @brief 질의 하나가 훑을 수 있는 최대 셀 수입니다.
         * @details 넘으면 셀을 도는 대신 등록된 핸들 전부를 훑습니다. 셀 수가 핸들 수보다 많아지는
         *          순간부터 그리드를 도는 것은 순수한 손해입니다. `queryRay` 는 예전부터 같은 이유로
         *          걸음 수를 막고 있었는데(당시 `kMaxRayStep`, 지금은 이 상한을 같이 씁니다), 나머지 셋은 막혀 있지 않았습니다.
         */
    ...
    private:
        ...
        /** @brief 셀을 훑기보다 등록된 핸들 전부를 도는 편이 나은지 반환합니다. 범위가 비었거나 너무 넓으면 true 입니다. */
        bool shouldScanAllHandles( const CellRange& range ) const;

        /**
         * @brief 범위에 걸린 후보 핸들마다 콜백을 부릅니다. 중복은 걸러지지 않습니다.
         * @details 세 질의가 함께 쓰는 자리입니다. "어디를 보는가" 는 여기 하나에만 적혀 있고, 각 질의는
         *          "무엇을 남기는가"(좁은 판정)만 스스로 합니다. 결과의 중복은 부르는 쪽이 마지막에
         *          정렬 · 유일화로 지웁니다.
         */
        template <typename Func>
        ...
        /** @brief 그리드에 흩뿌리기에는 너무 큰 핸들들입니다. 그리드를 보는 질의가 **항상 함께** 봅니다. */
