/**
 * @file SceneCooker.h
 * @brief 씬 XML 을 런타임이 바로 먹는 SCN1 바이너리로 굽습니다 (엔티티 상태까지 바이너리로).
 */
#pragma once
...
namespace sw
{
     ...
     *
     * @details **왜 C++ 에 있는가.** 나머지 쿠킹은 `Scripts/generate/CookAssets.py` 가 한다. 그런데
     *          엔티티 상태를 바이너리로 만들려면 리플렉션(`TypeInfo`·프로퍼티 표)이 있어야 하고,
     *          그것은 엔진 안에만 있다. 그래서 이 단계만 `App.exe --cook-scenes` 로 넘어온다 —
     *          셰이더가 `--bake-shaders` 로 넘어오는 것과 같은 이유, 같은 모양이다.
     *
     *          굽기 전에는 쿠킹된 `.scene.bin` 이 XML 문자열을 그대로 담고 있어서, 런타임이
     *          엔티티마다 XML 을 다시 파싱했다. 바이너리 상태는 엔티티당 4254 ns -> 1602 ns 다.
     */
    class SceneCooker
    {
    public:
         ...
         * @return 바이너리로 바꾼 엔티티 수.
         *
         * @details **왕복으로 검증하고 나서만 바꾼다.** 구운 바이트를 즉시 되읽어 컴포넌트 구성이
         *          같은지 본 뒤에 XML 을 버린다. 모르는 컴포넌트 타입(예: 이 프로세스에 안 올라온
         *          게임 모듈의 것)이 섞이면 검증이 실패하고 그 엔티티는 **XML 그대로 남는다** —
         *          쿠킹이 조용히 컴포넌트를 떨어뜨리는 일은 없다.
         */
         ...
         *          주석과 같다 — 소스가 옮겨진 뒤 낡은 `.bin` 이 남아 실패를 가린다.
         */
