 ...
 * @brief 호스트가 소유하는 엔진 서비스들의 **저장소** — `EngineServiceList.xxx` 에서 생성된다.
 *
 * [왜 필요한가]
 * 서비스 목록은 하나인데 **그것을 만들고 표에 꽂는 코드는 호스트마다 한 벌**이었다
 ...
 * 갔다(배포본에서 셰이더 캐시를 건너뛰고 DXC 를 부르다 죽은 사고가 목록 주석에 남아 있다).
 *
 * 이제 그 세 가지가 목록의 `owned=1` 한 글자에서 생성된다. 호스트는 이 구조체를 하나 들고
 * `createAll()` · `bindInto()` 만 부른다.
 *
 * [왜 `Common/` 이 아니라 루트인가]
 * 이 저장소는 서비스 스무 개의 **완전한 타입**을 안다. `Engine/Common` 은 티어 0(엔진의 아무것도
 * 참조하지 않는 토대)이라 거기 두면 레이어 규칙을 어긴다 — `CheckEngineLayers` 가 12건으로 잡아냈다.
 * 루트(`Source/Engine` 바로 아래)는 티어 6, **"전부를 엮는 자리"** 이고 `EngineLoop` 이 사는 곳이다.
 * 예외 목록에 이름을 적는 대신 **맞는 자리로 옮겼다.**
 *
 * [무엇을 안 하는가 — 일부러]
 * **초기화 순서와 종료 순서는 생성하지 않는다.** 이 저장소는 "누가 만들고 누가 들고 있는가" 만
 * 안다. 무엇을 언제 `initialize()` 하고 어떤 순서로 내리는지는 호스트가 **손으로 적는다** —
 * 그 순서는 이 저장소가 알 수 없는 사실(디바이스가 죽기 전에 무엇을 놓아야 하는지, 씬이 사라진
 * 뒤에 모듈을 내려야 한다는 것)로 정해지고, 그 줄마다 과거에 한 번씩 무너진 이유가 주석으로
 * 붙어 있다. 순서를 자동으로 정하려면 그 지식을 의존성으로 다시 적어야 하는데, 그건 같은 것을
 * 두 번 적는 일이다.
 */
#pragma once
...
// owned 열로 갈라지는 자리들. `SW_CONCAT` 으로 0/1 을 붙여 고른다 — `if constexpr` 로는
...
namespace sw
{
     ...
     * @note 소멸 순서는 **선언의 역순**, 즉 목록의 역순이다. 그래도 호스트는 자기 종료 절차에서
     *       필요한 것을 먼저 명시적으로 놓는다 — 순서가 중요한 것들은 거기서 정해진다.
     */
    struct SW_API EngineOwnedServices
    {
        /** @brief 빈 저장소. 만드는 것은 `createAll()` 이 한다. */
        EngineOwnedServices();
        /**
         * @brief 남은 소유를 놓습니다.
         * @details **정의는 `.cpp` 에 있다.** 여기서 인라인으로 두면 이 헤더를 include 하는 모든 TU 가
         *          서비스 스무 개의 완전한 타입을 알아야 한다(unique_ptr 의 소멸자가 그렇다).
         *          실제로 그렇게 두었다가 엔진 곳곳이 컴파일되지 않았다.
         */
        ~EngineOwnedServices();
...
#undef SW_ENGINE_SERVICE_OPT
         ...
         * @details **이미 있는 것은 건드리지 않는다.** 호스트가 먼저 만들어야 하는 것이 있기 때문이다 —
         *          `EngineLoop` 은 명령줄을 파싱하려고 `CommandLineManager` 와 `GlobalVariableManager` 를
         *          이 호출보다 앞에서 만든다. 덮어썼다면 파싱 결과가 통째로 사라졌을 것이다.
         *          나머지는 목록 순서로 만든다 — 이 타입들의 생성자는 서로를 보지 않으므로 순서에 뜻이
         *          없다. **뜻이 있는 것은 초기화 순서**고 그것은 호스트가 적는다.
         */
        void createAll();

        /**
         * @brief 만든 것을 서비스 표에 꽂습니다. `owned=0` 자리는 **건드리지 않습니다.**
         * @details 그래서 호스트는 이 호출 전후 어느 쪽에서든 자기 몫(팩토리·조건부 생성)을 채울 수 있다.
         */
...
#undef SW_ENGINE_SERVICE_OPT
        ...
        /**
         * @brief 남은 소유를 전부 놓습니다.
         * @details 종료 절차에서 순서가 중요한 것은 호스트가 **먼저** 놓는다. 이것은 그 뒤에 남은
         *          것을 쓸어 담는 자리다 — 그래야 목록에 줄을 더한 사람이 종료 코드를 잊어도
         *          객체가 새지 않는다.
         */
        void destroyAll();
    };
