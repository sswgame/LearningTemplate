 ...
 * @brief 구운 산출물이 최신인지 판정합니다. **파일 시간이 아니라 내용 해시**로 봅니다.
 * @details 이 저장소는 구운 바이너리까지 커밋하므로 `git pull` 이 소스와 산출물의 mtime 을 임의의 순서로 덮어씁니다.
 *          예전 판정(`산출물 mtime >= 소스 mtime`)은 그때 "이미 최신" 이라 답했고, 그래서 `forwardlit` 바이너리가
 *          라이트 버퍼 이전 것으로 커밋된 채 돌았습니다. Vulkan 만 다른 그림을 내는 것을 백엔드 버그로 오인해 오래 쫓았습니다.
 *          그 판정이 이 파일 하나에 모여 있습니다(`ShaderBakeStampTest.FreshnessIsJudgedByContentNotFileTime` 이 고정합니다).
 */
#include "pch.h"
...
namespace sw
{
    namespace
    {
        struct ShaderBakeStampInternal
        {
            ...
            /**
             * @brief 베이크 스탬프 헤더입니다. **버전을 올리면 스탬프가 모두 불일치가 되어 한 번 전부 다시 굽습니다.**
             * @details 2 → 3 은 포맷이 아니라 **판정 기준**이 바뀐 것입니다(파일 시간 → 내용 해시).
             *          3 이전 스탬프는 "구운 것이 소스와 같다" 는 보장을 하지 못하므로 믿지 않습니다.
             */
             ...
             * @details 셰이더 요청 하나가 같은 소스를 세 번 읽었습니다(`ShaderCache::getOrCompile` 의 메모리 캐시 확인 · 로컬 캐시
             *          경로 · 굽기 신선도). 퍼뮤테이션 · 단계마다입니다. 이 PC 에서 파일 하나 여는 데 ~200 us 라 시작 시간 게임
             *          스레드의 12.6 % 가 이 읽기였습니다. 이제 두 번째부터는 파일을 열지 않고 크기 · 시각만 봅니다(~75 us). 실행 중에
             *          고친 파일은 시각이 달라져 다시 읽습니다. 편집을 알아채는 것은 예전과 같습니다.
             */
             ...
             * @details 베이크 스탬프 · 베이크 판정 · 컴파일 캐시 키가 **같은 값**을 써야 합니다. 예전에는
             *          스탬프만 내용 해시였고 판정은 파일 시간이었는데, 이 저장소는 구운 바이너리까지
             *          커밋하므로 `git pull` 이 소스와 산출물의 mtime 을 **임의의 순서**로 덮어씁니다.
             *          그러면 소스가 바뀌었는데도 "산출물이 더 새것" 이라 판정돼 그대로 넘어갑니다.
             *          실제로 그 일이 일어나 `forwardlit` 바이너리가 라이트 버퍼 이전 것으로 커밋됐고,
             *          Vulkan 만 다른 그림을 내는 것을 **백엔드 버그로 오인**해 오래 쫓았습니다.
             */
            ...
            /**
             * @brief `bin/<rhi>/bake.stamp` 를 읽어 소스 해시 표를 반환합니다(폴더당 한 번 읽고 캐시합니다).
             * @details 스탬프 버전이 다르면 **빈 표**를 반환합니다. 그러면 모든 것이 낡은 것으로 판정돼
             *          한 번 전부 다시 굽습니다. 버전을 올리는 것이 곧 강제 재굽기입니다.
            ...
            static const StampInfo& readBakeStamp( const string& binDirectory, const string& shadersDir )
            {
                ...
                if ( FileUtil::readTextFile( stampPath, text ) && text.compare( 0, kBakeStampHeader.size(), kBakeStampHeader ) == 0 )
                {
                    ...
                    while ( pos != string::npos )
                    {
                        ...
                        // **갓 체크아웃한 트리가 통째로 "낡음"** 으로 판정된다(모두 다시 굽는다).
