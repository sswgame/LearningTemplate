...
namespace sw
{
    ...
    {
        // **경로에 유효 소스 해시를 한 칸 끼운다.** 예전에는 경로가 소스와 무관하고 "캐시 파일이 소스보다
    ...
    ShaderCompileResult ShaderCache::getOrCompile( const ShaderCompileDesc& desc )
    {
        ...
        //
        // **지금 소스에서 나오지 않은(낡은) 바이너리는 쓰지 않는다.** 예전에는 .dxil 이 있기만 하면 무조건 이겼다.
        ...
        // **신선도는 `bake.stamp` 의 내용 해시로 본다.** 예전에는 구운 파일의 mtime 과 소스의 mtime 을
        ...
        // 버그로 오인했다. 검사가 개발 빌드 전용인 것은 그대로다. 배포에는 다시 컴파일할 길이 없어
