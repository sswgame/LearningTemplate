...
namespace sw
{
    ...
    public:
         ...
         * @details 채우기는 프리미티브당 ~12 ns 라 8000 개가 직렬 100 us 입니다. 잡 디스패치 바닥(~50 us)과 같은
         *          자릿수라 지금은 병렬이 86 us 로 비기는 수준입니다. 프리미티브당 일이 늘면(LOD 선택 · 스키닝 바운드)
         ...
         * @details 렌더 스레드가 처음 그릴 때 만들던 것을 **그리기 전에** 만들어 두기 위한 것입니다. 큐가 만들어
    ...
    private:
         ...
         * @details 투명 인스턴스는 불투명 뒤에 연속으로 앉습니다(`_opaqueInstanceCount`). 예전에는 투명 순서가
         *          바뀌면 `refreshInstancesInPlace` 가 실패해 **불투명 8000 개까지** 통째로 다시 지었습니다.
         *          투명 큐브가 위아래로 흔들리는 벤치에서 거의 매 프레임 그랬습니다(배치 단계 최소 139 · p50 327 us).
         *          접두부는 제자리 갱신이 맡고, 여기서는 꼬리를 잘라 내고 새 순서로 다시 붙입니다.
         * @note **접두부 갱신과 같은 병렬 구간에서 돕니다**(`refreshInstancesInPlace` 의 블록 0). 워커에서 돌 수도 있습니다.
         *       쓰기 슬롯의 [0, `_opaqueInstanceCount`) 는 워커들이 포인터로 쓰는 중이라 건드리지 않습니다: 자르고 뒤에 붙일 뿐이고,
         *       붙일 자리는 부르는 쪽이 미리 잡아 둡니다(재할당이 없습니다). 접두부 청크가 읽는 것(발행본 · raw · 원천 인덱스의 접두부)도
         *       쓰지 않습니다. 여기에 접두부를 만지는 일을 더하면 레이스입니다.
         */
        ...
        /**
         * @brief 배치 키에 쓸 머티리얼입니다. 합치기가 켜져 있으면 같은 퍼뮤테이션의 대표 머티리얼, 아니면 그 머티리얼 자신입니다.
         * @details 대표는 재구축마다 처음 만난 머티리얼입니다(_mapShaderRepresentative). 재구축 여부 판단은 후보 자체를 비교하므로 대표가 바뀌어도 무관합니다.
         *          불투명 나누기만 부릅니다. 투명 방출은 해시가 같다는 것까지 본 뒤라 맵 없이 같은 답을 냅니다(`emitTransparentBatches`).
         *          대표는 **퍼뮤테이션 단위**입니다. 같은 .hlsl 이라도 정적 스위치가 다르면 다른 셰이더이므로 합칠 수 없습니다.
         */
        Material* batchKeyMaterial( Material* pMaterial, uint64 permutationHash );
        /**
         * @brief 투명 인덱스를 카메라 거리순(먼 것 → 가까운 것)으로 정렬합니다.
         * @details 지난 프레임 순서에서 삽입 정렬로 출발하고, 옮김이 원소당 `kTransparentInsertionMovesPerElement` 를 넘으면
         *          std::sort 로 넘깁니다. 순서는 전순서라 어느 쪽이든 결과가 같습니다.
         */
        void sortTransparent( const float3& cameraPos );
        /**
         * @brief 투명 정렬이 삽입 정렬로 버틸 옮김 예산입니다(원소당 칸 수).
         * @details 한 프레임에 조금씩 움직이는 씬은 뒤집힌 쌍이 원소 수보다 훨씬 적습니다. 이 예산을 넘는 프레임(카메라가 크게
         *          돌았다 · 나누기를 다시 했다)은 삽입 정렬을 멈추고 std::sort 가 마저 합니다. 최악이 N log N 에 예산만큼 더한 값입니다.
         */
        static constexpr size_t kTransparentInsertionMovesPerElement = 8;
        /** @brief 불투명 · 투명 인덱스 표를 후보에서 다시 만듭니다. */
        void rebuildPartitionTables();
        /**
         * @brief 배치를 다시 나누지 않고 인스턴스 값만 제자리에서 갱신합니다.
         * @details 배치 키가 그대로일 때 씁니다. 그러면 불투명 접두부의 자리 배치와 각 원소의
         *          `_meshBatchIndex` · `_materialIndex` 가 그대로라 바뀐 것은 트랜스폼 · 바운드 · 회전 시드뿐입니다.
         *          투명 정렬 순서가 바뀌었으면 투명 꼬리만 다시 짓습니다(`rebuildTransparentTail`).
        ...
        /// @brief 셰이더 경로 → `_snapshot._listMaterialGroup` 인덱스입니다. 예전에는 배치마다 그룹 목록을 string 비교로 훑었습니다.
        unordered_map<string, uint32> _mapShaderPathToGroup;

        /**
         * @brief 머티리얼 그룹의 원소 인덱스 표입니다. `_snapshot._listMaterialGroup` 과 같은 인덱스로 나란히 갑니다.
         * @details 스냅샷의 그룹에는 RT 가 읽는 것(경로 · 원소)만 남기고 표는 여기 둡니다. 프레임마다 패킷으로 복사되지 않습니다.
         */
        ...
        /// @brief 빌드 번호입니다. 원소가 마지막으로 쓰인 시점을 재는 데만 씁니다(회수 판정).
        uint64 _buildCounter{ 0 };
        /**
         * @brief 마지막 수집 때 본 퍼뮤테이션 세대입니다(`MaterialUtil::getPermutationGeneration`).
         ...
         *          한 대표로 접혀 같은 배치가 되고, 배치는 PSO 하나로 그리므로 한쪽 퍼뮤테이션이 통째로 사라졌습니다.
         */
        ...
        struct DrawCandidate
        {
             ...
             *          다시 나뉘지 않아 **바뀐 퍼뮤테이션이 화면에 반영되지 않았습니다**(런타임에 정적 스위치를
             ...
             * @details 예전에는 후보마다 100여 바이트를 FNV 로 섞어 64비트 지문을 만들어 비교했습니다.
             *          그것은 (1) 바이트마다 곱셈이 들어가 이 비교 자체가 수집 비용에 맞먹었고,
             *          (2) 해시가 충돌하면 바뀐 씬을 "그대로" 로 보고 화면이 멈추는, 재현이 사실상
             *          불가능한 버그를 남겼습니다. 어차피 같은 바이트를 다 읽어야 한다면 지문을 만들지 말고
             *          **그냥 비교**하는 것이 더 싸고 정확합니다. 다르면 즉시 빠져나올 수도 있습니다.
             * @note 부동소수는 float3 · float4x4 의 `operator==` (nearEqual, 엡실론 비교)가 아니라
             *       **비트 단위로** 비교합니다. 엡실론 비교는 매 프레임 엡실론 미만으로 움직이는 물체를
             *       영원히 "안 바뀜" 으로 보고 화면에 오차를 누적시킵니다. 반대로 비트 비교가 틀리는
             *       방향(-0.0 과 0.0 을 다르게 봄)은 불필요한 재구축일 뿐이라 안전합니다.
             *       비교 대상 블록은 모두 float 연속이라 패딩이 끼지 않습니다.
             */
         ...
         * @details 머티리얼이 없으면 씬 기본 머티리얼을 씁니다(언리얼의 기본 머티리얼). **블렌드 모드는 머티리얼의 성질입니다.**
         *          언리얼도 블렌드 모드가 머티리얼 에셋에 있고, 그 값이 셰이더 퍼뮤테이션(불투명 · 반투명)을 가릅니다. 메시가 뒤집을 수
         *          있게 두면 불투명으로 컴파일된 머티리얼을 블렌딩으로 그리는 어긋난 상태가 만들어집니다. 블렌드 모드는 후보의
         *          머티리얼에서 읽고, 그것도 없으면(씬 기본 머티리얼도 없음) 인스턴스의 부모 머티리얼, 그것마저 없으면
         *          `fallbackBlendMode`(메시 쪽 값)를 씁니다. 머티리얼이 없는 디버그 · 픽스처 메시가 마지막 경우입니다.
         *          인스턴스만 붙은 메시도 씬 기본 머티리얼이 있으면 **그것이** 후보의 머티리얼이 됩니다(인스턴스의 부모가 아닙니다).
         *          (예전에는 인스턴스를 이 판단 **뒤에** 채워서 인스턴스 쪽이 한 번도 걸리지 않았습니다.)
         */
        ...
        /**
         * @brief 후보 인덱스 -> 마지막 방출이 그 후보에 준 머티리얼 원소 인덱스입니다.
         * @details 원소 인덱스는 (머티리얼, 인스턴스) 쌍에 영속이라 배치 키가 그대로인 프레임에는 다시 묻지 않아도 됩니다.
         ...
         * @details 워커가 자기 칸을 채우면서 **지난 후보와의 비교까지** 한 번에 끝냅니다. 예전에는 직렬 패스 둘이
         *          같은 8000 칸을 따로 지나갔습니다: 해시 찍기(모두, 머티리얼 게터 둘씩) · 배치 키 비교(모두).
        ...
        /** @brief 후보 인덱스 -> 인스턴스 슬롯입니다(`kInvalidCandidateIndex` = 인스턴스 없음). `_listInstanceSrcIndex` 의 역입니다. */
        vector<uint32> _listCandidateToInstance;
        /** @brief 마지막 수집이 만든 후보 수입니다. 부분 수집이 자리 수를 그대로 이어받습니다. */
        size_t _lastCandidateCount{ 0 };
        /** @brief 부분 수집이 한 프리미티브를 채워 볼 임시 자리입니다(프레임마다 할당하지 않습니다). */
        ...
        struct SortKey
        {
            ...
            /**
             * @brief (셰이더 경로 + define) 해시입니다. 배치가 쓸 PSO 를 정합니다.
             * @details **대표 머티리얼 포인터로는 대신할 수 없습니다.** 합치기가 켜지면 대표는 "이 퍼뮤테이션을
             *          처음 들고 온 머티리얼" 인데, 퍼뮤테이션이 **인스턴스**에서 오면 부모가 같아 대표도
             *          같아집니다. 서로 다른 셰이더로 그려야 할 것들이 한 배치로 접힙니다. 해시를 키에 직접
        ...
        /// @brief 정렬된 불투명 항목의 후보 인덱스만 뽑은 것입니다. `emitBatch` 가 투명 쪽과 같은 모양으로 받습니다.
        vector<uint32> _listScratchOpaqueIdx;
        vector<uint32> _listScratchTransparentIdx;
        /**
         * @brief 부분 업로드로 나눌 구간 수 상한입니다. 넘으면 전체를 올립니다.
         *
         * @details 구간들은 **한 번의 호출**로 올라가므로(`updateStructuredBufferRegions`) 비용이 구간
         *          수에 비례하지 않습니다. 그래도 상한을 두는 이유는 두 가지입니다: 구간 목록 자체가 스냅샷에
         *          실려 복사되고, 변경이 배열 전체에 고르게 흩어졌다면 통째로 올리는 것이 단순하고 쌉니다.
         *
         *          (구간마다 따로 부르던 때는 이 값이 성능을 직접 좌우했습니다: DX12 · 8000 인스턴스에서
         *          구간 1/2/5/20/64 가 17/34/45/113/209 us 였고 통째로는 ~100 us 였습니다.)
         */
        static constexpr size_t kMaxDirtyInstanceRun = 256;

        /**
         * @struct InstanceRefreshChunk
         * @brief 전체 제자리 갱신을 청크로 나눠 돌릴 때 청크 하나의 결과입니다.
         * @details 청크는 인스턴스 슬롯의 연속 구간이라 더티 구간(run)도 청크 안에서 만들고, 끝난 뒤
         *          경계가 맞닿는 구간만 이어 붙입니다. 회전 인스턴스 수도 청크마다 세어 합칩니다.
         */
        struct InstanceRefreshChunk
        {
            uint32         _start{ 0 };
        ...
        /// @brief 마지막 방출이 쓴 투명 정렬 순서입니다. 이것이 바뀌면 투명 꼬리를 다시 방출합니다(`rebuildTransparentTail`).
        vector<uint32> _listBuiltTransparentIdx;
        /// @brief 마지막 전체 빌드에서 불투명이 차지한 인스턴스 수 · 배치 수 · 원소 표 항목 수입니다. 투명 꼬리를 자르는 자리입니다.
         ...
         * @details 예전에는 비교 함수가 원소마다 raw 에서 바운드를 읽어 거리를 **다시** 구했습니다. 투명 2000 개면
         *          비교 22000 번에 무작위 읽기 44000 번이고, 그 raw 는 다른 코어가 방금 쓴 것이라 원격 캐시에서 왔습니다.
         *          키를 한 번 계산해 두면 읽기는 2000 번이고 정렬은 12 바이트 연속 배열 위에서 돕니다.
         */
         ...
         * @details 회수 시계 도장은 **원소**에 찍는 것이라 인스턴스 8000 개가 아니라 (배치, 원소) 쌍만 돌면 됩니다.
