...
namespace sw
{
    ...
    private:
         ...
         *          인스턴스의 원소 바이트 · 퍼뮤테이션은 부모의 모양을 따르므로, 배치의 머티리얼 · 그룹 · 텍스처 · stride 도 부모의
         *          것이어야 합니다. 예전에는 씬 기본이 먼저여서 인스턴스만 붙은 메시가 두 머티리얼이 섞인 배치가 됐습니다.
         *          **블렌드 모드는 머티리얼의 성질입니다.** 언리얼도 블렌드 모드가 머티리얼 에셋에 있고, 그 값이 셰이더
         *          퍼뮤테이션(불투명 · 반투명)을 가릅니다. 메시가 뒤집을 수 있게 두면 불투명으로 컴파일된 머티리얼을 블렌딩으로
