 ...
 *
 *          래퍼 클래스는 일부러 두지 않습니다. 예전에 있던 `ComputePass` 는 아무도 쓰지 않은 채 남아 있어 지웠습니다
 *          (Renderer/README). 각자의 상수버퍼는 `RHIConstantBufferSlot` 이고 소유는 `FrameRenderer` 입니다.
 */
#include "pch.h"
...
namespace sw
{
     ...
     *          2 만으로 "컴퓨트가 범인인가" 가 갈립니다(백로그 1-4 는 이 모드로 컴퓨트를 무죄로 만들었습니다).
        ...
        if ( _gpuScene.isUploaded() && _gpuScene.areIndirectCountsGpuFilled() )
        {
                 ...
                 _gpuScene.getBatchInfoSrv() != kInvalidDescriptorIndex )
            {
                ...
                for ( uint32 viewIndex = 0; viewIndex < static_cast<uint32>( RenderViewType::Count ); ++viewIndex )
                {
                    ...
                    // **인스턴스마다** 스레드 하나다(예전에는 배치마다 하나였다). 그래야 보이는 것을 골라 압축할 수 있다.
                    ...
                    // 완료 순서라 그대로 두면 블렌딩이 틀린다. 예전에는 그래서 투명이 압축을 통째로
