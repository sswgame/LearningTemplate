 ...
 * @brief FrameRenderer 가 만든 PSO 들의 저장소 — 엔진 패스 PSO · Present 포맷별 PSO · 머티리얼 퍼뮤테이션 변형과 그 바인딩 레이아웃.
 * @details **만드는 일은 하지 않는다.** PSO 를 만드는 데는 파이프라인 XML·디바이스·씬 배치가 필요하고 그것은 FrameRenderer 의
 *          일이다(FrameRendererPso.cpp). 여기는 "만든 것을 어디에 두고, 누가 소유하고, 어떤 순서로 놓는가" 만 안다 —
 *          그 세 가지가 예전에 FrameRenderer 멤버 일곱 개와 뮤텍스 둘에 흩어져 있어, 해제 순서(변형 → 패스 PSO)가
 *          한 함수 안의 주석으로만 지켜졌다.
 *
 *          드로우 경로가 배치마다 조회하므로 조회는 락 하나로 짧게 끝난다. 삽입은 기록 시작 전에만 일어난다.
 */
#pragma once
...
namespace sw
{
     ...
     * @brief PSO 핸들과 레이아웃의 소유자. 해제 순서와 뮤텍스가 여기 한 곳에 있다.
     */
    ...
    public:
         ...
         * @brief (패스, 머티리얼 퍼뮤테이션, 뷰 모드) 하나에 대응하는 PSO.
        ...
        /** @brief 등록된 PSO 하나와 그 바인딩 레이아웃 — `collectLayouts` 가 돌려준다. */
        ...
        // ------------------------------------------------------------------------------
        /**
         * @brief 전부 파괴하고 비웁니다 — 디바이스가 **살아 있을 때**.
         * @details 순서가 규칙이다: 머티리얼 변형(소유한 것만) → 엔진 PSO → Present PSO → 레이아웃 표.
         *          `_bOwned` 가 0 인 변형은 패스 PSO 를 그대로 담고 있을 뿐이라 먼저 걸러야 두 번 파괴하지 않는다.
         */
    ...
    private:
        ...
        /// @brief Present PSO 를 대상 렌더타깃 포맷별로 — 백버퍼와 GameView RT 는 포맷이 다를 수 있다.
