/**
 * @file PassConstantRing.h
 * @brief 프레임마다 되감는 패스 상수버퍼 슬롯 링입니다. 드로우마다 하나씩 나눠 줍니다.
 * @details `updateConstantBuffer` 는 버퍼의 **프레임 슬롯 하나**에 쓰고 GPU 는 제출 뒤에 읽습니다. 그래서 여러 드로우가
 *          같은 버퍼를 쓰면 모두 마지막에 쓴 값을 봅니다. 한 패스의 드로우들이 서로를 덮어쓰던 버그가 그것이었습니다
 *          (RenderPassGpuTest.MultiBatchPassKeepsPerBatchConstants). 언리얼도 드로우별 느슨한 파라미터는 드로우마다
 *          유니폼 버퍼를 따로 잡습니다. 여기서는 슬롯을 미리 만들어 두고 원자 커서로 나눠 줍니다. 패스가 병렬로
 *          기록되므로 락 없이 분배해야 합니다.
 *
 *          0 번 슬롯은 **프레임 시드 전용**입니다(직렬 경로의 `FramePassContext` 가 듭니다). 패스와 드로우는 1 번부터 받습니다.
 */
#pragma once
#include "Core/Concurrency/atomic.h"
#include "Core/Container/vector.h"

#include "Engine/EngineMinimal.h"
#include "Engine/Graphics/RHI/RHIConstantBufferSlot.h"
#include "Engine/Graphics/RHI/RHITypes.h"

namespace sw
{
    class IRHIDevice;

    /**
     * @class PassConstantRing
     * @brief 패스 · 드로우 상수버퍼 슬롯의 소유자입니다. 만들고(initialize · ensureCapacity), 나눠 주고(acquire), 놓습니다(release).
     */
    class SW_API PassConstantRing
    {
    public:
        /// @brief 처음 만들어 두는 슬롯 수입니다. 배치 수에 따라 `ensureCapacity` 가 더 키웁니다.
        static constexpr uint32 kInitialSlotCount = 64;
        /// @brief 슬롯 상한입니다. 넘으면 에러를 남기고 마지막 슬롯을 공유합니다(그 프레임은 배치 상수가 섞입니다).
        static constexpr uint32 kMaxSlotCount = 4096;
        /// @brief 슬롯 하나의 바이트 수입니다. 엔진 PassCB 레이아웃(리플렉션)이 이 안에 들어야 합니다.
        static constexpr uint32 kSlotBytes = 512;

        PassConstantRing();
        ~PassConstantRing() = default;

        PassConstantRing( const PassConstantRing& )            = delete;
        PassConstantRing& operator=( const PassConstantRing& ) = delete;

        /** @brief 초기 슬롯(kInitialSlotCount)을 만듭니다. 이미 있으면 그대로 true 입니다. */
        bool initialize( IRHIDevice* pDevice );
        /** @brief 슬롯을 디바이스에 돌려주고 비웁니다. 디바이스가 **살아 있을 때** 부릅니다. */
        void release( IRHIDevice* pDevice );
        /** @brief 디바이스가 이미 사라졌을 때 부릅니다. 핸들만 잊습니다(destroy 를 부르면 해제 후 사용입니다). */
        void forget();

        /** @brief 프레임 시작: 커서를 1 번 슬롯으로 되감고 고갈 경고 래치를 풉니다. */
        void beginFrame();
        /**
         * @brief 슬롯을 하나 빌립니다. **드로우마다** 하나씩입니다. 슬롯이 하나도 없으면 false 입니다.
         * @details 커서는 원자적이라 병렬 기록에서도 안전합니다. 슬롯 배열은 기록 시작 전에 잡아 두고 병렬 구간에서
         *          크기가 변하지 않습니다. 모자라면 마지막 슬롯을 공유하고 프레임당 한 번 경고합니다.
         */
        bool acquire( RHIBufferHandle& outBuffer, RHIDescriptorIndex& outIndex );
        /**
         * @brief 슬롯 수를 `needed` 이상으로 늘립니다. **기록 시작 전에만** 부릅니다.
         * @details 버퍼 생성과 bindless 등록은 레지스트리를 바꾸므로 병렬 기록 중에는 할 수 없습니다
         *          (IRHIDevice::setParallelRecording). 배치 수를 아는 프레임 시작 지점에서 미리 키웁니다.
         */
        void ensureCapacity( IRHIDevice* pDevice, uint32 needed );

        /** @brief 0 번 슬롯입니다. 프레임 시드 전용이고, 없으면 false 입니다. */
        bool getSeedSlot( RHIBufferHandle& outBuffer, RHIDescriptorIndex& outIndex ) const;
        /** @brief 지금 만들어 둔 슬롯 수입니다. */
        uint32 getSlotCount() const { return static_cast<uint32>( _listSlot.size() ); }
        /** @brief 지금까지 한 프레임에서 쓴 슬롯 수의 최댓값입니다. 다음 프레임 용량 산정의 바닥값입니다(단조 증가). */
        uint32 getHighWater() const { return _highWater.load( std::memory_order_relaxed ); }

    private:
        /** @brief 슬롯을 하나 더 만듭니다. 못 만들면 false 입니다. */
        bool appendSlot( IRHIDevice* pDevice );

        vector<RHIConstantBufferSlot> _listSlot;
        atomic<uint32>                _cursor;
        atomic<uint32>                _highWater;
        /// @brief 슬롯 고갈 경고를 프레임당 한 번만 남기기 위한 래치입니다.
        atomic<uint8> _bExhaustedLogged;
    };
} // namespace sw
