...
namespace sw
{
    ...
    /// @brief 엔진(PassCB 등) 상수 버퍼 슬롯입니다(버퍼 핸들 + bindless 인덱스).
    ...
    {
        /**
         * @brief 그래픽스 드로우 바인딩을 합니다.
         * @param bNativeBindless true 면 텍스처 인덱스는 이미 CB 에 기록되어 따로 바인딩할 필요가 없습니다(DX12/VK).
         *        구조버퍼는 텍스처와 달리 네이티브 bindless 여도 백엔드별로 명시 바인딩이 필요할 수 있어
         *        이 플래그로 건너뛰지 않습니다(각 백엔드의 bindStructuredBuffer 가 스스로 판단합니다).
