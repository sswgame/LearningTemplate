 ...
 * @brief 풀스크린 패스 타입이 **읽는 입력의 역할** — 로드 시점 검증과 프레임 실행이 같은 표를 본다.
 ...
 *          정렬에만 쓰였다. 그래서 디퍼드 XML 이 Bloom 의 입력으로 `AOColor` 를 적어 두어도 아무도 걸지
 *          않았고 SSAO 는 매 프레임 돌고 버려졌다(백로그 1-6). 선언이 곧 바인딩이 되려면 "이 패스
 *          타입은 어떤 역할의 입력을 읽는가" 가 한 표에 있어야 한다 — 그 표가 이 파일이다.
 */
#pragma once
...
namespace sw
{
     ...
     * @brief 첨부가 패스 안에서 맡는 역할. 셰이더는 역할 이름(`g_<Role>Index`)으로 읽는다.
     * @details 첨부 **이름**은 파이프라인이 정하고(LitColor, BloomColor …) 역할은 패스가 정한다. 같은
     *          첨부가 Bloom 에서는 SourceColor 이고 Transparent 에서는 렌더 타깃이다.
     */
    ...
    /** @brief 역할의 셰이더 이름 — `g_<Name>Index` 의 `<Name>` 이자 FrameResourceRegistry 의 키. */
     ...
     *          SceneDepth, 나머지 컬러는 SourceColor 다. ShadowMap 은 깊이 포맷이지만 이름이 먼저다.
     */
     ...
     * @brief 패스 타입 하나가 읽는 역할 목록. 필수가 빠지면 검증 오류, 목록에 없는 역할을 선언해도 오류다.
     */
    struct RenderPassInputContract
    {
        ...
        /** @brief 이 역할을 읽는가 (필수 또는 선택). */
        bool reads( RenderPassInputRole role ) const;
    };

    /**
     * @brief 풀스크린 패스 타입의 입력 계약을 돌려줍니다. 메시 패스(Shadow · GBuffer · ForwardOpaque · Transparent …)는 nullptr.
     * @details 메시 패스의 입력은 지오메트리 드로우가 정하고(그림자·인스턴스·머티리얼), 선언은 그래프 순서용이다.
     */
