/**
 * @file Material.h
 * @brief XML 머티리얼 에셋과 MaterialInstance 오버라이드입니다(UE MIC / Unity MaterialPropertyBlock).
 * @details
 *  - 마스터 머티리얼: 프로퍼티 스키마 + `_defaultValue` + 퍼뮤테이션
...
namespace sw
{
    ...
    public:
         ...
         * @details 생성자가 이 열쇠를 요구하므로 `make_shared<Material>()` 도 스택의 `Material x;` 도 **컴파일되지 않습니다.**
         *          모든 Material 이 Engine 안에서 shared_ptr 로 태어난다는 것을 컴파일러가 보장합니다. 렌더 패킷이
        ...
        /** @brief create() 전용 생성자입니다. 열쇠 없이는 만들 수 없습니다. */
        explicit Material( CreateKey );
        /** @brief 빈 머티리얼을 Engine.dll 안에서 shared_ptr 로 만듭니다. Material 을 얻는 유일한 길입니다. */
        ...
        /**
         * @brief 프로퍼티의 오프셋 · 크기대로 바이트 버퍼를 다시 채웁니다. 리플렉션 동기화의 두 갈래가 같은 길을 씁니다.
         * @param minimumByteSize 셰이더가 선언한 크기. 프로퍼티가 그 밖으로 나가면 그만큼 늘리고, 끝은 256 정렬합니다.
         */
         ...
         * @details `getDesc()._shaderPath` 를 직접 쓰면 캐시가 옛 경로를 계속 들고 있습니다. 경로를
         *          바꾸는 곳은 여기를 씁니다.
         */
        ...
        /**
         * @brief GPUScene 머티리얼 데이터 버퍼(g_SwMaterials)의 원소 stride 입니다. 셰이더 리플렉션의 구조버퍼 원소 크기입니다.
         * @details 0 이면 셰이더가 SW_MATERIAL_BEGIN/END 를 쓰지 않습니다(레거시 MaterialCB). 백엔드마다 다를 수 있습니다.
         */
    ...
    private:
        /// @brief 비동기 파일 로드가 머티리얼이 아직 살아 있는지 확인하는 상태입니다(소멸자가 포인터를 끊습니다).
        ...
        /** @brief Desc 를 런타임 상태에 적용합니다. */
        void applyDescToRuntime();
        /** @brief 런타임 프로퍼티를 Desc 에 다시 씁니다(저장용). */
        void syncDescFromRuntime() const;
        /** @brief 비동기 로드 태스크 본문입니다. TaskArgs 는 AsyncLoadState shared_ptr · 경로 문자열입니다. */
        static void loadFromFileAsyncJob( const TaskArgs& args );

        /** @brief initialize 가 받은 에셋 경로입니다. 디바이스가 바뀌면 이것만으로 스스로 되살아납니다. */
        string                     _assetPath;
        MaterialDesc               _desc;
        MaterialData               _data;
