 ...
 *
 * @details 렌더 스레드는 그리기만 해야 합니다. 생성은 그리기 전에 이미 끝나 있어야 합니다. 예전에는 메시가
 *          `GpuScene::upload`(RT) 안에서 처음 그려질 때 만들어졌고, 그래서 새 메시가 등장한 프레임은 RT 가
 *          정점 버퍼 생성 비용을 통째로 뒤집어썼습니다(백엔드 교체처럼 모두 다시 올려야 할 때는 더 큽니다).
 *
 *          여기서는 게임 스레드가 "이번 프레임에 그릴 것" 을 알고 있으므로, 스냅샷을 내보내기 전에 아직
 *          올라가지 않은 것을 워커로 넘겨 **병렬로** 만듭니다. 만들기가 끝난 뒤에 스냅샷이 나가므로 팝인이
 *          없고 프레임 그림도 그대로입니다(스크린샷 비교가 계속 유효합니다).
 *
 *          **RT 경로는 그대로 남습니다.** `Mesh::initRhi` 는 이미 올라가 있으면 아무것도 하지 않는 멱등 함수라,
 *          큐가 먼저 만들어 두면 RT 의 그 호출은 확인만 하는 일이 됩니다. 큐가 꺼진 백엔드(OpenGL)나 큐가
 *          미처 다루지 못한 메시는 RT 가 예전처럼 그 자리에서 만듭니다. 이 큐는 **앞당기는 장치**이지
 *          유일한 통로가 아닙니다. 그래서 이것 때문에 화면이 비는 경우는 없습니다.
 *
 * @note **왜 렌더 스레드와 워커가 같은 메시를 동시에 만들 수 없는가.** 게임 스레드는 스냅샷을 내보내기 전에
 *       flush 하므로, 어떤 메시든 그것이 든 패킷이 렌더 스레드에 닿기 전에 이미 상주합니다. 렌더 스레드가 직전
 *       패킷을 그리며 같은 메시에 `initRhi()` 를 불러도 그때는 확인만 합니다. 이 불변식이 깨지는 길은 하나뿐입니다.
 *       **flush 를 기다리지 않고** 스냅샷을 내보내는 것입니다. 그래서 flush 는 동기입니다.
 *
...
namespace sw
{
    ...
    public:
         ...
         * @details 디바이스가 바뀌면 쌓여 있던 요청은 옛 디바이스의 것이므로 버립니다. 새 디바이스에 필요한
         *          메시는 다음 프레임의 GT 가 다시 요청합니다(상주 판단이 세대 기반이라 자동으로 걸립니다).
         */
         ...
         * @details 워커가 병렬로 만들고 게임 스레드가 기다립니다. RT 가 직렬로 만들던 것을 GT 가 병렬로
         *          기다리는 것으로 바꾼 것입니다. 기다리는 이유는 프레임 그림을 바꾸지 않기 위해서입니다.
         * @return 이번에 실제로 만든 개수입니다.
         */
        uint32 flush();
    ...
    private:
        ...
        /**
         * @brief 이번 프레임에 만들 메시입니다. **소유를 함께 듭니다.** 워커가 만드는 동안 게임 스레드가 놓아도
