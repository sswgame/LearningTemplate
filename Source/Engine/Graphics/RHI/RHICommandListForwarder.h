 ...
 * @brief 자기 커맨드 컨텍스트로 기록 API 를 그대로 넘기는 IRHICommandList 기반 클래스입니다.
 */
#pragma once
...
namespace sw
{
     ...
     * @brief `IRHICommandList` 의 기록 API 모두를 파생이 지정한 컨텍스트로 넘깁니다.
     * @tparam ContextT 백엔드의 커맨드 컨텍스트 구체 타입
     * @details 네 백엔드의 CommandList 는 모두 "자기 컨텍스트로 그대로 넘기기" 만 합니다. 그 26개
     *          메서드를 백엔드마다 복사해 두면 기록 API 를 하나 추가할 때마다 네 곳을 똑같이 고쳐야
     *          하고, 시그니처가 어긋나도 컴파일 전까지 모릅니다.
     *
     *          예전에는 이것을 매크로(`SW_FORWARD_RHI_COMMAND_LIST`)로 풀었는데, 그러면
     *          "DX12 에서 setViewport 가 어디 구현됐나" 를 정의로 점프해서 찾을 수 없습니다.
     *          읽으라고 만든 저장소에서 치르기엔 큰 비용입니다. 템플릿 기반 클래스로 바꾸면 같은
     *          중복 제거를 얻으면서 평범한 C++ 로 남습니다.
     *
     *          `ContextT` 가 구체 타입이라 호출은 **정적 디스패치**로 인라인됩니다. 매크로와 같은
     *          코드가 나옵니다. 컨텍스트는 파생 클래스의 다른 멤버(기록 상태 등)에 의존해 만들어지므로
     *          기반이 소유하지 않고, 파생이 생성자에서 `_pContext` 를 자기 것으로 가리키게 합니다.
     *
     *          특정 메서드만 다르게 처리해야 하면 파생에서 그것만 다시 override 하면 됩니다.
     */
    ...
    public:
        ...
        {
            _pContext->setGraphicsRootConstants( rootParameterIndex, num32BitValues, pData, destOffsetIn32BitValues );
        }
    ...
    protected:
        /// @brief 파생 클래스가 생성자에서 자기 컨텍스트를 가리키게 설정합니다.
