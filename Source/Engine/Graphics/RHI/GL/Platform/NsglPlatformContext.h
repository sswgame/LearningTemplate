 ...
 * @brief macOS(NSOpenGL) OpenGL 컨텍스트입니다.
 *
 * @details 구현 `.cpp` 는 파일 전체가 `SW_PLATFORM_MACOS` 가드 안에 있습니다.
 *
 * @note macOS 는 CI 에도 없습니다. 이 경로는 **어디서도 컴파일되지 않습니다.** 옮길 때 내용을 고치지
 *       않은 이유입니다(예전 `OpenGLRHIDeviceInit.cpp` 의 `#elif SW_PLATFORM_MACOS` 블록 그대로).
 */
#pragma once
