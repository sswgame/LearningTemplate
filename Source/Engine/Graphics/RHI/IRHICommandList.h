 ...
 * @brief GPU 명령을 기록하는 커맨드 리스트 인터페이스입니다.
 * @details 기록 API(뷰포트 · PSO · 렌더 패스 · 바인딩 · 드로우)를 여기 한곳에 모읍니다.
 *          `IRHICommandContext` 가 이것을 상속해 "기록 범위가 없는" 형태로 씁니다.
 */
#pragma once
...
namespace sw
{
     ...
     * @brief GPU 그래픽스 · 컴퓨트 명령을 기록하는 커맨드 리스트입니다.
     */
    ...
    public:
        ...
        /**
         * @brief 삼각형 리스트를 인스턴스드로 그립니다.
         * @details 지금 씬 드로우는 이 함수가 아니라 `drawIndirect` 를 씁니다(인스턴스 자리는 인스턴스 슬롯 스트림이 줍니다).
         * @param instanceCount 인스턴스 개수.
         * @param startInstance 시작 인스턴스. `SV_InstanceID` 가 이 값을 포함하는지는 API 마다 다르므로, 셰이더가 인스턴스 자리를
         *                      알아야 하면 인스턴스 슬롯 스트림(`constant::kInstanceSlotStreamSlot`)으로 받습니다.
         */
        ...
        /**
         * @brief 그래픽스 스테이지에 루트 · 푸시 상수를 씁니다. **드로우마다 바뀌는 소수의 값** 전용입니다.
         ...
         * @details 컴퓨트 패스(gpucull 등) 전용입니다. bindConstantBuffer 는 그래픽스 스테이지만 대상으로 합니다.
        ...
        /**
         * @brief 컴퓨트 스테이지에 읽기 전용 구조 SRV 버퍼를 지정한 레지스터 슬롯(tN)에 바인딩합니다.
         * @param index bindless 디스크립터 인덱스.
         * @param slot  HLSL `register(tN)` 의 N.
         */
        ...
        /**
         * @brief 텍스처를 렌더 타깃(또는 뎁스)으로 쓸 수 있는 상태로 만듭니다.
         * @details `prepareTextureForShaderRead` 의 짝입니다. 그래프가 웨이브를 **병렬로 기록하기 전에**
         *          단일 스레드에서 미리 불러 두는 용도입니다. 그러면 패스 콜백은 이미 맞는 상태를 보게
         *          되어, 기록 중에 리소스 상태 같은 공유 데이터를 바꾸지 않습니다. 배리어를 기록
        ...
        /**
         * @brief 텍스처를 컴퓨트가 쓰는 RW 텍스처(UAV) 상태로 만듭니다. 디스패치 전에 부릅니다.
         * @details DX12 는 UNORDERED_ACCESS 전이, Vulkan 은 GENERAL 레이아웃 전이입니다. DX11 은 할 일이 없고, GL 은 이미지 접근 배리어입니다.
         *          다시 샘플링하려면 prepareTextureForShaderRead 를 부릅니다.
         */
         ...
         * @param countBuffer          실제 개수를 GPU 가 적어 두는 버퍼(0 이면 drawCount 를 그대로 씁니다).
         *                             지원하지 않는 백엔드는 drawCount 로 폴백합니다.
         ...
         * @param argumentBuffer `RHIDispatchIndirectCommand` 하나. **그 구조체가 이 버퍼의 레이아웃 기준입니다.**
         *        값을 채우는 것은 C++ 이 아니라 컴퓨트 셰이더라, 여기 말고는 그 이름이 불릴 자리가 없습니다.
         ...
         * @details 상태가 바뀌지 않는 전이(UAV → UAV)는 `transitionBuffer` 가 아무것도 하지 않습니다.
         *          그런데 컴퓨트 두 개가 **같은 버퍼를 이어서** 쓰고 읽으면 그 사이에 장벽이 필요합니다.
         *          컬링이 채운 가시 목록을 정렬이 바로 읽는 자리가 그렇습니다. 없으면 정렬이 아직 안 채워진
         *          목록을 읽습니다(드라이버 · 백엔드마다 결과가 달라져 재현이 어렵습니다).
         *          DX12 는 UAV 배리어, Vulkan 은 버퍼 메모리 배리어, GL 은 glMemoryBarrier 이고,
         *          DX11 은 디스패치가 컨텍스트에서 직렬화되므로 할 일이 없습니다.
         */
        virtual void uavBarrier( RHIBufferHandle buffer ) = 0;
        /**
         * @brief 인덱스 버퍼를 쓰는 드로우를 GPU 가 적어 둔 인자로 한 번 발행합니다.
         * @param argumentBuffer `RHIDrawIndexedIndirectCommand` 하나. 위와 같은 이유로 그 구조체가 레이아웃 기준입니다.
         * @param argumentBufferOffset 그 버퍼 안의 바이트 오프셋.
         */
