 ...
 * @brief 네이티브 `ID3D11DeviceContext`(Deferred Context)를 소유하는 IRHICommandList 구현체입니다.
 */
#pragma once
...
namespace sw
{
     ...
     *          제출은 디바이스의 단일 Immediate Context 에서 `ExecuteCommandList` 로 합니다. DX11 은 한
     *          스레드에서 발행한 Immediate Context 호출 순서가 곧 실행 순서이므로(Vulkan 의 세마포어
     *          재정렬 문제가 없습니다) 프레임 스트림의 스왑체인 begin/end 호출과 섞여도 순서가 보장됩니다.
     */
    class D3D11RHICommandList : public RHICommandListForwarder<D3D11RHICommandContext>
    {
    public:
        /** @brief 자신의 네이티브 Deferred Context 를 만듭니다. */
        ...
        /** @brief 이 리스트가 유효한(생성에 성공한) 네이티브 Deferred Context 를 갖고 있으면 true 를 반환합니다. */
        bool isValid() const { return _pNativeContext != nullptr; }
        /** @brief `IRHIDevice::executeCommandList` 가 실제 제출에 쓰는 네이티브 커맨드 리스트입니다. */
        ID3D11CommandList* getNativeCommandList() const { return _pFinishedList.Get(); }
        /** @brief 이 리스트의 기록 상태입니다. 자원이 사라질 때 디바이스가 캐시를 지우려고 읽습니다. */
