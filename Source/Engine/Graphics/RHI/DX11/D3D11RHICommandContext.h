...
namespace sw
{
    ...
    private:
        ...
        /**
         * @brief 이 컨텍스트가 기록 중인 Deferred Context 면 이 스레드에 그 토큰을 묶습니다 — 패스가 시작되는 자리마다.
         ...
         *          즉시 컨텍스트가 아니라 **이 리스트**로 간다(D3D11 의 리스트 단위 버저닝). 즉시 컨텍스트는 슬롯이 없어 묶이지
         *          않는다 — 그쪽은 `_immediateContextMutex` 로 지키는 공유 자원이라 묶으면 그 잠금을 건너뛰게 된다.
         */
