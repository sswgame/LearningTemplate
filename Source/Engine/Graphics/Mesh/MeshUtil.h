...
namespace sw
{
    ...
    struct SW_API MeshUtil
    {
         ...
         * @note 면은 로컬 `y = 0` 이고 노멀은 +Y 입니다. 한때 `y = +0.5`(큐브 윗면 자리)에 두어야 했는데,
         *       정점에 노멀이 없어 셰이더가 위치로 노멀을 지어냈기 때문입니다(`y = 0` 이면 `|y|` 가 0 이라
         *       ±X/±Z 를 받아 바닥이 옆을 보는 것처럼 칠해졌습니다). 정점이 노멀을 들고 다니는 지금은
         *       그 회피가 필요 없습니다.
         */
         ...
         * @details 비었거나 "Cube" 면 큐브, "Quad"/"Rect" 면 쿼드, "Plane"/"Ground" 면 바닥 평면,
         ...
         *
         *          씬에서 온 `MeshComponent` 는 모두 이쪽을 씁니다. 예전에는 컴포넌트마다
         *          `createPrimitive` 로 **자기 Mesh 객체를 따로** 만들었는데, 배치 키가 메시 포인터라
         *          같은 큐브 8000 개가 배치 8000 개로 갈렸습니다(벤치는 하나를 나눠 써서 배치 2 개였고,
         *          그래서 이 결함이 벤치에는 한 번도 안 보였습니다). GPU 정점 버퍼도 8000 벌이었습니다.
         *
