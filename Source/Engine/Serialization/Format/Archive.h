/**
 * @file Archive.h
 * @brief 바이너리 아카이브 읽기 · 쓰기입니다(리플렉션 직렬화는 선택).
 */
#pragma once
...
namespace sw
{
    ...
    /// @brief 바이너리 아카이브를 읽고 씁니다. 리플렉션 직렬화는 선택입니다.
    ...
    public:
        ...
        /** @brief 문자열을 uint32 길이 접두사와 함께 기록합니다. */
        Archive& operator<<( string_view data );
        /** @brief 바이트 벡터를 uint32 길이 접두사와 함께 기록합니다. */
        ...
        /** @brief uint32 길이 접두사가 붙은 문자열을 읽어 옵니다. */
        Archive& operator>>( string& outData );
        /** @brief uint32 길이 접두사가 붙은 바이트 벡터를 읽어 옵니다. */
