 ...
 * @brief 지연 로드(delay-load)가 "이 DLL 은 이미 어디에 올라와 있나" 를 묻는 **유일한** 창구입니다.
 *
 * @details 모듈 DLL 마다 컴파일되는 지연 로드 훅(`DelayLoadNotifyHook.cpp`)은 핫 리로드가 갈아 끼운 섀도 복사본을
 *          가리켜야 합니다. 그 답을 아는 것은 App 의 `LiveReloadManager` 지만, 훅은 **모듈 DLL 안**에 있으므로 App.exe 의
...
namespace sw
{
     ...
     * @note 구현은 App 이 합니다. Engine 은 이 창구만 들고 있고, 수명은 등록한 쪽이 관리합니다.
     */
    ...
    public:
        ...
        /** @brief 창구는 값으로 복사 · 이동하지 않습니다. 등록한 쪽이 수명을 관리합니다. */
        ...
        /**
         * @brief 리로드 그래프가 깨져 있으면 true 입니다. 이때는 핸들을 믿으면 안 됩니다.
         * @details 리로드가 중간에 실패해 어떤 모듈이 어느 버전인지 알 수 없는 상태입니다. 그럴 때 옛 핸들을 반환하면
         *          지연 로드가 사라진 코드로 뛰어듭니다. 차라리 디스크에서 다시 읽게 둡니다.
         */
        virtual bool isModuleGraphBroken() const = 0;

        /** @brief 이름(확장자 없는 스템)으로 이미 로드된 모듈 핸들을 찾습니다. 없으면 nullptr 입니다. */
        virtual void* findLoadedModuleHandle( string_view moduleName ) const = 0;
    };

    namespace engine
    {
        /** @brief 제공자를 등록합니다. nullptr 로 해제합니다. 등록한 쪽이 사라지기 전에 반드시 해제하십시오. */
        SW_API void setModuleHandleProvider( IModuleHandleProvider* pProvider );

        /** @brief 등록된 제공자입니다. 없으면 nullptr 입니다(Shipping 은 항상 nullptr). */
