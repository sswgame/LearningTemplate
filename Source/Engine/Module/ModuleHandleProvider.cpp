...
namespace sw
{
    ...
    {
        /**
         * @brief 등록된 제공자입니다. 지연 로드 훅은 **로더 락 안에서** 불리므로 잠금을 쓰지 않습니다.
         * @details 훅은 DLL 로드 중(로더 락 보유)에 실행됩니다. 여기서 뮤텍스를 잡으면 다른 스레드가 로더 락을 기다리는
         *          동안 교착이 납니다. 포인터 하나를 원자적으로 읽고 쓰는 것으로 충분합니다. 등록 · 해제는 App 이 프레임
         *          바깥에서 한 번씩만 합니다.
         */
