...
- **D — `TickRegistry`** (`GameObject/TickRegistry.h`, 언리얼 `FTickTaskManager` 의 자리). 오브젝트가 자기 틱 항목(주 틱 ·
  서브틱, (그룹, 순서 키) 순)을 들고, 등록부는 그룹마다 "틱할 것이 있는 오브젝트" 목록을 든다. 컴포넌트가 틱을 켜고 끄면
  소유 오브젝트만 표시되어 다음 틱 전에 자기 항목을 다시 짓는다 — 씬 전체를 훑어 웨이브를 다시 만들던 0.5~1 ms 가 사라진
  자리다. 디스패치는 그룹마다 오브젝트 목록을 **한 번의** 포크-조인으로 나누고, 한 오브젝트의 항목은 한 워커가 순서대로
  돈다(같은 오브젝트의 컴포넌트 둘이 동시에 돌지 않는다). 서브틱에 선행 종속성이 하나라도 있으면 예전 DAG 웨이브 경로다.
- **F — 틱 중의 세터는 슬롯 큐에 쌓인다.** `onTick` 안의 `setLocalPosition` 은 `SceneTransformWrite` 한 건을 자기 스레드
  슬롯의 큐에 넣고(잠금·할당 없음), 틱이 끝나면 매니저가 슬롯 단위로 나눠 배치로 적용한다(`applyTransformBatch` 와 같은
  경로). 예전에는 세터마다 람다를 힙에 만들어 뮤텍스 하나에 줄을 섰고, 틱 뒤에 게임 스레드가 직렬로 되돌렸다 — 큐브 8000
  개가 틱 안에서 움직이는 프레임이 4.2 ms 에서 0.7 ms 가 됐다(Release). 같은 스레드가 같은 컴포넌트에 잇따라 쓴 값은
  마지막이 이기고, 다른 스레드가 같은 컴포넌트를 쓴 경우는 예전과 같이 순서가 없다.
