 ...
 *
 * [왜 별도 클래스인가]
 * 예전에는 매니저가 틱 웨이브를 **씬 전체를 훑어** 만들었습니다. 컴포넌트 8000 개면 한 번에 0.5~1 ms 인데, 틱하는 컴포넌트가
 * 하나라도 생기거나 없어지면(총알처럼 스폰이 잦은 게임은 매 프레임) 그것을 통째로 다시 만들었습니다. 게다가 같은 오브젝트의
 * 항목을 서로 다른 서브웨이브로 갈라 서브웨이브마다 포크-조인 한 번(디스패치 바닥 ~50 us)이었습니다.
 *
 ...
 *
 * 선행 조건(`addSubTickPrerequisite`)이 하나라도 등록되어 있으면 매니저가 DAG 웨이브 경로로 갑니다. 오브젝트 단위로는
 * 계층을 넘는 순서를 표현할 수 없기 때문입니다. 그 웨이브도 **이 등록부가 자기 항목으로 짓습니다**(`buildPrerequisiteWaves`).
...
namespace sw
{
    ...
    /** @brief 선행 조건 경로의 웨이브입니다. 같은 오브젝트의 항목이 한 웨이브에 둘 이상 오지 않습니다(웨이브 안은 병렬). */
    using TickWave = vector<TickItem>;

    /**
     * @class TickRegistry
     * @brief 그룹마다의 "틱할 것이 있는 오브젝트" 목록, 멤버십 더티 표시, 오브젝트 항목 재구축, 선행 조건 웨이브를 맡습니다.
    ...
    public:
         ...
         * @return 하나라도 다시 지었으면 true 입니다(DAG 웨이브 캐시를 버리는 신호).
         */
        ...
        /** @brief 선행 조건이 등록된 서브틱이 하나라도 있으면 true 입니다. 그때 매니저가 DAG 웨이브 경로로 갑니다. */
        bool hasPrerequisites() const { return _prerequisiteCount > 0; }
        /** @brief 항목이 바뀔 때마다 오르는 세대입니다. DAG 웨이브 캐시가 이것으로 무효화됩니다. */
        uint64 getGeneration() const { return _generation; }

        /**
         * @brief 선행 조건을 지키는 웨이브 목록을 등록된 항목으로 짓습니다. 그룹 순서대로 짓고, 웨이브 안은 병렬입니다.
         * @details 그룹마다: 선행 조건 DAG 를 Kahn 으로 레벨별 웨이브로 가르고(같은 레벨은 순서 키 · 등록 순서로 안정 정렬),
         *          레벨 하나를 다시 **오브젝트별 서브웨이브**로 가릅니다. 같은 오브젝트의 항목이 한 웨이브에서 나란히 돌지 않게 합니다.
         *          순환은 남은 것을 순서 키 순으로 마지막 웨이브에 붙여 방어합니다. `hasPrerequisites()` 일 때만 부를 값이 있습니다.
