 ...
 *
 * [왜 별도 클래스인가]
 * 렌더러가 매 프레임 씬을 뒤져 그릴 것을 **찾는** 대신, 그릴 것이 붙을 때 자기를 **등록합니다**.
 * 그러면 프레임 루프에서 타입 검사(`castTo`)가 사라지고, 비용이 "전체 컴포넌트 수"가 아니라
 * "그릴 것의 수"에 비례합니다.
 *
 * 이것을 GameObjectManager 안에 두지 않은 이유는 PhysicsWorld 와 같습니다. 매니저는 이미 오브젝트
 * 저장소 + 컴포넌트 풀 + 팩토리 + 틱 등록부를 들고 있어서, 여기에 등록부까지 넣으면 컴포넌트가
 * 등록 하나 하려고 그 모두에 손이 닿습니다. 능력을 별도 타입으로 떼어 두면 컴포넌트는 자기가 쓰는
 * 것만 들고 있으면 됩니다. 언리얼이 `UWorld` 안에 `FScene` · `FPhysScene` 을 따로 두는 것과 같은
 * 구성입니다.
 */
#pragma once
...
namespace sw
{
    ...
    /**
     * @brief 인스턴스 배치의 항목 하나입니다. 등록부의 프리미티브 번호 공간에서 메시 컴포넌트 뒤에 이어 붙습니다.
     * @details 번호 = 메시 컴포넌트 수 + 배치의 첫 항목 자리 + 항목 인덱스. 메시 컴포넌트가 늘거나 줄면 이 번호가 밀리는데,
     *          그 변경은 집합 세대를 올려 빌더가 전체 수집으로 가므로 더티 번호가 어긋나도 답이 틀리지 않습니다.
     */
     ...
     * @note 락 순서에 주의하십시오. 이 클래스의 락은 항상 **가장 안쪽**입니다. 매니저나 계층이 자기 락을 쥔 채
     *       등록/해제를 부를 수 있으므로, 반대로 이 락을 쥔 채 매니저 락을 잡으면 교착이 됩니다. 그래서 등록/해제는
     *       여기서만 끝냅니다(더티 표시는 락을 쓰지 않습니다).
     */
    ...
    public:
         ...
         * @details 아직 등록된 인스턴스 배치가 있으면 그 배치의 등록부 포인터를 비웁니다. 배치가 나중에 사라져도 여기로 오지 않게 합니다.
         */
        ~PrimitiveRegistry();
        ...
        /** @brief 프리미티브를 등록합니다. 붙을 때 한 번 부릅니다. 이미 등록됐으면 무시합니다. */
        void add( MeshComponent* pComp );
        /** @brief 프리미티브를 등록 해제합니다. 멱등입니다. */
        void remove( MeshComponent* pComp );

        /**
         * @brief 이 프리미티브의 렌더 상태가 바뀌었다고 표시합니다. **락이 없습니다.** 워커 스레드에서 불러도 됩니다.
         * @details 트랜스폼 플러시가 루트 서브트리마다 병렬로 돌면서 여기를 부릅니다. 예전에는 뮤텍스를 쥐고
         *          목록에 push 했는데, 큐브 8000 개가 모두 움직이는 프레임에는 워커들이 그 락을 8000 번
         *          다투게 됩니다. 지금은 칸마다 원자 비트 하나입니다(워드 하나가 64 칸, `_arrDirtyWord`). 0→1 로 바꾼 쪽만 "하나라도" 를 세웁니다. 컴포넌트
         *          쪽에는 더티 표시가 없습니다(비트필드라 워커의 쓰기가 이웃 비트와 같은 바이트를 고쳤습니다). 이 플래그가
         *          유일한 기준이고, 이미 선 칸은 읽기만 하므로 같은 프리미티브를 여러 번 찍어도 쓰기는 한 번입니다.
         *          `_listPrimitive` 는 락 없이 읽습니다. add/remove 는 게임 스레드가 병렬 구간 밖에서만 부릅니다
         *          (컴포넌트 추가 · 제거는 틱 중에 미뤄집니다). 그 전제가 깨지면 컨테이너 레이스 탐지기가 잡습니다.
         */
        ...
        /** @brief 등록된 메시 컴포넌트 목록입니다. 렌더 스냅샷 수집의 입력입니다(인스턴스 배치 항목은 `getInstanceEntries`). */
        ...
        /** @brief 렌더 상태가 바뀐 프리미티브가 하나라도 있으면 true 입니다. */
        bool hasDirty() const;
         ...
         * @details 배치는 소유하지 않습니다. 배치가 먼저 사라지면 자기 소멸자에서 빠지고, 등록부가 먼저 사라지면 배치의 포인터를 비웁니다.
         */
        void addInstanceBatch( MeshInstanceBatch* pBatch );
        /** @brief 인스턴스 배치를 뺍니다. 항목 구간을 지우고 뒤 배치의 자리를 당깁니다. 집합 세대가 오릅니다. */
        void removeInstanceBatch( MeshInstanceBatch* pBatch );
        /** @brief 배치의 항목 하나를 더티로 표시합니다. 그 항목의 프리미티브 번호에 깃발을 세웁니다. */
        void markInstanceDirty( MeshInstanceBatch* pBatch, uint32 index );
        /** @brief 인스턴스 배치의 항목들입니다. 메시 컴포넌트 목록 뒤에 이어지는 프리미티브들입니다. */
        const vector<PrimitiveInstanceEntry>& getInstanceEntries() const { return _listInstanceEntry; }
        /** @brief 프리미티브 번호 공간의 크기입니다(메시 컴포넌트 수 + 인스턴스 항목 수). */
    ...
    private:
        ...
        /**
         * @brief 칸마다 "바뀌었다" 비트입니다. **워드 하나가 칸 64 개**입니다. 원자라 워커 여럿이 동시에 찍어도 됩니다.
         * @details 예전에는 칸마다 원자 바이트였고 `consumeDirty` 가 선 칸마다 exchange 를 했습니다. 큐브 8000 개가 모두 움직이는
         *          프레임에 잠긴 명령 8000 번이 게임 스레드에서 줄을 섰고, 그 줄들은 방금 워커가 쓴 캐시 라인이었습니다(2026-09-23
        ...
        /**
         * @brief "서 있는 플래그가 하나라도 있다" 는 표시입니다. `hasDirty` 가 배열을 훑지 않고 답하는 근거입니다.
         * @details 개수가 아니라 플래그인 이유: 개수는 워커 열넷이 같은 캐시 라인을 8000 번 fetch_add 하는 것이라
         *          병렬 플러시가 직렬(200 us)보다 느려졌습니다(262 us). 플래그는 이미 서 있으면 읽기만 하므로(쓰기는
         *          프레임에 한 번) 라인이 공유 상태로 머뭅니다.
         */
        atomic<uint8> _bAnyDirty{ 0 };
        /// @brief add/remove(구조 변경)만 보호합니다. 더티 표시는 이 락을 쓰지 않습니다.
