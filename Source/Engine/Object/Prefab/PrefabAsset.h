/**
 * @file PrefabAsset.h
 * @brief 프리팹 XML 로드/저장 (GameObject 템플릿)
 */
#pragma once
...
namespace sw
{
	...
	public:
		...
		/** @brief Shipping cook: PFB2 (magic + version + name + xml body). */
		...
		/** @brief 직렬화된 XML 본문을 반환합니다. */
		const string& getXmlBody() const { return _xmlBody; }
	...
	public:
		...
		/** @brief Dev: XML 저작(.xml, 도구 JSON은 .json). Shipping: 쿠킹된 .prefab.bin만. 캐시 키는 확장자 없는 정규화 경로. */
		...
		/** @brief XML 저작본을 PFB2 binary로 쿠킹합니다. JSON은 도구 interchange만 허용합니다. */
