 ...
 * @brief 스포트라이트 하나 — 위치·방향·원뿔각·반경을 선언으로 다룹니다.
 */
#pragma once
...
namespace sw
{
     ...
     * @brief 한 점에서 **원뿔 안으로만** 퍼지는 빛. 위치와 방향이 모두 월드 트랜스폼에서 나옵니다.
     * @details 점광에 원뿔을 곱한 것이다 — 거리 감쇠는 `PointLightComponent` 와 **같은 식**을 쓰고
     *          (`lighting.hlsli` 의 한 분기), 거기에 안쪽/바깥쪽 각 사이의 부드러운 감쇠를 곱한다.
     *          언리얼 `USpotLightComponent` 의 `InnerConeAngle` / `OuterConeAngle` 과 같은 규약이다.
     * @note 방향은 이 컴포넌트의 전방 벡터다 — 회전을 주면 빛이 돈다. 회전이 없으면 **아래를 비춘다**
     *       (전방 +Z 그대로 두면 카메라 쪽으로 쏴 화면이 통째로 하얘져 "스폿인지" 를 알 수 없다).
     * @note 점광과 같은 이유로 **그림자를 드리우지 않는다** — 그림자 맵이 방향광 하나에 묶여 있다.
     */
    ...
    public:
        ...
        /** @brief 안쪽 원뿔 반각(라디안). 이 안은 감쇠가 없습니다. */
        ...
        /** @brief 바깥 원뿔 반각(라디안). 이 밖은 완전히 어둡습니다. */
