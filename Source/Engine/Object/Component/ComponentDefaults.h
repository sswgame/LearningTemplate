...
namespace sw
{
    ...
    public:
         ...
         * @details 이 수는 `setPath`/`reload` 당 **1 이어야 합니다.** 예전에는 로드가 실패하면
         *          "읽었다" 깃발이 false 로 남아 **컴포넌트를 만들 때마다 다시 열었습니다.** 기본값
         *          파일이 없는 게 정상인 게임에서 씬 로드가 `컴포넌트 수 x 파일 열기 실패` 가
         *          됐고, 엔티티 4000 개 씬의 로드 255 ms 중 207 ms 가 그것이었습니다.
         */
    ...
    private:
        /**
         * @brief 프로퍼티 하나에 넣을 기본값입니다. 타입당 한 번 풀어 둡니다(언리얼의 CDO 자리).
         * @details 예전에는 **인스턴스마다** XML 노드에서 속성을 문자열로 찾고 텍스트를 파싱했습니다(값 넷에 약 800 ns.
         *          스폰 한 사이클 300 ns 의 세 배). memcpy 로 넣을 수 있는 타입(스칼라 · 벡터 같은 POD)은 텍스트를 한 번
         *          파싱해 바이트로 들고 인스턴스마다 memcpy 합니다. 문자열 · 컨테이너 · 비트필드 · enum 은 원문을 들고
         *          인스턴스마다 파싱합니다(예전과 같습니다).
         */
        struct DefaultPatch
        {
        ...
        /**
         * @brief 한 타입에 적용할 `<Defaults>` 단계들입니다. **뿌리 → 파생** 순서입니다.
         * @details 이 목록은 `TypeInfo` 와 기본값 문서만으로 정해지고 문서는 로드 중 바뀌지
         *          않습니다. 그런데 예전에는 **컴포넌트를 만들 때마다** 다시 구했습니다. 상속 체인을
         *          모으며 레지스트리를 N 번 조회하고, 단계마다 조회 이름 문자열 둘을 만들고,
         *          그 이름으로 XML 트리를 훑었습니다. 씬 하나에 컴포넌트가 4000 개면 그 모두가
         *          4000 번입니다(실측 198 ms).
         */
        ...
        /**
         * @brief 기본값 문서가 올라와 있는지 나타냅니다. **원자적이어야 합니다.**
         * @details `apply` 는 락 없이 이 값을 보고 `_defaultsDoc` 을 읽습니다. 평범한 `bool` 이면 그 읽기가
         *          데이터 레이스이고, 더 나쁜 것은 **순서가 보장되지 않는다는 점**입니다. 한 스레드가 `true` 를 본
         *          시점에 `_defaultsDoc` 은 아직 다 만들어지지 않았을 수 있습니다. release 로 쓰고 acquire 로 읽어
         *          문서가 먼저 보이게 묶습니다. 이중 검사 잠금이 보는 깃발은 `_bLoadAttempted` 입니다.
         */
        atomic<bool> _bDefaultsLoaded;
        /**
         * @brief 문서를 **읽어 보려고 시도했는지** 나타냅니다. 성공 여부와 별개입니다.
         * @details 예전에는 성공 깃발 하나뿐이어서, 파일이 없거나 깨져 있으면 실패한 채로
         *          깃발이 false 로 남고 **다음 컴포넌트가 또 열었습니다.** 기본값 파일은 있어도
         *          되고 없어도 되는 것이라 없는 게 정상인데, 없을 때 씬 로드가
         *          `컴포넌트 수 x 파일 열기 실패` 가 됐습니다. 엔티티 4000 개짜리 씬에서
         *          로드 255 ms 중 **207 ms** 가 이것이었습니다(2026-09-20 실측).
         *
         *          `reload()` · `setPath()` 는 이 깃발도 내려서 다시 시도하게 합니다.
         *          개발 중 파일을 만들어 넣는 길은 그대로 열려 있습니다.
         */
        atomic<bool> _bLoadAttempted;
        /** @brief 문서를 실제로 열어 본 횟수입니다. `getLoadAttemptCount` 가 이것을 반환합니다. */
