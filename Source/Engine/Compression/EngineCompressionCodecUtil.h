 ...
 * @brief Engine 이 외부 라이브러리로 구현한 압축 코덱들의 **등록 목록 한 자리**.
 */
#pragma once
...
namespace sw
{
     ...
     * @details Core 는 압축 라이브러리에 종속되지 않으므로(`ReflectionParser` 가 Core 를 링크한다)
     *          LZ4 · Zstd · Zlib 은 Engine 이 들고 와 붙인다. **그 목록을 호출부에 두지 않는다** —
     *          `EngineLoop::initialize` 가 손으로 둘만 부르고 있었고, 그래서 `Zlib` 은 클래스도
     *          열거값도 있는데 아무도 등록하지 않아 `CompressionStream` 에서 쓸 수 없었다.
     *          (요청하면 무압축으로 떨어졌고, 다른 도구가 쓴 Zlib 스트림은 읽히지 않았다.)
     */
