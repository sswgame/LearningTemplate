...
namespace sw
{
    ...
    public:
        ...
        /**
         * @brief 카메라를 흔듭니다.
         * @param intensity 최대 흔들림 크기. 남은 시간에 비례해 **0 으로 잦아든다.**
         * @param duration 흔들리는 시간(초).
         * @param frequency 초당 진동 수. 기본값은 `kDefaultShakeFrequency`.
         * @details 예전에는 이 함수가 `_shakeFrequency` 를 **건드리지 않았다.** 그 값은 기본이
         *          0 이고 코드에서 넣을 창구가 없었으므로(리플렉션 프로퍼티뿐), 코드로 부른
         *          흔들림은 `sin( t * 0 ) = 0` · `cos( t * 0 ) = 1` 이 되어 **떨리지 않고
         *          한쪽으로 밀린 채 있다가 시간이 다 되면 툭 돌아왔다** — 흔들림이 아니라
         *          정적인 어긋남이었다. 씬 파일에 `shakeFrequency` 를 손으로 적은 경우에만
         *          제대로 흔들렸다.
         *
         *          그리고 위상을 **남은 시간**으로 계산해서, 잦아들 무렵 `cos` 항이 최대가 됐다
         *          — 끝나기 직전이 가장 크게 튀고 그 다음 프레임에 0 으로 끊겼다. 위상은 흐른
         *          시간으로, 크기는 남은 비율로 간다.
         */
