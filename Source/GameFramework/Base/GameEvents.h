 ...
 *
 * @warning **프레임워크는 이 이벤트들을 하나도 발행하지 않는다.** 2026-09-18 확인 기준
 *          열두 종 전부, 그리고 `gameEventChannel()` 까지 저장소 안에 **발행자도 구독자도
 *          없다.** 이름만 보고 "세이브를 요청하면 프레임워크가 `SaveRequestedEvent` 를
 *          쏴 주겠지" 라고 기대하면 **영원히 오지 않는 것을 기다리게 된다.**
 *
 *          여기 있는 것은 게임들이 **자기들끼리** 주고받을 때 이름이 갈리지 않도록 모아 둔
 *          어휘일 뿐이다 — 쏘는 것도 받는 것도 게임 코드가 한다. 프레임워크가 실제로
 *          발행하게 만들 생각이라면 그 자리를 정하는 것이 먼저다(백로그 1-0c 참고).
 */
#pragma once
