...
어떤 게임을 활성화하여 빌드할지는 CMake 설정인 `SW_ACTIVE_GAME` 변수를 통해 결정합니다. 기본 예제는 `Demo`이고, 키트 없는 최소 템플릿은 `Empty`입니다 (`-DSW_ACTIVE_GAME=Empty`).
...
1. **템플릿 복사하기**: `Source/Games/Demo/` 폴더를 복사하거나 새로운 게임 폴더 `Source/Games/MyGame/` (원하는 게임 이름)를 생성하세요.
