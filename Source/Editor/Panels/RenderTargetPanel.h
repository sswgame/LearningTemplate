 ...
 * @brief 프레임 렌더 타깃(G버퍼·그림자 맵·포스트 단계)을 찾아보고 화면으로 확인하는 에디터 윈도우
 */
#pragma once
...
namespace sw::editor
{
     ...
     * @brief 파이프라인이 만든 렌더 타깃을 목록·검색하고 하나를 골라 미리 봅니다.
     * @details 이것이 없을 때 "지금 G버퍼에 뭐가 들어 있나" 를 보려면 `-gv_screenshotAttachment` 로
     *          **프로세스를 다시 띄워** PPM 을 한 장 찍는 수밖에 없었다. 디퍼드가 아무것도 안 그리던
     *          버그 셋을 그 방식으로 쫓았고, 한 장 볼 때마다 앱을 새로 켜야 했다.
     * @note 목록은 `RenderTargetRegistry` 의 **스냅샷**이다 — 렌더 스레드가 트랜지언트를 다시 만드는
     *       동안 UI 가 죽은 핸들을 들고 있지 않도록 세대 번호가 바뀌면 다시 가져온다.
     */
    class RenderTargetPanel : public IEditorPanel
    {
    public:
        /** @brief 렌더 타깃 윈도우를 생성합니다. (도구 창으로 기본 비활성 시작) */
        RenderTargetPanel();
        /** @brief ImGui 에 등록한 텍스처를 놓습니다. */
    ...
    private:
        ...
        /** @brief 고른 타깃의 이름. 인덱스가 아니라 이름으로 들고 있는다 — 목록이 다시 만들어져도 선택이 살아남는다. */
        string _selectedName;
        /** @brief 마지막으로 읽은 레지스트리 세대. 값이 같으면 다시 읽지 않는다. */
        uint64 _lastGeneration;
        /** @brief ImGui 에 등록한 미리보기 텍스처 id. 소유한다 — 선택이 바뀌면 놓고 다시 등록한다. */
        void* _pPreviewTextureId;
        /** @brief 그 id 가 가리키는 RHI 텍스처 핸들. 같으면 다시 등록하지 않는다. */
        uint64 _previewTexture;
        /** @brief 미리보기 확대 배율. 0 이면 창에 맞춘다. */
