 ...
 *
 * @details 예전에는 `ResourceManager` 가 이 감시를 직접 붙였습니다. 그래서 에디터가 없는 배포본도 감시 스레드를 띄우고
 *          이벤트를 큐에 쌓았는데, 정작 그 이벤트로 할 일(애셋을 고쳐서 바로 보는 것)은 에디터에만 있습니다. 소유를
 *          에디터로 옮기면 배포본에는 감시가 아예 없고, "무엇을 언제 다시 읽을지" 는 개발 도구 쪽의 결정으로 남습니다.
 */
#pragma once
