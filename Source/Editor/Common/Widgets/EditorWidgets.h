...
namespace sw::editor
{
	...
	// 편의용 네임스페이스 인라인 포워딩
	// ------------------------------------------------------------------------------
	inline bool drawVec3Control( const utf8* pLabel, float3& values, float32 resetValue = 0.0f, float32 columnWidth = 100.0f, float32 speed = 0.1f )
	{
		return EditorWidgets::drawVec3Control( pLabel, values, resetValue, columnWidth, speed );
	}
	inline bool beginComponentCard( const utf8* pName, uint64 id, bool* pBActive, bool* pBRemoveRequested, bool bAccent = false )
	{
		return EditorWidgets::beginComponentCard( pName, id, pBActive, pBRemoveRequested, bAccent );
	}
	...
	inline void drawChip( const utf8* pLabel, const Color4& color ) { EditorWidgets::drawChip( pLabel, color ); }
	inline bool drawPropertyRowBegin( const utf8* pLabel, float32 labelWidth = 120.0f ) { return EditorWidgets::drawPropertyRowBegin( pLabel, labelWidth ); }
	inline void drawPropertyRowEnd() { EditorWidgets::drawPropertyRowEnd(); }
	inline bool drawSearchFilter( const utf8* pId, string& filterText, float32 width = 0.0f ) { return EditorWidgets::drawSearchFilter( pId, filterText, width ); }
	inline bool drawAssetSlot( const utf8* pLabel, string& assetPath, const utf8* pExpectedExt = nullptr, float32 labelWidth = 120.0f )
	{
		return EditorWidgets::drawAssetSlot( pLabel, assetPath, pExpectedExt, labelWidth );
	}
	inline bool drawSplitter( const utf8* pId, bool bVertical, float32 thickness, float32* pSize1, float32* pSize2, float32 minSize1 = 50.0f, float32 minSize2 = 50.0f )
	{
		return EditorWidgets::drawSplitter( pId, bVertical, thickness, pSize1, pSize2, minSize1, minSize2 );
	}
