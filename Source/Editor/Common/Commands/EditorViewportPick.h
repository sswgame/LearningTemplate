 ...
 * @brief 뷰포트 클릭 피킹입니다. 레이와 처음 만나는 오브젝트 · 컴포넌트를 찾습니다(ImGui 없음).
 */
#pragma once
...
namespace sw::editor
{
     ...
     *          `considerScenePick` 네 함수가 있었고, 마지막 하나는 `getPrimarySceneComponent()` **하나만** 봤습니다. 그래서
     *          게임이 만든 컴포넌트는 그것이 주 컴포넌트가 아니면 뷰포트에서 클릭으로 집을 수 없었습니다. 지금은 (1) 종류를
     *          아는 제공자는 표의 한 줄이고, (2) 표가 잡지 못한 오브젝트는 그 오브젝트의 **모든** SceneComponent 를 기본
     *          반지름으로 훑습니다. ImGui 를 쓰지 않으므로 `Test/EditorTest` 가 실제 씬으로 검증합니다.
     */
    class EditorViewportPick
    {
    public:
        /** @brief 기본 반지름입니다. 고유한 경계가 없는 컴포넌트를 집을 수 있게 하는 크기입니다. */
        ...
        /** @brief 레이와 구의 교차를 검사합니다. 맞으면 outHitT 에 원점으로부터의 거리(0 이상)를 씁니다. */
         ...
         * @details 근평면(NDC z 0)과 원평면(z 1)을 역투영해 잇습니다. 원점은 근평면 위의 점이고, 방향은 단위 벡터입니다.
         *          피킹 · 자 · 애셋 드롭이 같은 레이를 씁니다. 예전에는 셋이 이 계산을 각자 갖고 있었습니다.
         * @return 역행렬이 퇴화했으면 false
         */
        static bool makeRay( const float4x4& invViewProj, float32 u, float32 v, EditorPickRay& outRay );

        /**
         * @brief 레이와 축 평면(axisIndex 0 · 1 · 2 = X · Y · Z = 0)의 교점을 구합니다. 나란하면 false 입니다.
         * @details 자는 바닥(Y = 0)을, 애셋 드롭은 2D 에서 Z = 0, 3D 에서 Y = 0 을 씁니다. 평면이 레이 뒤에 있어도 교점을
         *          냅니다(카메라가 바닥 아래일 때 드롭이 원점으로 튀지 않게). 교점의 축 성분은 정확히 0 입니다.
         */
