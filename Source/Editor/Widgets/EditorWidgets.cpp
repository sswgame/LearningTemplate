...
namespace sw::editor
{
	...
	void drawPanelHeader( const utf8* pTitle, const utf8* pSubtitle )
	{
	...
	bool drawSearchFilter( const utf8* pId, string& filterText, float32 width )
	{
		...
		else
		{
			...
			ImGui::SetNextItemWidth( ( avail > 32.0f ) ? ( avail - 28.0f ) : avail );
		}
		...
		ImGui::SameLine();
		if ( ImGui::Button( "X", ImVec2{ 22.0f, 0.0f } ) && filterText.empty() == false )
		{
			filterText.clear();
			bChanged = true;
		}
