...
namespace sw
{
	...
	void InspectorWindow::drawEngineSection( const EditorUIContext& ctx )
	{
		...
		if ( ctx.material )
		{
			...
			{
				const std::string savePath = "Material/" + ctx.material->getName() + ".material";
				if ( ctx.material->saveToFile( savePath ) )
	...
	void InspectorWindow::drawSelectionSection()
	{
		...
		for ( Component* comp : obj->getAllComponents() )
		{
			...
			bool		bRemove	 = false;
			const bool	bAccent	 = ( comp->asSceneComponent() != nullptr );
			const bool	bScrollTo = ( editor::scrollToComponentId() != 0 &&
									  editor::scrollToComponentId() == comp->getComponentId() );

			if ( bScrollTo )
			{
				ImGui::SetNextItemOpen( true );
				editor::scrollToComponentId() = 0;
			}

			if ( editor::beginComponentCard( name, comp->getComponentId(), &bActive, &bRemove, bAccent ) )
			{
				if ( bScrollTo )
		...
		if ( ImGui::Button( "Open Bone Hierarchy" ) )
			editor::boneHierarchyPopupOpen() = true;
	}

	void InspectorWindow::drawGameObjectHeader( GameObject* obj )
	{
	...
	void InspectorWindow::drawComponentSection( Component* comp )
	{
		ImGui::TextDisabled( "ID: %llu", static_cast<unsigned long long>( comp->getComponentId() ) );
	...
	void InspectorWindow::drawSceneComponentExtras( SceneComponent* sceneComp )
	{
	...
	void InspectorWindow::setLastDroppedAsset( const char* path )
	{
		_lastDroppedAsset = ( path != nullptr ) ? path : "";
	}

	void InspectorWindow::acceptAssetDrop( const char* path )
	{
		if ( path == nullptr || path[0] == '\0' )
			return;
	...
	void InspectorWindow::drawTypeProperties( void* instance, const TypeInfo* typeInfo )
	{
		if ( instance == nullptr || typeInfo == nullptr )
			return;
	...
	void InspectorWindow::drawPropertyWidget( void* instance, const PropertyInfo& prop )
	{
		...
		if ( typeNameIs( prop, "sw::float3" ) || typeNameIs( prop, "float3" ) )
		{
			...
			// Prefer RGB axis control when writable.
			if ( ptr == nullptr )
				return;
			if ( bReadOnly )
			{
			...
			editor::drawVec3Control( label, *ptr, 0.0f, 100.0f, 0.1f );
			return;
		}
	...
	void InspectorWindow::drawTypeMethods( void* instance, const TypeInfo* typeInfo )
	{
	...
	void InspectorWindow::renderMaterialUI( Material* material, IRHIDevice* rhiDevice )
	{
