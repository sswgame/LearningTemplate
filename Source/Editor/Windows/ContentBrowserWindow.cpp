...
namespace sw
{
	...
	void ContentBrowserWindow::drawToolbar()
	{
		...
		if ( ImGui::InputTextWithHint( "##cb_search", "Search Content", _arrSearchBuffer, sizeof( _arrSearchBuffer ) ) )
		{
			// 필터는 실시간 적용되므로 폴더 재스캔이 필요 없습니다.
		}
	...
	void ContentBrowserWindow::drawFolderTreeNode( string_view folderPath, string_view label, int32 depth )
	{
	...
	void ContentBrowserWindow::drawTilesView( const vector<AssetEntry>& visible )
	{
	...
	void ContentBrowserWindow::drawListView( const vector<AssetEntry>& visible )
	{
	...
	void ContentBrowserWindow::selectFolder( string_view absolutePath, string_view breadcrumb )
	{
	...
	void ContentBrowserWindow::importFilesFromDialog()
	{
		...
		FileUtil::openFileDialog( params, SW_DELEGATE_METHOD( FileDialogDelegate, &ContentBrowserWindow::onImportDialogResult, this ) );
	}

	void ContentBrowserWindow::onImportDialogResult( const vector<string>& paths )
	{
