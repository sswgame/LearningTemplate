...
namespace sw
{
	ContentBrowserWindow::ContentBrowserWindow() noexcept
		: _bRootsDirty{ 1 }
		, _bFolderDirty{ 1 }
		, _reservedFlags{ 0 }
	{
	...
	void ContentBrowserWindow::refreshRoots()
	{
		_roots.clear();
	...
	void ContentBrowserWindow::selectFolder( const std::string& absolutePath, const std::string& breadcrumb )
	{
	...
	void ContentBrowserWindow::refreshCurrentFolder()
	{
		_entries.clear();
	...
	bool ContentBrowserWindow::passesTypeFilter( const AssetEntry& entry ) const
	{
	...
	bool ContentBrowserWindow::passesSearchFilter( const AssetEntry& entry ) const
	{
	...
	void ContentBrowserWindow::selectAsset( const AssetEntry& entry )
	{
		_selectedAssetAbs = entry.absolutePath;
		if ( entry.bIsDirectory == false )
			editor::setFocusedAssetPath( entry.relativePath.c_str() );
	}

	void ContentBrowserWindow::openAsset( const AssetEntry& entry )
	{
		if ( entry.bIsDirectory )
		{
		...
		if ( lower == ".xml" && entry.relativePath.find( "Map" ) != std::string::npos )
			editor::requestOpenWindow( "Tile Map Tool" );
		else if ( lower == ".xml" && ( entry.relativePath.find( "Tile" ) != std::string::npos ||
									   entry.relativePath.find( "tile" ) != std::string::npos ) )
			editor::requestOpenWindow( "Tile Map Tool" );
		else if ( lower == ".png" || lower == ".jpg" || lower == ".jpeg" || lower == ".tga" || lower == ".dds" )
			editor::requestOpenWindow( "Sprite Clip" );
		else if ( lower == ".material" )
			editor::setInspectMode( editor::InspectMode::Asset );
	}

	void ContentBrowserWindow::importFilesFromDialog()
	{
		if ( _selectedFolderAbs.empty() )
		{
	...
	void ContentBrowserWindow::processPendingImports()
	{
	...
	void ContentBrowserWindow::drawFolderTreeNode( const std::filesystem::path& folderPath, const std::string& label, int depth )
	{
	...
	void ContentBrowserWindow::drawSourcesPanel()
	{
	...
	void ContentBrowserWindow::drawBreadcrumbs()
	{
	...
	void ContentBrowserWindow::drawTilesView( const std::vector<AssetEntry>& visible )
	{
	...
	void ContentBrowserWindow::drawListView( const std::vector<AssetEntry>& visible )
	{
	...
	void ContentBrowserWindow::drawAssetView()
	{
