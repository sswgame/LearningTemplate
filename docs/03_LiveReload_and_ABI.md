...
- **버전 체크**: 모듈 간 통신에는 항상 `_abiVersion`과 `_structSize` 핸드셰이크가 포함되어 구조체 크기 변경으로 인한 크래시를 방지합니다.
...
> 반드시 유지되어야 하는 전역 상태는 App이 관리하는 `EditorUIContext`나 `Engine` 전용 레지스트리를 통해 할당/접근해야 합니다.
