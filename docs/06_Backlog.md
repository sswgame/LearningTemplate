...
- **헤더 기본값 초기화 vs 생성자 초기화 — 규칙이 현실과 다르다.** AGENTS 는 "생성자가 있으면 필드는 헤더가 아니라
  생성자에서, 선언 순서로 초기화한다" 고 적는데, 전수로 세어 보니 **헤더 33개 · 멤버 217개**가 그 반대다
  (`VulkanRHIDevice.h` 39, `FrameRenderer.h` 32, `ActionMap.h` 23, `D3D12RHIDevice.h` 20 …). 린트가 검사하지
  않으므로 규칙이 사실상 죽어 있다. 두 길뿐이고 **고르는 것은 사람의 몫이다**:
  (a) 규칙을 살린다 — 217 멤버를 기계적으로 옮기고 린트에 넣는다. 초기화 순서 실수 위험이 있고 기능 이득은 없다.
  (b) 규칙을 현실에 맞춘다 — 기본 멤버 초기화를 정식으로 인정하고(새 생성자가 생겨도 빠뜨릴 수 없어 더 안전하다),
      "생성자가 있으면 그 안에서" 는 **비자명한 값**에만 요구한다. 그리고 그것을 린트에 넣는다.
  지금은 **아무것도 바꾸지 않았다.** 절반만 옮기면 지금보다 나쁘다.
