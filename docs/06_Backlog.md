...
### 1-4. GPU 메시 모프의 **OpenGL** 경로 — 아직 안 닫혔다 (정점 풀링 전환의 선결 조건)

`_bGpuMeshMorph = SW_FALSE` 로 GL 만 꺼 둔 상태다. GL 은 레스트 포즈로 깨끗이 폴백한다(실측: 모프
켬/끔이 비배경 51,905 vs 51,856 · 평균 동일).

**측정은 전부 진짜 백엔드(`-gl`)로 다시 했다.** 그 전 진단은 `-gv_rhiBackend=` 로 골랐는데 그
스위치가 아무 일도 하지 않았다(3절) — 그래서 아래 "배제" 는 전부 새로 잰 것이다.

**확정된 것**
- 수식이 아니라 **데이터**다. 컴퓨트가 위치·노멀을 그대로 복사해도 GL 만 다르다
  (DX12 58,266 → 58,255 로 같고, GL 58,288 → 36,834 으로 기하가 사라진다).
- 컴퓨트는 **쓰고 있고** 정점 셰이더는 **읽고 있다**. 컴퓨트가 위치를 `* 0.25` 로 쓰게 하면 두
  백엔드 모두 도형이 그만큼 작아진다(GL 51,840 → 1,901).
- 읽는 **원소 번호는 맞다**. 컴퓨트가 `pos.w` 에 자기 `idx` 를 싣고 정점 셰이더가
  `SW_DRAW_MORPH_BASE + SV_VertexID` 와 비교하게 하니 **불일치 0**(양쪽 다). 다만 GL 은 그 비교에
  도달하는 픽셀 자체가 적다(37,173 vs 58,351) — **일부 정점이 무너진 채로 래스터에 오지 않는다.**

**배제된 것 넷** (전부 실측)
1. ~~정점 스테이지 SSBO 한도~~ — 질의값 **정점 96 · 프래그먼트 96 · 컴퓨트 96**. `t11` 은 네 번째다.
   (한도 로그를 GL 디바이스 초기화에 남겨 두었다.)
2. ~~`ARB_gl_spirv` 의 `gl_VertexID` 의미~~ — `SV_VertexID` 를 색으로 찍으니 DX12·Vulkan·GL 동일.
3. ~~텍스처/버퍼 **인덱스 공간 추측**~~ — 실재하는 결함이라 고쳤다(아래). 다만 모프 증상의 원인은 아니었다.
4. ~~컴퓨트 상수버퍼~~ — CB 를 **한 줄도 안 쓰는** 항등 복사로 바꿔도 GL 만 다르다.

**다음에 볼 것** — "일부 정점만 무너진다" 를 좁히는 쪽으로:
- 컴퓨트 디스패치가 풀 전체를 덮는가. 그룹 수와 `g_MorphVertexCount` 를 로그로 찍어 **실제 원소 수**와
  대조할 것. 덮지 못한 구간은 초기화되지 않은 값이라 삼각형이 퇴화한다 — 관측(픽셀이 **줄어든다**)과 맞는다.
- 같은 버퍼가 컴퓨트에서 `u0`(SSBO binding 16), 그리기에서 `t11`(binding 11)로 걸린다. 드로우 시점에
  binding 11 과 binding 16 에 **무엇이 걸려 있는지** `glGetIntegeri_v( GL_SHADER_STORAGE_BUFFER_BINDING, … )`
  로 직접 확인할 것. 컴퓨트가 걸어 둔 `u0` 바인딩을 그리기 전에 푸는지도(DX11 에서 같은 종류의 사고가 있었다).
- 버퍼 되읽기 API 가 없어 CPU 로 내용을 직접 못 본다. `glGetBufferSubData` 한 줄짜리 진단 경로를
  만들면 "컴퓨트 쓰기가 틀렸나 / 정점 읽기가 틀렸나" 를 셰이더 프로브 없이 가를 수 있다.

**왜 이게 우선순위인가.** 정점 데이터를 입력 어셈블러에서 구조버퍼로 옮기는 전환(언리얼의 manual
vertex fetch — 이 엔진이 인스턴스·머티리얼·가시 목록에 이미 쓰는 방식)은 **정확히 이 경로**에 기댄다.
지금 전환하면 OpenGL 은 폴백이 "레스트 포즈" 가 아니라 **아무것도 안 그려짐**이 된다.
