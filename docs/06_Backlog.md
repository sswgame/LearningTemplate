...
- ~~GPU 리소스 생성·삭제를 워커로~~ → **메시는 닫았다**(아래 "업로드를 그리기 앞으로"). 남은 것은 **머티리얼 상수버퍼와
  PSO** 다. 상수버퍼는 생성(디바이스 레벨, 안전)과 갱신(`updateConstantBuffer` — DX11 은 즉시 컨텍스트라 안전하지 않다)이
  한 함수에 붙어 있어 그대로는 못 옮긴다; 갈라야 한다. PSO 는 컴파일이 밀리초 단위라 이득이 가장 크지만 셰이더 캐시·레이아웃
  등록까지 함께 봐야 한다. 둘 다 `GpuUploadQueue` 에 얹을 자리는 이미 있다.
