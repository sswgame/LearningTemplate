...
### 1-4. GPU 메시 모프의 **OpenGL** 경로 — 원인이 좁혀졌다 (정점 풀링 전환의 선결 조건)

`_bGpuMeshMorph = SW_FALSE` 로 GL 만 꺼 둔 상태 그대로다. **다만 이전 진단은 믿지 말 것** —
그때는 `-gv_rhiBackend=` 로 백엔드를 골랐는데 그 스위치가 아무 일도 하지 않고 있었다(3절 참고).
아래는 진짜 GL(`-gl`)로 다시 잰 결과다.

**확정된 것: 수식이 아니라 데이터 경로다.** 컴퓨트가 레스트를 **그대로 복사**하게 해도(위치·노멀
둘 다 항등) GL 만 그림이 달라진다. DX12 는 모프 끔과 사실상 같다.

| | 모프 끔 | 완전 항등 모프 |
|---|---|---|
| DX12 | 비배경 52,051 · 평균 98.2 | 52,028 · 98.1 **(같다)** |
| OpenGL | 51,855 · 98.2 | **44,085** · 92.5 (기하가 사라진다) |

**제거된 의심 둘** (둘 다 진짜 GL 로 측정):
1. ~~정점 스테이지 SSBO 한도~~ — 실제 질의값이 **정점 96 · 프래그먼트 96 · 컴퓨트 96** 이다.
   `t11` 은 네 번째일 뿐이라 한도와 무관하다. (한도 로그는 GL 디바이스 초기화에 남겨 두었다.)
2. ~~`ARB_gl_spirv` 의 `gl_VertexID` 의미~~ — `SV_VertexID` 를 색으로 찍어 비교하니 DX12 · Vulkan ·
   OpenGL 이 **동일**하다(평균 71.7 / 71.3 / 71.4).

**남은 자리 하나.** 정점 스테이지의 SSBO 읽기 자체는 GL 에서 멀쩡하다 — 인스턴스(`t4`)와 가시
목록(`t10`)을 정점 셰이더가 읽고 기하가 맞게 나온다. 모프 버퍼만 다른 점은 **같은 GL 버퍼가
컴퓨트에서는 `u0`(SSBO binding 16), 그리기에서는 `t11`(SSBO binding 11)로 걸린다**는 것이다.
다음에 볼 것:
- 드로우 시점에 SSBO binding 11 에 그 버퍼가 **실제로 걸려 있는지** (`glGetIntegeri_v( GL_SHADER_STORAGE_BUFFER_BINDING, 11, … )`).
  안 걸려 있으면 정점 셰이더가 0 을 읽어 삼각형이 퇴화한다 — 비배경 픽셀이 **줄어드는** 관측과 맞는다.
- 컴퓨트 디스패치가 `u0` 에 걸어 둔 바인딩을 그리기 전에 **풀어 주는지**(DX11 에서 같은 종류의
  사고가 있었다 — 3절 "DX11 이 스모크에서 아무것도 안 그렸다").

**왜 이게 우선순위인가.** 정점 데이터를 입력 어셈블러에서 구조버퍼로 옮기는 전환(= 언리얼의
manual vertex fetch, 이 엔진이 이미 인스턴스·머티리얼·가시 목록에 쓰는 방식)은 **정확히 이 경로**에
기댄다. 지금 전환하면 OpenGL 은 폴백이 "레스트 포즈" 가 아니라 **아무것도 안 그려짐**이 된다.
이걸 먼저 닫아야 그 결정을 할 수 있다.
