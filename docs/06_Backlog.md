...
- **GPU 상주를 CPU 에셋에서 떼어낸다 (`_s_deviceGeneration` 을 없애는 유일한 길).** 지금은 `Mesh` 가 정점 버퍼 핸들과
  디바이스 포인터를 들고 `releaseGpu()` 로 해제까지 한다. 그래서 "디바이스가 아직 살아 있나" 를 물어야 하고 전역 세대가
  필요하다. `Mesh` 를 **데이터만** 들게 하고(정점 배열 + 안정된 키) GPU 버퍼는 상주 표가 소유하면, 디바이스가 죽을 때 표가
  함께 죽으므로 그 질문 자체가 사라진다. 문제는 **표를 누가 소유하느냐**이고 여기서 갈린다:
  - **(a) `IRHIDevice` 가 소유** — "디바이스가 자기 리소스를 소유한다" 는 원칙 그대로이고, 죽으면 표가 함께 죽어 무효화가
    공짜다. 대신 워커 업로드가 있으므로 표에 잠금이 필요하고, 에셋이 먼저 죽었을 때 항목을 거두려면 키로 지울 길이 있어야 한다.
  - **(b) RT 쪽 `GpuScene` 이 소유** — 이미 `_mapMaterialGpu` 를 같은 방식(프레임 지연 GC)으로 들고 있어 패턴이 있다.
    대신 **GT 선업로드(`GpuUploadQueue`)가 RT 표에 손대야 해서** 오늘 세운 GT/RT 분리가 무너진다.
  - 현재 기울기는 (a)다. (b)는 업로드 큐의 이득(RT 최악 프레임 80ms 제거)을 되돌리지 않으려면 결국 GT 가 RT 표를 만지게 된다.
  - 함께 정해야 할 것: 에셋이 프레임 도중 죽을 때 항목 회수(지금은 `~Mesh` 가 직접 해제한다), 같은 메시를 여러 디바이스에
    올리는 경우(테스트가 그렇게 쓴다), `MaterialInstance::_constant` 도 같은 표로 옮길지.
