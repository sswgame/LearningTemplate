...
**6. 부분 업로드(언리얼 GPUScene dirty-primitive) 는 재고 나서 보류했다.** 인스턴스 버퍼는 뭐
하나라도 dirty 면 **전체**를 다시 올린다(`GpuScene::upload`). 벤치에 "움직이는 개수" 스위치를
임시로 넣어 8000 개 중 일부만 움직여 봤다:

| 움직이는 큐브 | RT.GpuScene.instanceBuffer |
|---|---|
| 0 | (건너뜀) |
| 10 | **100 us** |
| 800 | **101 us** |
...
**10 개만 움직여도 800 개와 같은 값이다** - 전제는 사실이었다. 다만 값이 프레임의 ~7% 이고,
고치려면 `updateStructuredBuffer` 에 **오프셋 인자가 없어** RHI 인터페이스를 4 백엔드에 걸쳐
바꾸고 ABI 스탬프를 올려야 한다(`RHIModuleAbi.h` 를 건드리면 엔진과 모든 RHI_*.dll 을 함께
다시 지어야 한다). 게다가 지금 프레임은 대부분 GPU 가 정한다. **이득 대비 위험이 맞지 않아
하지 않는다** - 다시 제안하기 전에 위 표를 볼 것.

(같은 이유로 "영속 렌더 씬 + 변경 이벤트"(언리얼 `FScene` 모델)도 보류다 - 이득의 대부분이
같은 dirty 추적에서 나오는데 규모는 더 크다.)
