...
**그리고 병목은 씬 크기에 따라 뒤집힌다 (2026-09-20 실측).** 위 표는 큐브 2000 개일 때다.

| 큐브 | GT.Packet.submit | RT.BeginFrame | 누가 기다리나 |
|---|---|---|---|
| 2000 | 722us | 516us | **GPU** — GT 도 RT 도 기다린다 |
| 8000 | **98us** | **135us** | **게임 스레드** — 아무도 안 기다린다 |

8000 에서는 `GT.Scene.tick.flushTransforms`(315us) + `GT.GpuScene.build`(490us) 가 임계
경로를 채운다. 그러므로 **"렌더 CPU 경로를 깎아도 소용없다" 는 2000 규모에서만 참이다.**
큰 씬에서는 게임 스레드가 먼저 막힌다 — 어느 쪽을 깎을지는 **재고 나서** 정할 것.

**(B) 첫 커밋 — 2026-09-21 · `GpuSceneBuilder` 전체 수집을 "워커가 비교까지" 로.**

큐브 8000 이 전부 움직이는 프레임(위 표의 조건)에서 게임 스레드가 임계 경로였고, 그 안에서 `GT.GpuScene.build`
가 가장 컸다(p50 786). 그 안을 뜯으면 **같은 8000 칸을 직렬 패스가 따로따로 지나가고 있었다**: 워커가 후보를
채우면 → 직렬로 8000 개 전부 퍼뮤테이션 해시 찍기(머티리얼 게터 둘씩) → 직렬로 8000 개 배치 키 비교
(`hasSameBatchKeysAsBuilt`, 65~81 us) → 직렬로 raw 채우기(61~65 us). 고친 것:

- **워커가 자기 칸에서 지난 후보와의 배치 키·내용 비교까지 끝낸다** (`CollectFlag` 비트). 해시는 (머티리얼,
  인스턴스, 퍼뮤테이션 세대) 의 함수라 셋이 같으면 지난 값을 그대로 받는다 — 직렬 구간은 앞으로 당기기와,
  달라진 칸의 해시 찍기(지연 캐시라 직렬)만 남는다. `hasSameBatchKeysAsBuilt` 와 전체 배열 비교는 지웠다.
- **재사용성 —** 불투명·투명 배치 방출이 같은 40여 줄을 따로 들고 있던 것을 `emitBatch` 하나로, 직렬 부분
  갱신과 청크 병렬 갱신이 같은 판정·복사를 따로 적던 것을 `isPayloadChanged` · `copyPayload` 로 모았다.
  (A) 에서 가장 잦았던 "한 곳에 넣은 고침이 형제에게 안 갔다" 의 자리 둘이다.

Release · DX12 · 큐브 8000 · 300 프레임, 이전/이후를 번갈아 2회씩 (p50 us):

| scope | 이전 | 이후 |
|---|---|---|
| GT.GpuScene.build | 786 / 851 | **720 / 720** |
| └ collect | 245 / 262 | 229 / 245 |
| └ batchKeys | 65 / 81 | **0** |
| └ fill | 61 / 65 | 73 / 73 |
| GT.Frame | 983 / 1048 | 917 / 983 |

검증: Debug 경고 0 · nogpu 7/7 · hostgpu 2/2 · 린트 20/20, Shipping nogpu 7/7 · hostgpu 2/2.

**8000 규모에서 남은 게임 스레드 그림 (p50, 이후 기준).** `GT.Game.update` 655~720 은 **벤치 코드**가 큐브 8000 개에
`resolveComponent` + `setLocalPosition` + `setLocalScale` 을 부르는 값이다 — 엔진 세터가 큐브당 ~90 ns 이고 그
대부분은 오브젝트·컴포넌트가 힙에 흩어져 있어 나는 캐시 미스다(Unity 의 `TransformHierarchy` 처럼 SoA 로 옮기는
것이 상용 엔진의 답이지만 며칠짜리다). `flushTransforms` 229, `refresh.restore` 73(800 KB 되복사 — 발행이 옮기기라
생기는 값, 풀은 더 느렸다고 위에 적혀 있다). RT 는 p50 917 이라 이 규모에서는 여전히 GT 가 먼저 막힌다.

**(B) 둘째 커밋 — 2026-09-21 · 형제 중복 셋을 하나로 (성능 아님, 재사용성).**

`py -3 Scripts/lint/report/RunDuplicateCode.py --filter Source/Engine --no-headers` 가 뽑은 상위 건 중 "본체가 같고
정책만 다른" 셋을 합쳤다. 남은 상위 건(`MaterialPacking` 의 두 switch · DX11/GL `DeviceSubmission` · `TypeInfo`
복사/이동 생성자)은 그 스크립트 머리말이 이미 "합칠 대상이 아니다" 로 적어 둔 모양이다.

- **`BinarySerializer` — 엄격/소프트 역직렬화 루프 두 벌 + 세 갈래(비트필드·컨테이너·값) 세 벌 → 루프 하나
  (`deserializeTagged`, 정책 인자 셋).** 신뢰할 수 없는 스트림의 경계 검사가 이 안에 있다 — 사본이 둘이면 한쪽만
  검사를 잃는다. 동작은 그대로다(전선 타입 불일치 때 소프트의 "제 타입 읽기 → 이관" 순서는 이관이 제 타입 읽기부터
  시도하므로 결과가 같다). `applyPropertyPayload` 에 "끝까지 읽었는가" 인자를 두어 엄격만 그것을 본다.
- **DX12 bindless — 등록 함수 넷이 온라인 CPU/GPU · 오프라인 핸들 여섯 줄을 각자** → `bindlessHandlesAt`.
- **DX11 루트 상수 흉내 — 그래픽스/컴퓨트 진입점이 스무 줄을 각자** → `writeRootConstants( …, bCompute )`.

**(B) 셋째 커밋 — 2026-09-21 · 투명 순서가 바뀐 프레임에 배치 전체를 다시 짓고 있었다.**

`batches` 단계 안에 스코프 없는 ~170 us 가 있었고 최솟값(139)과 p50(327)이 크게 갈렸다 — 정체는 **투명 정렬 순서가
바뀌면 제자리 갱신이 실패해 불투명 8000 개까지 통째로 다시 짓는 것**이었다. 투명 큐브가 위아래로 흔들리는 벤치에서는
거의 매 프레임이 그 경우다(투명 25% = 2000 개의 카메라 거리 순서가 프레임마다 바뀐다). 고친 것:

- **투명 꼬리만 다시 방출한다** (`rebuildTransparentTail`). 투명 인스턴스는 불투명 뒤에 연속으로 앉으므로
  (`_opaqueInstanceCount` 등 세 값이 자르는 자리), 접두부는 제자리 갱신하고 꼬리는 잘라 내고 새 순서로 붙인다.
  방출은 전체 재구축과 **같은** `emitTransparentBatches` 다. 꼬리는 더티 구간 하나로 실린다.
- **꼬리 재방출은 머티리얼 원소를 다시 묻지 않는다.** 원소 인덱스는 (머티리얼, 인스턴스) 쌍에 영속이라 배치 키가
  그대로인 프레임에는 지난 답(`_listCandidateMaterialElement`)을 읽으면 된다 — 깊이순이라 원소가 번갈아 와서
  직전 조회 캐시가 못 맞히던 해시 조회 2000 번이 배열 읽기가 됐다(꼬리 131~147 → 98 us).
- **투명 정렬 키를 한 번만 계산한다.** 비교 함수 안에서 거리를 다시 구하면 투명 2000 개에 무작위 읽기 44000 번이었다.
- **회수 도장은 (배치, 원소) 쌍에 찍는다.** 인스턴스 8000 개를 돌던 것이 배치가 적어 둔 원소 목록만 돈다(24 → 2 us).
- 스코프를 더했다: `sortTransparent` · `retire` · `publish` · `refresh.transparentTail`. 회귀 테스트
  `GpuSceneTest.TransparentOrderChangeKeepsOpaqueSlots` 가 불투명 슬롯 불변 · 먼 것부터 · 꼬리 더티 셋을 본다.

Release · DX12 · 큐브 8000 · 300 프레임, 이전/이후 번갈아 2회 (p50 us):

| scope | 이전 | 이후 |
|---|---|---|
| GT.GpuScene.build | 720 / 720 | **589 / 589** |
| └ batches | 360 / 360 | 245 / 245 |
| GT.Frame | 917 / 917 | **851 / 851** |

**한 번 본 것 — 재현 안 됨.** 이 커밋의 검증 중 Shipping `EngineTest_NoGPU` 가 **한 번** 실패했다(3.73 초 — 정상
3.46 초와 같아 조기 중단은 아니다). 출력은 요약만 잡아 어느 케이스인지 남지 않았고, 이후 같은 바이너리로 12 회 연속
통과했다(GpuSceneTest 만 따로도 17/17). 다음에 또 보이면 **`--output-on-failure` 전체를 파일로 남길 것** — 간헐이면
스레드 관련 케이스(병렬 수집 · 틱 웨이브)를 먼저 의심한다.

(B) 시작 시점의 빌드 p50 786/851 에서 세 커밋을 거쳐 589 다. 8000 규모의 게임 스레드는 `GT.Game.update`(벤치 세터,
655) + `GT.Frame`(851) ≈ 1.5 ms 이고 RT 는 p50 917 — 여전히 GT 가 먼저 막히지만 차이는 줄었다.

**(B) 넷째 커밋 — 2026-09-21 · 인스턴스 배열 되복사(73 us)를 링으로 없앴다.**

발행이 작업 배열을 **옮겨** 주고 다음 프레임의 제자리 갱신이 발행본을 **되복사**하던 것(800 KB · 73 us, 움직이는
프레임마다)을 링으로 바꿨다. 이 문서 위쪽의 "풀은 더 느렸다" 는 발행 **복사**를 잰 것이었다 — 링은 복사가 아니다.

- 게임 스레드는 **아무도 안 읽는 슬롯**(`shared_ptr::use_count() == 1`, 링만 든다)에 짓고 포인터만 발행한다.
  패킷이 든 슬롯은 골라지지 않으므로 렌더 큐 깊이를 알 필요가 없다 — 모자라면 하나 더 만든다(큐 깊이 + 1 에서 멈춘다).
- 이전 값은 마지막 발행본에서 **읽기만** 한다. 전체 갱신은 슬롯 전부를 새로 쓰고, 부분 갱신은 낡은 슬롯을 먼저
  발행본에 맞춘다 — 슬롯이 발행된 뒤 바뀐 구간만(발행 이력 `_listPublishHistory`, 8 건) 복사하고 이력이 끊겼으면
  통째로(예전 되복사와 같은 값 — 나빠지는 경우는 없다).
- 회귀 테스트 `GpuSceneTest.InstanceRingKeepsPublishedImmutableAndCatchesUp`: 들고 있는 발행본이 안 바뀌는 것과,
  되쓰인 슬롯이 중간 프레임의 변경을 따라잡는 것. 따라잡기를 끄면 실제로 실패한다(변이 검사 확인).

Release · DX12 · 큐브 8000 · 300 프레임, 이전/이후 번갈아 2회 (p50 us):

| scope | 이전 | 이후 |
|---|---|---|
| refresh.restore | 73 / 65 | **없음** |
| refresh.loop | 147 / 147 | 163 / 163 (이전 값을 다른 배열에서 읽는다) |
| GT.GpuScene.build.batches | 245 / 245 | **196 / 196** |
| GT.Frame | 917 / 851 | **786 / 786** |

(B) 시작 시점의 GT.Frame p50 983/1048 에서 네 커밋을 거쳐 786 이다.

**늘어난 것도 있다 — 전부 같은 단계 안의 맞바꿈이고, 벽시계는 빨라졌다 (2026-09-21 확인).** 기준 커밋(`4ebd915c`)의
빌더로 만든 빌드와 번갈아 2000 프레임씩 잰 벽시계: **4.97 / 4.74 초 → 4.33 / 4.26 초 (약 12 %)**.

| 늘어난 곳 (p50 us) | 이전 → 이후 | 왜 | 상쇄 |
|---|---|---|---|
| `build.collect` | 245 → 262 | 워커가 지난 후보와 비교까지 한다 | `batchKeys` 65~81 → 0 |
| `build.fill` | 61 → 73~81 | 워커가 쓴 후보를 이 스레드가 읽는다 | 위와 같음 |
| `refresh.loop` | 147 → 163 | 이전 값을 다른 배열(발행본)에서 읽는다 | `refresh.restore` 73 → 0 |
| `RT.GpuScene.instanceBuffer` | 81~90 → 98~114 | **잡음 안이다** — 같은 바이너리도 98 ↔ 114 로 흔들린다. 구간 업로드가 배열 전부일 때 통째로 올리게 해 봤지만 안 움직였다(되돌림) | — |

`RT.BeginFrame` · `GT.Packet.submit` 의 **평균**이 커 보이는 것은 대기다 — GT 가 빨라져 RT/GPU 가 먼저 막히는
프레임이 생겼다. 300 프레임 측정에서는 크게 흔들리고 2000 프레임에서는 기준과 같다(BeginFrame 49 → 44).
**렌더 스레드가 이제 게임 스레드보다 느리다**(p50 917 vs 786) — 다음에 깎을 곳은 RT 다.

**(B) 다섯째 커밋 — 2026-09-21 · 렌더 스레드: DX12 업로드 제출을 프레임에 한 번으로.**

렌더 스레드가 게임 스레드보다 느려진 뒤(p50 917 vs 786) 스코프를 더 잘게 넣어 보니 `RT.GpuScene.upload` 212 의 절반이
**배치 표·간접 인자 업로드(106 us)** 였다 — 배치가 몇 개뿐인데도. 정체는 DX12 백엔드가 **업로드 호출마다** 복사 커맨드
리스트를 닫고 `ExecuteCommandLists` 를 부르던 것이다(인스턴스 · 배치 표 · 뷰마다의 간접 인자 · 머티리얼 그룹 = 프레임에
대여섯 번, 제출 하나가 수십 us). 고친 것:

- **복사 리스트를 열어 둔 채 기록만 하고, 제출은 `flushPendingUploads` 가 프레임에 한 번** — `endFrame` 이 프레임
  리스트 **앞에** 끼워 한 번의 `ExecuteCommandLists` 로 내보낸다(큐 순서가 실행 순서라 예전과 같은 순서). 즉시 제출 모드 ·
  큐 대기(`waitForQueueDrain` · `waitForPreviousFrame`) · 즉시 실행(`executeCommandListImmediate`) · 리사이즈 앞에서도 먼저 내보낸다.
- 복사 리스트는 게임 스레드(텍스처·메시 업로드)와 렌더 스레드(인스턴스·표)가 같이 쓰므로 **열어 두는 동안 잠근다**
  (`_uploadSlotMutex`). 예전에는 호출마다 닫아서 창이 좁았을 뿐 같은 리스트였다.
- 스코프를 더했다: `RT.GpuScene.batchTables` · `visibleBuffers`, DX12 의 `RT.Present.submit` · `present` · `signalAcquire`.

Release · DX12 · 큐브 8000 · 1000 프레임, 이전/이후 번갈아 2회 (p50 us):

| scope | 이전 | 이후 |
|---|---|---|
| RT.GpuScene.upload | 212 / 212 | **65 / 73** |
| └ batchTables | 106 / 106 | 7 / 7 |
| └ instanceBuffer | 106 / 98 | 57 / 65 |
| RT.ExecutePacket | 589 / 589 | 393 / 425 |
| RT.Frame | 983 / 917 | **786 / 786** |

**Present 의 내역 (DX12, p50).** 닫기·제출 90~98 · **DXGI Present 196~212** · 펜스 신호와 다음 이미지 20. Present 자체가
200 us 인 것은 창 모드 DWM 의 몫이라 여기서 깎을 수 없다.

**제출 90 us 의 내역 (2026-09-21 재확인).** 닫기(타임스탬프 Resolve 포함) 6 · **`ExecuteCommandLists` 81~90** · 리스트 수
**프레임당 12** (`RT.Present.submit.listCount`). 드라이버의 리스트당 ~7 us 다. 줄이려면 리스트 수를 줄여야 한다:
- **재서 기각** — 한 웨이브의 패스 리스트들을 한 번의 자르기로 넘기는 `executeCommandLists` 일괄 API. 리스트 수가
  12 그대로였다 — 이 씬의 웨이브는 전부 패스 하나라 사이 조각이 없다. 되돌렸다.

**(B) 여섯째 커밋 — 2026-09-21 · 웨이브 배리어를 패스 리스트 앞머리로, 빈 프레임 스트림 조각은 자르지 않는다 (RDG 의 자리).**

배리어의 **판단**은 이미 렌더 그래프가 하고 있었다(웨이브마다 실제로 바뀌는 전이만 추린다 — 언리얼 RDG 와 같다).
언리얼과 다른 것은 **기록하는 자리**였다: 프레임 스트림(디바이스 소유 리스트)에 적어서 웨이브마다 스트림을 잘라야 했고,
잘린 조각이 큐에 리스트 하나로 나갔다. 고친 것 둘:

- **웨이브 프롤로그를 그 웨이브 첫 패스 리스트의 앞머리에 기록한다.** 렌더 스레드가 첫 리스트를 열고 배리어를 적은 뒤
  워커에게 넘기고(`ParallelPassEntry::_bAlreadyBegun`), 워커는 열기를 건너뛰고 이어 적는다. 판단·기록 모두 렌더 스레드가
  병렬 기록 **전에** 끝내므로 "배리어는 렌더 스레드가 미리 정한다" 는 계약은 그대로다. 같은 웨이브의 다른 리스트는 큐
  순서상 첫 리스트 뒤라 배리어가 앞선다.
- **DX12 는 빈 조각을 자르지 않는다.** 컨텍스트의 모든 기록이 `commandListForRecord()` 한 창구를 지나며
  `D3D12RecordingState::_bRecordedAny` 를 세운다. `executeCommandList` 는 조각이 비어 있으면 패스 리스트만 넣고 조각을 열어 둔다
  — 그 조각은 뒤에 기록될 것만 담으므로 뒤에 제출돼도 순서가 맞다. (이 표시 없이 첫 항목만 옮겼을 때는 리스트가 12
  그대로였다 — 자르기는 배리어가 아니라 `executeCommandList` 자체가 하고 있었다.)
...
|---|---|---|
| 리스트 수 / 프레임 | 12 | **8** |
| RT.Graph.executeParallel | 229 / 212 | **163 / 163** (조각마다 하던 Reset·Close 가 사라졌다) |
| RT.ExecutePacket | 393 / 393 | 327 / 327 |
| RT.Present.submit.execute | 81 / 81 | 65 / 73 |
| RT.Frame | 786 / 720 | **655 / 655** |

정적 장면 스크린샷(큐브 2000 · `-gv_benchAnimate=0`)은 DX12 · Vulkan 모두 이전과 **바이트 단위로 같다**(잡음 바닥 0).
남은 8 개는 그래프 앞 조각 1 · 패스 4 · 그래프 뒤 조각 1 · 업로드 1 · 오프스크린 조각 1 이다.
벽시계는 3.15/2.92 → 2.93/2.98 초로 같다 — 이제 GT · RT 둘 다 786~851 이고 GPU 프레임 페이싱이 맞물린다.

**(B) 일곱째 커밋 — 2026-09-22 · 게임 스레드: 트랜스폼 쓰기를 배치로 (Unity 의 TransformAccessArray 자리).**

세터 경로(`GT.Game.update` 655~720 p50)를 나눠 보니 핸들 해석 + 값 읽기가 212~262, 세터 둘이 ~380 이었다 — 세터
하나 ~24 ns(원자 두 번 · 거리 비교 · 부모 걷기)에 큐브 8000 × 2. 상용 엔진은 이것을 컴포넌트마다 부르지 않고 **모아서
워커에 나눈다**(Unity `IJobParallelForTransform`, 언리얼 ISM `BatchUpdateInstancesTransforms`). 고친 것:

- `GameObjectManager::applyTransformBatch( const SceneTransformWrite*, count )` — 건마다 핸들 해석 · 필드 쓰기 ·
  더티 표시를 워커에 나누고(문턱 `kParallelWriteCount` 1024), 세대는 끝에 한 번 올린다. 결과는 세터와 같다(값이
  같으면 건너뜀). 틱 중이면 건마다 세터로 돌린다(세터가 지연 경로를 탄다).
...
Release · DX12 · 큐브 8000 · 1000 프레임, 이전/이후 번갈아 2회:

| | 이전 | 이후 |
|---|---|---|
| GT.Game.update (p50 us) | 655 / 655 | **360 / 360** |
| 벽시계 (1060 프레임) | 3.22 / 2.99 초 | **2.71 / 2.59 초** |

(B) 시작 시점과 견주면 벽시계 4.97/4.74 → 2.71/2.59 초 — 약 45 % 빨라졌다. 남은 게임 스레드는 `GT.Frame` 786
(플러시 196 · GpuScene 빌드 589)이고, 렌더 스레드는 655~917(GT 가 빨라져 대기가 늘었다).

**(B) 여덟째 커밋 — 2026-09-22 · 플러시는 더티 루트만 (언리얼의 dirty 목록 자리) · 인스턴스 링 분리 · 값 반환 API 정리.**

- **`SceneTransformHierarchy` 가 더티 루트 목록을 든다.** 더러워진 노드는 부모 사슬을 올라가 루트에 닿으면 그 루트를
  한 번만 올린다(루트의 원자 플래그 `_bQueuedDirtyRoot` 가 중복을 막는다). 플러시는 그 목록만 돈다. 예전에는 **루트
  전부**를 돌며 더티인지 물었다 — 루트마다 캐시 미스 하나라 8000 개 중 800 개만 움직여도 8000 개를 만졌다. 배치 쓰기의
  워커는 슬롯별 스크래치에 올리고 배치 끝에 합친다. `hasDirty()` 는 목록만 본다(O(1)).
  `-gv_benchMovePercent=<퍼센트>` 스위치가 "일부만 움직이는 씬" 을 잰다 — 그 이득은 전부 움직이는 벤치에는 없다.
  회귀 테스트 `GameObjectTest.FlushQueuesOnlyDirtyRoots`(하나만 · 같은 루트의 자식 둘 · 배치 · 부모 바꾸기).
- **`GpuInstanceRing` 을 빌더에서 떼어 냈다.** 발행·슬롯 재사용·따라잡기는 "누가 어느 배열을 읽고 있나" 의 문제이지 씬
  수집의 문제가 아니다. 빌더는 창구 셋(`instanceWork` · `syncWriteSlotFromPublished` · `publishInstances`)만 쓴다.
  빌더 없이 규칙을 검사하는 `GpuSceneTest.InstanceRingStandalone` 을 더했다.
- **값으로 컨테이너를 돌려주던 API.** `GameObject::getChildren()` 은 부르는 자리마다 벡터 하나였고, 에디터 계층 패널은
  "자식이 있나" 를 물으려고 노드마다 프레임마다 그 벡터를 만들었다 → `getChildren( out )` + `hasChildren()`.
  `Material/MaterialInstance::collectShaderKeywords()` 는 `getCachedShaderDefines()` 의 복사본 별칭이라 지웠다.
  나머지 값 반환(`getAllGameObjects()` 37 곳 · `getMemberNames` 등)은 로드·도구 경로라 두었다 — 프레임 경로에 들어오면 그때.

Release · DX12 · 큐브 8000 · 1000 프레임, 이전(엔진만 옛것 + 같은 벤치)/이후 번갈아 2회 (p50 us):

| | 100 % 이동 (이전 → 이후) | 10 % 이동 (이전 → 이후) |
|---|---|---|
| flushTransforms | 245 / 180 → 212 / 245 (잡음 안) | 131 / 131 → **40 / 36** |
| GT.Frame | 1048 / 851 → 851 / 917 (잡음 안) | 589 / 524 → **458 / 458** |
| 벽시계 (1060 프레임) | 3.08 / 2.41 → 2.67 / 2.69 초 | 2.09 / 1.91 → **1.89 / 1.80 초** |

**한 번 본 것 — 재현 안 됨 (둘째).** 이 커밋의 첫 검증에서 Debug `EngineTest_HostOnly` 가 **한 번** 세그폴트로 죽었다
(`ForgetThenInitDoesNotDoubleMaterialTextureOrdinals` 의 Vulkan 구간 "Saved pipeline cache" 직후). 같은 바이너리로 단일
케이스 3 회 · 전체 스위트 7 회 통과. 그 사이 한 번의 실패는 **타임아웃**이었고 ASan 빌드(컴파일 16 개)와 동시에 돌린 부하
때문이다(케이스당 0.1 → 2.6 초) — GPU 스위트는 빌드와 겹치지 말 것. 어제의 Shipping `EngineTest_NoGPU` 1 회와 함께,
간헐 실패 둘이 다 Vulkan/DX 디바이스 생성·해제 근처다. 다음에 또 보이면 `--output-on-failure` 전체 출력을 파일로 남기고,
`-gv_` 로 백엔드를 하나씩 빼 보는 것이 먼저다.

**같은 커밋에서 고친 Vulkan 결함 — 검증 중에 나왔다.** `RenderPassGpuTest.FusedPostChainMatchesStaged` 의 Vulkan 구간이
통과하면서도 `[Error]` 17 줄을 찍고 있었다(테스트는 검증 레이어 오류를 세지 않는다). 둘이다: (1) 스왑체인 이미지를
...
**(B) 열한째 커밋 — 2026-09-22 · `TypeInfo` 주소 고정(세대 검사 제거) · 컴포넌트는 자기 풀을 든다.**

열째 커밋이 "여기서 멈춘 이유" 로 적은 둘을 없앴다.

- **`TypeInfo` 의 주소는 고정이다.** 레지스트리가 `unique_ptr<TypeInfo>` 로 들어 표가 커져도 옮기지 않고, 모듈 해제는
  항목을 지우는 대신 `_bAlive` 를 내린다(**묘비**). 같은 FQN 이 다시 등록되면 같은 객체에 덮어써 되살린다. 그래서
  `TypeLookupCache::find` 의 적중은 "포인터 하나 + 살아 있나" 로 끝난다 — 세대 로드 둘이 사라졌고, 세대는 빈 답(미등록)
  을 매번 다시 찾지 않는 용도로만 남았다. `findType` · `forEachType` · 별칭은 죽은 타입을 없는 것으로 답한다.
  - **묘비는 이름만 남긴다.** 처음엔 내용을 두었더니 SmokeTest 의 핫리로드가 죽었다 — 프로퍼티·메서드 목록이 그 모듈의
    코드(델리게이트 · `$ctor` · 소멸 함수)를 가리키는데, 모듈이 내려간 뒤 재등록 대입이 옛 내용을 파괴하며 사라진 코드를
    불렀다. 해제 시점(모듈이 살아 있을 때) `clearContent()` 로 비운다.
  - 회귀 테스트: `TypeInfoAddressIsStableAcrossRegistrations`(200 개 등록 뒤에도 같은 포인터 · 해제 뒤 죽음 · `forEachType`
    에서 빠짐), `TypeLookupCacheFollowsRegistryGeneration` 에 "재등록은 같은 객체를 되살린다" 를 더했다.
  - 뮤테이션: 재등록이 새 객체를 만들게 하면 그 테스트가 실패한다(확인).
- **컴포넌트는 자기가 나온 풀을 든다(`_pPool`).** 파괴가 `getTypeInfo()->_fullyQualifiedName` 으로 풀을 **다시 찾던**
  것이 함정이었다 — 이름이 바뀌었거나(공개 `setComponentName`) 타입이 그새 해제되었으면 풀을 못 찾아 풀 블록을 힙으로
  반납했고, Shipping 에서 힙이 깨졌다(Debug·ASan 은 조용했다). 생성이 한 번 적고 파괴가 읽는다. 회귀 테스트
  `ComponentReturnsToItsPoolAfterRename` 은 풀 자유 목록이 LIFO 인 것을 써서 "다음 할당이 같은 주소" 로 검사한다 —
  **Debug 에서도 잡힌다**. 뮤테이션(파괴가 다시 이름으로 찾기)으로 실패를 확인했다.

Release · 마이크로벤치(4M 회 · 단일 객체 · 캐시 뜨거움, ns) — 열째 커밋 → 이 커밋:

| 항목 | 열째 | 이 커밋 |
|---|---|---|
| `SceneComponent::StaticType()` | 2.2 | 1.8 |
| `findCachedTypeInfo()` | 1.8 | 1.8 |
| `isDerivedFrom` 적중 / 실패 | 1.9 / 2.1 | 2.4 / 2.3 |
| `castTo<Scene>(mesh)` 적중 / `castTo<Scene>(tag)` 실패 | 5.4 / 5.9 | 5.5 / 5.9 |
| `castTo( pSrc, pToType )` 적중 / 실패 | 3.8 / 3.4 | 3.7 / 4.0 |
| `getComponent<Camera>()` 실패, 컴포넌트 2 개 | 12.5 | 13.1 |
| `getComponent<Scene>()` 적중, 첫 컴포넌트 | 6.9 | 6.7 |

**숫자는 움직이지 않았다(잡음 안).** 세대 로드 둘은 L1 적중이라 비용이 아니었다 — 열째 커밋의 "남은 것은 캐시 라인 셋" 은
맞았지만, 그 셋을 둘로 줄여도 시계는 같다. 그래도 남긴 이유는 성능이 아니라 **구조**다: `findType` 이 내준 포인터가 무효가
되는 사건 자체가 사라져 `TypeLookupCache` 가 "포인터 하나 + 살아 있나" 가 되었고, 밀집 배열 이동에 기대던 논리(이동
생성자의 캐시 비우기 · `clearParentType` · "다음 등록에서 무효" 주석들)가 더는 정확성의 근거가 아니다. 캐스트 한 번을 더
깎을 자리는 이제 없다 — 남은 것은 호출과 루프 자체다.

**(B) 열두째 커밋 — 2026-09-22 · 리플렉션의 남은 셋: 못 푸는 부모는 세대와 함께 기억 · 계층 프로퍼티 맵 · 기본값은 타입당 한 번 패치로.**

`castTo` 뒤에 리플렉션이 남은 자리는 프레임이 아니라 **스폰**(기본값 적용)과 **로드**(이름으로 타입·프로퍼티 찾기)다.
Release 로 잰 연산별 비용(2026-09-22): `findProperty` 적중 2.1(선형)/8.6(맵) · `findPropertyInHierarchy` 17/34 ·
`findType(FQN)` 19 · `applyTypeDefaults`(문서 없음) 8 · 컴포넌트 생성+파괴 한 사이클 300 ns(리플렉션 몫 약 10).
기본값 문서를 켜면 그 사이클이 1100 ns 였다 — 값 넷에 800 ns. 셋을 고쳤다.

- **못 푸는 부모는 세대와 함께 기억한다.** `Component` 처럼 REFLECT 가 아닌 기반은 이름만 적혀 있고 등록되지 않는데,
  `getParentType()` 이 그 이름을 **부를 때마다** 잠금 잡고 찾았다(24 ns) — `castTo` 에서 걷어낸 것과 같은 숨은 잠금이
  계층 프로퍼티 조회 · 이름 걷기 · 기본값의 사슬 수집에 남아 있었다. `_parentMissGeneration` 에 못 푼 세대를 적고, 등록·
  해제가 세대를 올릴 때만 다시 찾는다. 회귀 테스트는 `AncestorDisplay…` (3b) — 두 번 물어도 nullptr, 부모가 등록되면 풀린다.
  뮤테이션(기억을 영영 안 지움)으로 실패 확인.
- **계층 프로퍼티 맵.** `getPropertiesWithBase()` 가 병합 목록과 함께 이름·별칭 → 항목 맵을 짓고, `findPropertyInHierarchy`
  는 그 맵 하나로 답한다(작으면 선형, 파생부터). 회귀 테스트 `FindPropertyInHierarchyMatchesMergedList`. 뮤테이션(맵을
  안 채움)으로 실패 확인.
- **기본값은 타입당 한 번 패치로(언리얼의 CDO 자리).** 예전엔 인스턴스마다 XML 노드에서 속성을 문자열로 찾고 텍스트를
  파싱했다. 이제 `resolveFor` 가 프로퍼티마다 `DefaultPatch` 를 만든다 — POD(스칼라 · 벡터)는 한 번 파싱해 바이트로 들고
  인스턴스마다 `Memory::copy`, 문자열·컨테이너·비트필드·enum 은 원문을 들고 예전처럼 파싱한다. 패치가 `PropertyInfo*` 를
  들므로 타입 표 세대를 같이 적어 재등록 뒤엔 다시 푼다. 기존 테스트 `BaseTypeDefaultsApplyBeforeDerived`(기반 → 파생 ·
  파생이 이김)가 그대로 지키고, 뮤테이션(memcpy 생략)으로 실패 확인.

Release · 마이크로벤치(ns) — 이전 → 이후:

| 항목 | 이전 | 이후 |
|---|---|---|
| `applyTypeDefaults` (문서 있음 · 2 단계 · 값 넷) | 790 | **76** |
| 컴포넌트 생성+파괴 한 사이클 (문서 있음) | 1100 | **370~460** (문서 없음 300) |
| `getParentType` (부모 미등록) | 24 | **4.8~6.9** |
| `findPropertyInHierarchy` 기반 적중 / 실패 | 17 / 34 | **10 / 9** |
| `isDerivedFrom(이름)` 실패 | 28 | **9.8** |

셋 다 프레임 시간에는 보이지 않는다(그 경로에 없다). 기본값 문서는 이 저장소에 없어 지금은 0 이지만, 쓰기 시작하면 스폰당
약 700 ns 를 돌려받는다. 남은 리플렉션 비용은 직렬화기(프로퍼티 이름 대조 · 텍스트 파싱)와 리소스 로드 쪽이며, 그것은 이번에
재지 않았다.

**(B) 열셋째 커밋 — 2026-09-22 · GameObject/Component 의 제곱 시간 둘과 틱 웨이브: 이름 카운터 · 루트 인덱스 · 틱 멤버십 기준 더티 · 재구성 경량화.**

Release 로 잰 GameObject/Component 의 비용(객체 8000, 각각 틱하는 씬 컴포넌트 하나)에서 제곱 시간 함정이 둘 나왔다.

- **이름 유일화가 O(N) 이었다.** 같은 이름이 있으면 `_2, _3, …` 을 하나씩 만들어 맵에 물어 빈 번호를 찾았다 — 같은 이름
  N 개면 생성 하나가 N 번 조회(8000 개 2.6 초, 개당 326 µs). 이제 이름마다 다음 번호를 기억한다(`_mapNameNextSuffix`,
  언리얼 `MakeUniqueObjectName` 의 자리). 번호는 오르기만 한다 — 지운 이름을 되쓰지 않는 것이 O(1) 의 조건이다.
  회귀 테스트 `DuplicateNamesUniquifyWithoutRescan`(3000 개 · 지운 뒤 3001). 뮤테이션(매번 2 부터)으로 실패 확인.
- **루트 씬 컴포넌트 목록이 선형이었다.** 등록은 중복 검사로 목록 전체를 훑고 해제는 선형으로 찾아 swap-remove 했다 —
  8000 개를 만들고 지우면 각각 3200만 번 비교(파괴 개당 2 µs). 컴포넌트가 루트 목록·더티 목록의 자기 자리를 든다
  (`_rootIndex` · `_dirtyRootIndex`), 둘 다 O(1). 병렬 스크래치에 있던 더티 루트는 병합 때 자리를 받는다. 회귀 테스트
  `RootListSurvivesIndexedRemoval`(가운데 지우기 · 옮겨진 루트 지우기 · 붙였다 떼기 · 플러시). 뮤테이션(옮겨진 루트의 자리
  미갱신)으로 실패 확인.
- **틱 웨이브는 틱 멤버십이 바뀔 때만 다시 만든다.** 예전엔 아무 구조 변경에나(병합 · 지연 파괴 처리 · 메시 추가·제거)
  다시 만들었다 — 8000 틱 컴포넌트에 재구성 하나가 2~2.5 ms 라, 총알이 매 프레임 생기는 게임은 그것을 매 프레임 냈다.
  `Component::hasTickWork()`(주 틱 또는 서브틱)가 기준이고, `addComponent` · `removeComponent` · `clearComponents` ·
  `destroyComponent` · `destroyObject` 가 그것을 보고 더럽힌다. 틱 그룹·서브틱 변경은 그대로 더럽힌다.
  `getTickWaveBuildCount()` 로 세는 회귀 테스트 `TickWavesRebuildOnlyWhenTickWorkChanges`. 뮤테이션(파괴 시 무조건 더티)
  으로 실패 확인.
- **재구성 자체도 가볍게.** 선행 종속성이 하나도 없으면(보통) 핸들 맵·인접 리스트를 만들지 않고 바로 정렬한다. 객체별
  서브웨이브 분할은 서브웨이브마다 `unordered_set` 을 두던 것을 "객체마다 이미 든 서브웨이브 수" 하나로.

Release · 마이크로벤치 — 이전 → 이후:

| 항목 (객체 8000) | 이전 | 이후 |
|---|---|---|
| 같은 이름으로 `createGameObject`+`addComponent` ×8000 | 2.6 초 (개당 326 µs) | **11 ms** (개당 1.4 µs) |
| `destroyObject` ×8000 + 처리 | 16 ms (개당 2 µs) | **3.3 ms** (개당 0.42 µs) |
| 틱, 틱하지 않는 컴포넌트를 붙였다 뗀 직후 | 2.0~2.5 ms (재구성) | **0.06~0.10 ms** (재구성 없음) |
| 틱, 틱하는 컴포넌트를 붙였다 뗀 직후 (재구성) | 2.0~2.5 ms | **1.0~1.5 ms** |
| 틱, 정상 상태 (웨이브 캐시) | 117~195 µs | 67~147 µs (잡음 안) |
| `addComponent<Mesh>` (한 객체에 2000 개) | 개당 400~600 ns | 개당 140~370 ns |
| `flushSceneTransforms`, 8000 더티 루트 | 노드당 28~35 ns | 노드당 22~25 ns (잡음 안) |

생성 11 ms 중 처음 164 ms 는 중복마다 찍던 경고 로그였다(개당 약 20 µs) — 이름당 첫 한 번만 남기니 사라졌다. 재구성이
아직 1 ms 남는 것은 8000 컴포넌트를 훑어 후보를 다시 모으고 웨이브 벡터를 다시 짓는 비용이다 — 틱 참여 컴포넌트의
등록부를 들고 증분으로 유지하면(언리얼 `FTickTaskManager` 의 자리) 그것도 사라진다. → **열다섯째 커밋에서 닫았다**(`TickRegistry`).

**남긴 것.** 플러시의 노드당 28~35 ns 는 메모리 바운드다. `SceneComponent` 가 288 바이트라 노드마다 캐시 라인 넷쯤을 만진다.
로컬 TRS·월드 행렬·부모·플래그만 모은 트랜스폼 블록을 계층이 따로 들면(Unity `TransformHierarchy`) 둘로 줄고 병렬 플러시도
포인터를 쫓지 않게 되는데, 리플렉션이 멤버를 직접 읽으므로 세터·직렬화·인스펙터 쓰기가 미러를 갱신해야 하는 큰 작업이다 —
다른 날의 일. 틱 정상 상태 15~24 ns/항목과 핸들 해석 6 ns 는 손댈 이유가 없다. 중복 이름마다 경고 한 줄을 남기는 것은
그대로다 — 스폰이 잦은 게임이면 로그 스팸이라 첫 한 번만 남기는 것이 후보.

**(B) 열넷째 커밋 — 2026-09-22 · 메시 인스턴스 배치(언리얼 InstancedStaticMesh 의 자리): 씬 컴포넌트 없이 N 개를 한 배치로.**

트랜스폼 SoA 대신 택한 길이다(열셋째 커밋 "남긴 것" 참고). 게임플레이 객체는 `GameObject` + `SceneComponent` 로 두고, 수천 개가
매 프레임 움직이는 것은 `MeshInstanceBatch` 로 다룬다 — 메시 하나 · 머티리얼 하나 · 항목 N 개(월드 행렬 · 바운드 · 시드).

- **등록부의 프리미티브 번호 공간에 항목마다 하나로 들어간다** — 메시 컴포넌트 뒤에 이어서(`PrimitiveInstanceEntry`).
  그래서 `GpuSceneBuilder` 는 새 경로가 없다: 후보 채우기가 `fillCandidateFromInstanceEntry` 로 갈릴 뿐, 부분 수집 · 배치
  병합(같은 메시·머티리얼의 컴포넌트와 **한 배치**) · 더티 구간 · 투명 정렬 · 머티리얼 원소 · GPU 회전 시드가 전부 그대로다.
  메시 컴포넌트 수가 바뀌면 항목 번호가 밀리는데, 그 변경은 집합 세대를 올려 전체 수집으로 가므로 더티 번호가 어긋나도
  답은 틀리지 않는다(`markInstanceDirty` 주석). 배치는 소유되지 않는다 — 먼저 죽는 쪽이 상대의 포인터를 비운다.
- **벤치 `-gv_benchInstanced=1`.** 큐브 8000 개를 GameObject 없이 메시 종류마다 배치 하나로 만든다. 격자·움직임·시드가
  컴포넌트 경로와 같아 애니메이션을 끈 프레임은 **픽셀이 같아야** 한다(아래). 큐브별 머티리얼 인스턴스·투명 비율은 이 모드에
  없다(배치 하나 = 메시·머티리얼 하나 — 언리얼 ISM 도 그렇다).
- 회귀 테스트 `GpuSceneTest.MeshInstanceBatchRendersWithoutComponents`: 컴포넌트 하나 + 항목 셋이 한 배치(인스턴스 4) ·
  항목 하나만 옮기면 등록부가 더티를 알고 그것만 새 자리 · 숨기면 빠지고 · 놓으면 사라진다.

Release · DX12 · 큐브 8000 · 1000 프레임 — 컴포넌트 경로 vs 인스턴스 배치 (p50 us):

| 항목 (p50 us) | 컴포넌트 경로 | 인스턴스 배치 |
|---|---|---|
| GT.Frame | 720 / 786 | **393 / 393** |
| GT.Scene.tick (= flushTransforms) | 180 / 196 | **1 / 1** |
| GT.Game.update (벤치 자체의 쓰기) | 425 / 458 | 327 / 327 |
| GT.GpuScene.build | 524 / 524 | 360 / 393 |
| GT.GpuScene.build.collect / batches | 262 / 98 | 147~163 / 81 |
| RT.Frame | 786 / 851 | **589 / 589** |
| 픽셀 비교 (애니메이션 끔, 30 프레임) | — | **차이 0** (1280×720) |

투명 비율 0 · 큐브별 머티리얼 0 으로 잰 값이다 — 인스턴스 모드가 그 둘을 지원하지 않으므로 같은 조건으로 맞췄다. 처음엔
벤치 기본값(투명 25%)이 켜진 채 비교해 먼 줄에서 16,182 픽셀이 달랐고, 실행 간 차이는 두 경로 모두 0 이라 그 차이가 결정적
이었다 — 경고 줄 하나가 원인을 말해 줬다(컴포넌트 경로만 유리 큐브를 그렸다). 인스턴스 배열 자체는 두 경로가 원소까지 같다
(임시 진단 테스트로 400 개를 대조).

**남은 것.** 인스턴스 모드의 GT.Game.update 327 us 는 벤치가 8000 개의 sin·행렬을 만들어 `setWorld` 로 넣는 비용이다 — 실제
게임은 GPU 컴퓨트나 잡으로 옮길 자리다(회전은 이미 GPU 다). 배치 하나 = 메시·머티리얼 하나라 큐브별 머티리얼 인스턴스와
투명 정렬은 컴포넌트 경로에 남는다 — 언리얼 ISM 과 같은 제약이다. 항목 수는 만들 때 정해진다(늘리려면 배치를 다시 만든다).

**(B) 열다섯째 커밋 — 2026-09-23 · Component/GameObject 를 상용 엔진 모양으로 넷: 틱 중 세터는 슬롯 큐 · 틱 등록부는 오브젝트 단위 · 파괴 경로의 제곱 · primary 캐시 — 그리고 스크래치 슬롯의 진짜 버그.**

**벤치가 재지 않던 경로가 진짜 게임플레이 경로였다.** 기본 벤치는 게임 스레드가 틱 **밖에서** 배치로 쓰지만, 실제 게임 코드
(`GravityComponent` · `ProjectileComponent`)는 `onTick` **안에서** `setLocalPosition` 을 부른다. 그 경로를 재는 스위치 둘을 벤치에
더했다 — `-gv_benchTickMovers=N`(큐브마다 틱 무버 N 개, 첫 번째가 틱 안에서 위치·스케일을 쓴다) ·
`-gv_benchSpawnChurn=N`(프레임마다 큐브 N 개를 지우고 같은 자리에 다시 만든다 — 총알). 기준선은 처참했다: 큐브 8000 이 틱 안에서
움직이면 **GT.Frame 4.2~4.7 ms**(배치 경로 0.5 ms).

- **틱 중의 세터는 슬롯 큐에 쌓인다** (Unity `TransformAccessArray` 의 자리, 이제 틱 안에서도). 예전엔 세터마다 람다(매니저 포인터 +
  핸들 + float3 = 36 바이트 > 델리게이트 SBO 24)를 **힙에 만들어** 뮤텍스 하나(`_deferredTransformMutex`)에 워커 열넷이 줄을 섰고,
  틱 뒤 게임 스레드가 건마다 핸들을 다시 풀어 세터를 **직렬로** 돌렸다(틱 2.9 ms + 재적용 1.0 ms). 지금은 `SceneTransformWrite` 를
  자기 스레드 슬롯의 큐에 넣고(`SceneTransformHierarchy::queueWriteParallel`, 잠금·할당 없음, 같은 컴포넌트에 잇따른 건은 합친다),
  틱 뒤 `applyQueuedTransformWrites` 가 슬롯 단위로 나눠 배치 적용한다(같은 슬롯의 건은 순서대로 → 마지막이 이긴다; 다른 슬롯이 같은
  컴포넌트를 쓴 경우는 예전과 같이 순서 없음). 회귀 테스트 `GameObjectTest.TickSettersQueueAndApplyAfterTick`(틱 밖 세터와 월드 행렬
  동일 · 두 번 쓰면 마지막 · 위치+스케일 합침 · 다른 오브젝트 쓰기 · 큐와 더티 비움). 뮤테이션(적용 생략)으로 실패 확인.
- **스크래치 슬롯의 진짜 버그 — 기다리는 스레드는 남의 잡을 실행한다.** 첫 측정에서 `mergeQueuedDirtyRoots` 가 세그폴트로 죽었다.
  `waitStage` · `waitAll` · `runParallel` 의 `tryHelpAndExecute` 는 메인만이 아니라 **렌더 스레드**(패스 기록 대기) · **로더 스레드**도
  지나는데, `getParallelScratchSlot()` 이 워커가 아니면 전부 "마지막 칸" 하나를 주어 메인과 렌더가 같은 슬롯 벡터에 동시에 push 했다.
  더티 루트 스크래치와 플러시 DFS 스택도 같은 구멍이었고(창이 좁아 안 보였을 뿐), 틱 전체를 창으로 하는 쓰기 큐가 드러냈다. 지금은
  `TaskManager::getCurrentThreadScratchSlot()` 이 비-워커 스레드에 고유 번호(워커 수 + n, 상한 `kMaxHelperThreadCount` 8, 넘으면 로그)를
  주고 슬롯 수는 `getScratchSlotCount()` 다. 고친 뒤 같은 조건 4회 연속 통과.
- **틱 등록부 `TickRegistry` (언리얼 `FTickTaskManager` 의 자리).** 오브젝트가 자기 틱 항목(주 틱 · 서브틱, (그룹, 순서 키) 순)을 들고
  등록부는 그룹마다 "틱할 것이 있는 오브젝트" 목록을 든다. 컴포넌트가 틱을 켜고 끄면 소유 오브젝트만 표시되어 다음 틱 전에
  자기 항목을 다시 짓는다(컴포넌트 몇 개를 훑는 값) — 씬 전체를 훑어 웨이브를 다시 만들던 것이 사라졌다. 디스패치는 그룹마다
  오브젝트 목록을 **한 번의** `runParallel` 로 나누고 한 오브젝트의 항목은 한 워커가 순서대로 돈다(같은 오브젝트의 컴포넌트 둘이
  동시에 돌지 않는다는 규칙은 그대로 · 예전엔 같은 오브젝트의 항목을 서브웨이브로 갈라 k 개면 그룹마다 k 번의 포크-조인이었다).
  서브틱 선행 종속성이 하나라도 있으면 예전 DAG 웨이브 경로이고 그 캐시는 등록부 세대로 무효화된다. 파괴는 `processDeferredDestruction`
  이 메모리를 놓기 전에 `unregisterObject`. 항목은 생포인터다 — 지워진 컴포넌트는 소유 오브젝트가 표시되어 다음 틱 전에 다시
  지어지고, 삭제 대기는 디스패치가 건너뛴다(예전의 핸들 재해석과 같은 검사, 해석만 없다). 회귀 테스트
  `GameObjectTest.TickRegistryTracksMembershipPerObject`(그룹 목록 · 항목 순서 · 서브틱 토글 · 선행 종속성 플래그 · 파괴 뒤 자리).
  뮤테이션(파괴 때 안 뺌)으로 실패 확인. `getTickWaveBuildCount()` 는 이제 "등록부가 항목을 다시 지은 틱의 수" 다.
- **파괴 경로의 제곱 — `PrimitiveRegistry::remove` 가 지울 때마다 깃발 전부를 내렸다.** 번호 공간(8000)을 훑는 것이라 프레임에 100 개를
  지우면 80만 번 — 지우기 하나가 4 us 였고 이것이 churn 프레임의 `GT.Scene.tick` 미계측 450 us 였다. swap 한 칸의 깃발만 옮긴다.
  같이: `_mapIdToObject` 는 슬롯 표가 못 다루는 id(범위 밖)만 든다(스폰마다 노드 할당 · 파괴마다 해제 하나가 사라졌다),
  `mergePendingAdds` 가 이름·id 맵을 **다시** 넣던 것을 뺐다, 지연 파괴의 `_listPendingAdd` 선형 탐색은 본 목록에 없을 때만,
  `clearComponents` 는 복사 대신 이동, `~GameObject` · `refreshActiveInHierarchy` 는 자식 벡터를 만들지 않는다.
- **primary SceneComponent 캐시.** `getPrimarySceneComponent()` 가 부를 때마다 목록을 캐스트로 훑었다(호출처 35 곳 — 부모·자식·부착·
  에디터 계층 패널이 전부 지난다). 첫 씬 컴포넌트가 붙을 때 적고(플래그 비트, 리플렉션 없음), 빠지거나 죽으면 다음 호출이 다시 찾는다.
  원자 포인터인 이유: 틱 중 워커들이 읽고 죽은 것을 본 워커가 같은 답을 겹쳐 쓴다.

Release · DX12 · 큐브 8000 · 1000 프레임, 이전/이후 (p50 us):

| 조건 | 항목 | 이전 | 이후 |
|---|---|---|---|
| 무버 1 (틱 안 세터 8000) | GT.Frame | 4194 / 4718 | **917 / 917** |
| | GT.Scene.tick | 3932 / 4194 | 524 / 524 |
| | └ components (병렬 틱) | 2883 / 3145 | **212 / 212** |
| | └ deferredTransforms → queuedTransforms | 1048 / 1048 | **163 / 163** |
| | 벽시계 (1060 프레임) | 5.5 ~ 6.2 초 | **1.6 ~ 1.7 초** |
| 무버 3 (오브젝트당 틱 컴포넌트 셋) | GT.Frame | 4718 / 5242 | **983** |
| | └ components | 2883 / 3407 | 262 |
| 무버 1 + churn 100 (프레임마다 100 개 파괴·스폰) | GT.Frame | 1835 / 1703 (첫 커밋 뒤) | **983** |
| | GT.Scene.tick | 1310 / 1310 | 491 |
| | └ waves (전체 재구축) → registry | 458 / 425 | **9** |
| | └ destroy + merge (미계측이었다) | ~450 | 28 + 1 |
| | GT.Game.update (100 파괴 + 100 스폰) | 114 | 98 |
| 무버 0 (배치 쓰기, 대조군) | GT.Frame | 491 | 524 / 524 (잡음 안) |
...
churn 100 의 GT.Frame 983 → **917**, GT.Game.update 98 → 90.

**같은 날 셋째 커밋 — 남은 비용은 캐시 이동이었다: 잎 루트는 쓰는 자리에서 월드를 만든다 · 틱 큐 건은 대상 포인터를 든다.**

무버 1 의 `GT.Scene.tick` 491 은 병렬 틱 212 + 큐 적용 131 + 사후 플러시 114 였다. 큐 적용은 워커 14 개로 131 us 라 건당 CPU 가
약 230 ns — 계산이 아니라 **캐시 이동**이다(기록한 코어 ≠ 적용한 코어 ≠ 플러시한 코어: 슬롯 표 · 오브젝트 · 컴포넌트 목록 · 씬
컴포넌트 두세 라인 · 루트 큐). 워커 지정 실행은 태스크 매니저에 없다(`Any` · `MainThread` 뿐) — 대신 옮겨 다닐 라인 자체를 줄였다.

- **잎 루트(부모도 자식도 없음)는 `applyTransformWrite` 가 월드 행렬까지 그 자리에서 만든다** — 방금 쓴 라인이 뜨거운 채로 같은
  워커가. 자손이 없으니 순서를 기다릴 것이 없고, 더티 목록에도 오르지 않아 사후 플러시가 그 루트를 만지지 않는다. 자식 있는
  루트는 예전처럼 목록에 올라 플러시가 내려간다. 회귀 테스트 `GameObjectTest.LeafRootWriteFlushesInPlace`(잎은 플러시 전에
  월드가 맞고 렌더 더티가 찍힘 · 가지 루트는 목록 1 · 플러시 뒤 자식 월드). 뮤테이션(인라인 합성 끔)으로 실패 확인.
- **틱 큐의 쓰기 건은 `_pTarget` 을 든다.** 같은 `tick()` 안에서 쌓이고 적용되며 그 사이 컴포넌트는 해제되지 않으므로 핸들을
  다시 풀지 않는다(슬롯 표 · 오브젝트 · 목록 탐색의 미스 셋). 삭제 대기는 적용 쪽이 본다. 바깥에서 주는 배치는 핸들만 쓴다
  (`bTrustTarget` 은 틱 큐 적용만 true).
- 오브젝트 단위 디스패치의 항목 검사는 소유자 활성을 다시 묻지 않는다(`isSelfActive`).

Release · DX12 · 큐브 8000 · 1000 프레임, 둘째 커밋 → 이 커밋 (p50 us; 이 세션은 같은 바이너리도 실행마다 15% 쯤 흔들려 순서를
섞어 재고 낮은 쪽을 적었다):

| 조건 | 항목 | 이전 | 이후 |
|---|---|---|---|
| 무버 1 | GT.Frame | 851 / 851 | **720 / 786** |
| | GT.Scene.tick | 491 | **360 / 360** |
| | └ queuedTransforms | 131 | 122 |
| | └ flushTransformsPost | 114 | **0** (스코프가 돌지 않는다) |
| 배치 쓰기 (무버 0) | GT.Frame | 491 | **393 / 393** |
| | GT.Scene.tick (= flushTransforms) | 131 | **2** |
| | GT.Game.update (배치 적용, 합성 포함) | 262 | 212 / 229 |
| 무버 1 + churn 100 | GT.Frame / tick | 917 / 458 | **851 / 327** |

배치 경로에서 합성이 적용 안으로 들어갔는데도 `GT.Game.update` 가 줄었다 — 라인이 뜨거울 때의 합성은 플러시 패스의 콜드 합성보다
싸고, 플러시 122 는 통째로 사라졌다. 인스턴스 배치의 병렬 쓰기 기각(아래)은 그대로다.

**이제 남은 것(측정 근거로).** 무버 1 의 `GT.Scene.tick` 360 = 병렬 틱 245(오브젝트당 ~30 ns, 메모리 바운드) + 큐 적용 122.
큐 적용을 더 줄이려면 기록한 워커가 자기 슬롯을 적용해야 하는데(라인이 그 코어에 있다) 태스크 매니저에 워커 지정이 없다 — 넣는다면
훔치기 불가 태스크가 생겨 대기가 늘 수 있어 재 봐야 한다(후보). 그 밖의 Object 계층 비용은 스폰 0.6 us(잠금 여섯: 이름 맵 ·
풀 · 루트 등록 · 프리미티브 등록 · 틱 등록부) 인데 총알 수백 개/프레임에서도 0.1 ms 미만이라 두었다.

**(B) 열여섯째 — 2026-09-23 · TaskManager: 워커·대기자는 자기 워드 하나에 잠들고(`Futex`), 병렬 그룹은 노드 대신 티켓 — 잠든 풀의 포크-조인 바닥 67~118 → 34 us.**

"디스패치 바닥(~50 us)은 잠든 워커의 OS 웨이크 지연이고 이 풀 구조로는 더 못 내린다"(3절 09-20)는 **틀렸다** — 바닥의 대부분이
풀 구조였다. 먼저 잰 것: `TaskManagerBenchTest`(새 파일, Release 로 읽는다)와 App 벤치. 이 PC 는 6 코어(SMT 없음) 라 워커 4 다.

Release · 워커 4 · 이전 / 이후 (p50 us, 2회):

| 벤치 | 이전 | 이후 |
|---|---|---|
| 포크-조인, 워커 잠든 뒤 (`runParallel` 4096 × 빈 본문) | 67 / 118 (p90 153~292, max 344~539) | **34 / 36** (p90 60~68, max 110~149) |
| 포크-조인, 연달아 | 8 / 8 | 7 / 7 |
| 렌더 그래프 모양 — 스테이지 + High 넷 + waitStage, 잠든 뒤 (본문 45 us 가 이상값) | 306 / 309 | **79 / 84** |
| 같은 것, 연달아 | 51 / 77 | 49 / 58 |
...
- **`Core/Concurrency/Futex`** — 32비트 워드 하나에 잠들고 그 주소로 깨운다(Windows `WaitOnAddress` · Linux `futex` · 그 외 버킷 뮤텍스
  폴백). `synchronization.lib` 는 **Engine 이 직접** 링크한다 — Engine 은 `Core_objects` 를 흡수하므로 Core 의 PUBLIC 링크가 안 넘어온다.
- **워커는 자기 워드에 잠들고 유휴 비트마스크에 올라간다.** 깨우는 쪽은 `_workEpoch` 를 올리고(스핀 중인 워커 몫) 비트를 **원자로
  내린 워커만** 깨운다 — n 명 깨우기가 서로 독립인 주소 두드리기 n 번이다. 예전엔 `_workerMutex` 아래 `notify_one` × n 이었고 깨어난
  n 명이 그 뮤텍스를 다시 잡느라 줄을 섰다(호송) — 렌더 그래프 모양의 306 us 가 그것이다. 잠들기 전 "비트 올리고 → 큐 보기" 와
  제출의 "큐 넣고 → 세대 올리고 → 비트 보기" 가 seq_cst 데커 짝이라 잠든 워커 뒤에 일감이 남지 않는다.
- **대기자도 자기 워드에 잠든다 — `JoinCounter`.** 남은 수(하위 32)와 대기자 슬롯(상위 32)을 워드 하나에 담아, 마지막으로 끝내는
  쪽의 CAS 하나가 "0 이 됐다 · 누구를 깨울까" 를 같이 답하고 그 뒤로는 워드를 **다시 만지지 않는다** — 그래서 `runParallel` 의 그룹이
  호출 스레드의 **스택**에 살 수 있다. 스테이지도 같은 카운터다(`_mutex`·`_cv` 삭제). 같은 것을 둘째로 기다리는 스레드와 `waitAll` 만
  방송 세대(`_completionEpoch`)에 잠들고, 방송은 대기자가 0 이면 원자 하나 읽고 끝이다 — 완료 경로에 시스템 콜이 없다.
  메인 스레드는 어느 대기에서든 잠들기 전에 `_mainThreadParkedSlot` 에 자기 슬롯을 적고, 메인 일감을 넣는 쪽이 그것을 깨운다
  (회귀 테스트 `TaskManagerTest.MainThreadTaskWakesParkedMainThread`).
- **병렬 그룹은 티켓이다.** `runParallel` · `emplaceParallel*` 은 청크마다 노드를 만들지 않는다. 그룹 하나(`ParallelGroup`: 본문 ·
  범위 · `_nextChunkStart` · 합류)에 **티켓 몇 장(워커 수 이하)** 만 큐에 넣고, 티켓을 집은 스레드는 원자 덧셈으로 청크를 남는 만큼 돈다
  (Unreal ParallelFor · TBB). 청크 하나의 비용이 원자 하나라 참여자당 넷으로 잘게 나눈다(`kChunksPerThread`) — 먼저 끝난 스레드가
  꼬리를 이어 집는다. **호출 스레드는 워커가 깨기를 기다리지 않고 곧바로 첫 청크를 집는다** — 잠든 풀의 바닥이 "고정 지연" 에서
  "호출자가 청크를 더 한다" 로 바뀐 것이 이득의 본체다. 큐 항목은 `uintptr_t` 로, 하위 비트가 켜지면 티켓이다. 청크마다 노드 할당 ·
  상태 CAS · 큐 넣고 빼기 · 활성 수 · 후속 목록 스핀락 · 노드 반납이던 공유 라인 원자 연산 열 개가 하나가 됐다.
- **청크 사이에 High 레인을 본다.** 첫 판에서는 티켓을 쥔 워커가 그룹을 다 돌 때까지 High 를 안 봐서, 무버 벤치의 렌더 스레드
  `RT.ExecutePacket` 이 524 → 589 로 밀렸다(GT 그룹이 RT 패스 기록을 세워 뒀다 — 레인을 만든 이유 그대로의 역전). 예전엔 청크가
  노드라 노드 사이에 저절로 봤다. 지금 청크(참여자당 넷)는 그때의 노드(워커당 둘)보다 작아 기다림의 상한이 더 짧다.
- **부모-자식 합류의 진짜 경쟁 하나를 닫았다.** 본문이 "상태 = 자식 대기" 를 적고 자식 수를 읽는 사이에 마지막 자식이 수를 내리고
  상태를 읽으면 **둘 다** 완료를 처리했다(seq_cst 로도 못 막는 창 — 활성 수·스테이지가 두 번 내려간다). 본문 자신을 1 로 세는
  `_pendingChildren` 하나로 바꿨다. 노드에서 `_pSharedCallable` · 범위 · `WaitingForChildren` 데커가 빠졌다.

App 벤치는 **번갈아** 잰다 (Release · DX12 · 큐브 8000 · 1000 프레임, `BinOld` = 이전 바이너리 사본, 이전/이후 2회). 아침 기준선과
견주면 "전부 15 % 느려졌다" 로 보였는데 **기계가 느려진 것이었다** — 같은 시각의 이전 바이너리도 그만큼 느렸고 CPU 시간이 같았다
(스핀 누수 아님). 같은 조건에서는 전부 같거나 한 버킷 낫다. 벽시계는 GPU·present 페이싱이라 안 움직인다(3.0~3.1 초).

| 조건 | 항목 (p50 · avg us) | 이전 | 이후 |
|---|---|---|---|
| 기본 | GT.Frame | 655·709 / 720·768 | 655·703 / 655·693 |
| | GT.GpuScene.build | 589·686 / 655·744 | 589·676 / 589·667 |
| | RT.ExecutePacket | 458·503 / 491·545 | 458·513 / 458·509 |
| | RT.Graph.executeParallel | 229·246 / 229·259 | 229·250 / 229·247 |
| 무버 1 | GT.Frame | 1179·1410 / 1179·1383 | 1179·1350 / 1179·1351 |
| | GT.Scene.tick.components | 425·487 / 425·479 | 425·472 / 425·477 |
| | RT.ExecutePacket | 524·576 / 524·575 | 491·556 / 524·562 |
| | RT.Graph.executeParallel | 229·257 / 229·259 | 229·239 / 212·237 |

이 PC 에서 프레임 이득이 작은 이유는 워커가 넷뿐이고 프레임이 GPU 에 물려 있어서다 — 마이크로벤치의 2~4 배(잠든 풀의 디스패치)가
남는 부분이고, 16 스레드 PC 에서는 프레임 단위로 다시 잴 것.

**함정 · 바뀐 약속.**

- **A/B 는 반드시 번갈아.** 아침에 잰 기준선과 오후의 결과를 비교하면 기계 상태(터보·백그라운드)가 결과로 읽힌다. `BinOld` 를 만들어
  두고(`git stash -u` → 빌드 → 복사 → `stash pop` → 빌드) 같은 스크립트로 번갈아 돌린다 — 이번엔 프로세스 CPU 시간까지 같이 찍어
  "워커가 돌고 있나" 를 가렸다.
- **`clear()` 는 아무것도 돌고 있지 않을 때만** — 예전과 같다. 돌고 있는 태스크가 있는 채로 부르면 활성 수를 0 으로 놓은 뒤 그 태스크의
  완료가 0xFFFFFFFF 로 감아 `shutdown` 의 `waitAll` 이 영원히 기다린다. 그 조건을 어긴 테스트를 하나 썼다가 CTest 타임아웃으로 알고 지웠다.
- **`submitWithoutWake` 는 세대도 올리지 않는다.** 묶음 끝의 `wakeSleepingWorkers` 가 한 번 올린다 — 그것을 안 부르면 스핀 중인 워커도
  모른다(예전엔 세대만은 매번 올렸다). 부르는 곳(렌더 그래프 · 병렬 그룹 · 벤치)은 전부 부른다.
...
- **`DrawCandidate` 의 shared_ptr 셋(메시·머티리얼·인스턴스)을 날 포인터로.** 수집 비용의 후보로 적어 두었는데(2026-09-22 확인)
  **그대로 둔다.** 이유 둘: (1) 채우기는 이미 날 포인터를 먼저 비교해 같으면 대입하지 않으므로 참조 카운트는 포인터가
  바뀔 때만 움직인다 — 비용이 아니다. (2) 지난 프레임 후보를 날 포인터로 들면 **ABA** 가 생긴다: 메시 A 가 죽고 같은 주소에
  B 가 태어나 컴포넌트가 B 를 가리켜도 "같은 키" 로 보여 배치가 A 의 `shared_ptr` 를 계속 든다(그림이 옛 메시). 소유를 들면
  주소가 재사용될 수 없다. 날 포인터로 가려면 리소스마다 고유 ID 를 붙여 (포인터, ID) 로 비교해야 한다 — 그날 다시.
- **트랜스폼 SoA 저장소 (Unity `TransformHierarchy`).** 플러시는 메모리 바운드다(병렬 14 스레드에 1.8 배뿐). 노드당 캐시
  라인 다섯을 둘로 줄이려면 로컬 TRS 를 컴포넌트 밖 배열로 옮겨야 하는데, `PROPERTY` 가 멤버를 직접 읽으므로 리플렉션이
  접근자 프로퍼티를 알기 전에는 안 된다. 더티 루트 목록이 "일부만 움직이는" 보통의 씬은 이미 덮는다 — 전부 움직이는
  씬만 남는다.

**측정해서 기각한 것 — 다시 제안하지 말 것.**

- **인스턴스 배치의 병렬 쓰기 `MeshInstanceBatch::updateParallel`** (2026-09-22, 열넷째 커밋 뒤). 인스턴스 모드 벤치의
  GT.Game.update 327 us(항목 8000 개의 sin·행렬을 한 스레드가 `setWorld`) 를 잡으로 나누면 106 us 가 된다. 그런데 GT.Frame 이
  393 → 524, RT.Frame 이 589 → 655 로 한 버킷씩 올라 **벽시계는 같았다**(직렬 1775~1865 ms · 병렬 1796~1845 ms, 3회씩, 같은
  빌드에서 문턱만 바꿔 A/B). 워커가 쓴 항목을 수집 잡의 다른 코어가 읽으며 코어 간 캐시 이동이 생기고, 항목당 일(약 40 ns)이
  그 비용보다 작다 — 빌더의 "raw 페이로드를 워커가 쓰지 않는" 결정과 같은 이유다. 되돌렸다. 항목당 수백 ns 이상의 일
  (AI·물리가 인스턴스 위치를 정하는 경우)이 생기면 그때 다시 — 그때는 쓰는 잡과 수집 잡의 구간을 같은 워커에 맞추는 것까지 봐야 한다.

- **꼬리 재방출에서 머티리얼 그룹·퍼뮤테이션 인덱스까지 후보별로 캐시** (2026-09-21). 배치마다 셰이더 경로 문자열
  해시 조회를 없애려 했는데 꼬리 98 → 98/106 으로 **안 움직였다**. 꼬리의 남은 비용은 조회가 아니라 배치 객체 자체
  (배치 수가 많다 — 깊이순이라 머티리얼이 번갈아 갈린다)다. 되돌렸다.
- **`SceneTransformHierarchy::notifyDirtied` 를 원자 RMW 대신 단일 쓰기자 load+store 로** (2026-09-21). 세터마다
  지나는 자리라 큐브 8000 × 세터 둘 = `lock xadd` 16000 번을 아끼려 했는데 **숫자가 안 움직였다**(같은 조건 번갈아
  2회, `GT.Game.update` p50 655/655 vs 655/655). 세터 경로의 비용은 원자 연산이 아니다.
- **`SceneComponent` 의 플러시가 만지는 필드(더티 비트·부모·자식·월드 캐시)를 붙여 두기** (2026-09-21). 노드당
  캐시 라인 다섯을 셋으로 줄이려 했는데 **숫자가 안 움직였다**(`flushTransforms` p50 196/212 vs 196/245 — 잡음 안).
  이 둘로 보건대 8000 규모 게임 스레드의 남은 비용은 **오브젝트 그래프 자체**(힙에 흩어진 오브젝트·컴포넌트를
  핸들로 찾아 들어가는 것)이고, 필드 순서나 원자 연산 같은 미세 조정으로는 안 움직인다. 답은 SoA 트랜스폼 저장소
  (Unity `TransformHierarchy`)인데 며칠짜리다.
- **워커가 자기 칸의 raw(`GpuInstance` 페이로드)까지 쓰기** (2026-09-21). 직렬 채우기 패스 65 us 가 사라졌는데
  그만큼이 수집(+50~115)과 배치(+66~98)에 도로 붙었다 — 같은 조건 번갈아 2회, 빌드 p50 786/786 vs 786/851.
  정렬(`sortTransparent`)과 제자리 갱신이 다른 코어가 쓴 raw 를 원격 캐시에서 끌어온다. **이어서 쓰고 이어서
  읽는 스레드가 같아야** 싸다. 워커에는 비교만 남겼다(위 표).
- **`SceneComponent::markTransformDirty` 의 "이미 더티면 조기 반환"** (2026-09-21). 위치·스케일을 한 프레임에 같이
  바꾸는 흔한 모양에서 두 번째 호출을 아끼려 했는데, `GameObjectTest.TransformDirtyGenerationEarlyExit` 가
  **더티 표시마다 세대가 오른다** 는 계약을 이미 못박고 있다(갓 만든 컴포넌트는 더티로 태어나고, 그 첫 세터가 세대를
  올려야 다음 플러시가 돈다). 계약을 지키면 남는 것은 부모 걷기 몇 번이라 값이 없다. 되돌렸다.
- **`GpuSceneBuilder::collect` 의 월드 행렬 이중 복사 제거.** 지역 변수로 한 번, 후보로 또 한 번
  복사하던 것을 한 번으로 줄였는데 **숫자가 안 움직였다**(Release · 큐브 8000 · 300프레임 · 3회):
  `collect` 평균 130/119/118 → 125/124/124 us, 최솟값 78/75/80 → 77/75/76 us — 완전히 겹친다.
  컴파일러가 이미 없애고 있었다. 되돌렸다.
...
**(B) 스물한째 — 2026-09-23 · SIMD 없이 게임 스레드를 줄였다: 투명 꼬리는 접두부 갱신과 나란히 · 투명 정렬은 지난 순서에서 삽입 정렬 · 갱신 청크 2048 → 512 · 투명 배치 키는 대표 맵 없이 · 회전 없는 TRS 는 대각선만.**

요청은 "최적화할 수 있는 곳은 최적화, **SIMD 등은 쓰지 않는다**". 기준 `14c893f0`, Release, dx12 · 큐브 8000(투명 25 %) · 1000 프레임,
`BinOld` 와 번갈아 세 번씩(std · `-gv_benchTickMovers=1`). 8000 규모에서는 **게임 스레드가 임계 경로**다(GT ≈ 1100 / 1380 us, 렌더 스레드의
CPU 일은 ~550 us 이고 나머지는 GPU 를 기다린다). 임시 스코프로 쪼개 보니 수집 병렬 ~300 · 배치 180(갱신 청크 86 과 투명 꼬리 114 가
**차례로**) · 투명 정렬 30 · 채우기 90 · 쓰기 적용 322(std) / 틱 512 + 큐 적용 243(mover). 원소당(스레드당) 수집 ~225 · 틱 ~380 · 쓰기
~240 ns 는 객체 그래프를 따라가는 메모리 지연이 대부분이라, SIMD 없이 손댈 곳은 **직렬로 기다리는 자리**와 **안 해도 되는 일**이었다.

- **투명 꼬리를 접두부 갱신과 같은 병렬 구간에서** (`refreshInstancesInPlace`). 투명 순서만 바뀐 프레임은 불투명 접두부를 청크로
  제자리 갱신하고(워커) **다 기다린 뒤** 투명 꼬리를 다시 방출했다(직렬) — 두 시간이 더해졌다. 이제 꼬리가 `runParallel` 의 블록 0 이다.
  호출 스레드가 첫 블록을 집으므로 대개 게임 스레드가 꼬리를, 워커가 접두부를 돈다(워커가 먼저 집어도 결과는 같다). 둘은 만지는
  메모리가 겹치지 않는다 — 접두부는 쓰기 슬롯 [0, 불투명 수) 를 포인터로 쓰고 꼬리는 자르고 뒤에 붙인다. 붙일 자리는 구간 전에
  `reserve` 해 워커가 쥔 `data()` 가 옮겨지지 않게 했다. 계약은 헤더 `rebuildTransparentTail` 에 적었다(접두부를 만지는 일을 더하면 레이스다).
- **갱신 청크 2048 → 512.** `runParallel` 의 둘째 인자는 문턱이지 청크 크기가 아니다 — 청크 수는 부르는 쪽이 정한다. 불투명 6000 개에
  청크 셋이라 여섯 스레드 중 셋이 놀았다(갱신 구간 81~86 us 가 청크 하나의 길이였다).
- **투명 정렬은 지난 순서에서 삽입 정렬** (`sortTransparent`). 나누기를 다시 하지 않은 프레임의 `_listScratchTransparentIdx` 는 지난 정렬
  결과라 거의 정렬돼 있는데 std::sort 는 N log N 을 다 치렀다. 옮김이 원소당 8 칸(`kTransparentInsertionMovesPerElement`)을 넘으면(카메라가
  크게 돌았다 · 나누기를 다시 했다) std::sort 가 마저 한다 — 순서가 전순서(거리 → 후보 인덱스)라 어느 쪽이든 결과가 같다.
- **투명 배치 키는 대표 맵 없이** (`emitTransparentBatches`). 원소마다 `batchKeyMaterial` 로 해시 맵을 찾았다(투명 2000 개면 2000 번, 직렬).
  비교는 퍼뮤테이션 해시가 같을 때만 머티리얼까지 오므로 합치기가 켜져 있으면 "둘 다 머티리얼이 있나", 꺼져 있으면 포인터 비교가 같은
  답이다. 맵은 불투명 나누기가 비우고 채우므로 투명 쪽이 넣던 대표는 아무도 읽지 않았다.
- **회전 없는 TRS 는 대각선만** (`float4x4::createTrs` 오일러 오버로드). 세 각이 모두 0 이면 사원수(삼각 함수 여섯 번)와 사원수 → 행렬을
  건너뛴다. 합성 하나 23.6 → 8.5 ns(마이크로벤치), 회전이 있으면 비교 셋만큼(~2 ns) 는다. 잎 루트의 즉시 합성 · 플러시 · `getWorldMatrix`
  가 모두 지나는 자리다. 값은 단위 사원수 경로와 같다(음수 스케일에서 0 의 부호만 +0).

| 평균 us (잡음 큰 두 판 — max 7~15 ms 튄 OLD std 2 · NEW std 1 — 은 뺐다) | std OLD → NEW | mover OLD → NEW |
|---|---|---|
| GT.Frame | 656 · 710 → 642 · 636 | 1399 · 1389 · 1342 → 1277 · 1275 · 1234 |
| GT.Scene.tick (components · queuedTransforms) | — | 733 · 725 · 709 → 678 · 679 · 665 (큐 적용 230 · 230 · 219 → 199 · 199 · 194) |
| GT.GpuScene.build | 633 · 682 → 618 · 611 | 653 · 651 · 621 → 587 · 582 · 557 |
| build.batches | 172 · 189 → 134 · 132 | 188 · 192 · 180 → 127 · 130 · 121 |
| build.refresh.loop | 140 · 156 → 118 · 116 | 156 · 159 · 149 → 111 · 114 · 106 |
| build.sortTransparent | 30 · 31 → 14 · 14 | 30 · 30 · 29 → 13 · 14 · 13 |

`refresh.loop` 이 이제 `refresh.transparentTail`(~110) 과 거의 같다 — 접두부가 꼬리 뒤로 숨었다. 남은 배치 단계의 임계 경로는 꼬리
자체(투명 2000 개를 깊이순으로, 즉 후보 · raw 를 무작위로 읽는다)다. std 의 `GT.Game.update`(쓰기 적용) 은 잡음 폭 안이다.

**렌더 스레드는 CPU 일이 그대로다.** mover 의 `RT.Frame` 이 1047~1055 → 1070~1086 으로 오른 몫은 `RT.Present.present`(285~294 → 310~312) —
DXGI 가 GPU 를 기다리는 자리다. `RT.ExecutePacket`(545~549 → 549~564) 은 잡음 폭이고 `RT.Present.submit` 은 125~134 → 118~125 로 내렸다.
std 에서는 `RT.Frame` 이 프레임당 **1.1 번** 불린다 — 게임 스레드가 빨라져 새 패킷 없이 도는 렌더 프레임이 생겼다(합이 커 보이는 이유).

**재고 기각한 것.**
- 회전 사원수 캐시(오일러 비트를 키로) — 삼각 함수는 합성당 16~24 ns 라 캐시가 벌 몫보다 SceneComponent 가 커지는 것(+28 B, 296 → 324)이
  크다. 회전 없는 경우는 위 지름길이 다 가져간다.
- 축마다 각이 0 이면 sin/cos 건너뛰기 — UCRT 의 `sinf(0)` 이 이미 빠르다. 요만 도는 경우 18.0 → 18.7 ns 로 오히려 느렸다.
- 프리페치 — 수집 · 적용이 객체 그래프 지연에 묶여 있어 효과가 있을 자리지만 요청 조건(SIMD 등 하드웨어 명령 없이)에 따라 넣지 않았다.
- 채우기(후보 → raw, 직렬 ~90 us)를 워커로 — 2026-09-21 에 재서 기각한 그대로다(정렬 · 꼬리가 raw 를 원격 캐시에서 끌어온다).
- 벤치의 쓰기 목록 만들기(`BenchScene::update`, 직렬 ~120 us)는 게임 코드라 두었다 — 엔진 숫자를 벤치 손질로 부풀리지 않는다.
- 렌더 스레드의 인스턴스 업로드(768 KB 복사 ~90 us)는 임계 경로가 아니다(위의 1.1 번).

**테스트.** `GpuSceneTest.IncrementalTransparentTailMatchesFreshBuild` — 불투명 1500(청크 셋) + 투명 300(인스턴스 셋을 번갈아)을 여섯 프레임
사인파로 움직이고 한 프레임은 투명 깊이를 통째로 뒤집어(삽입 정렬 예산 초과) 이어 지은 결과를 **처음 보는 빌더**의 결과와 칸마다
(월드 비트 · 바운드 · 배치 번호 · 가리키는 머티리얼/인스턴스) 견준다 — 합치기 끔/켬 둘 다, 움직이기만 한 프레임이 제자리 갱신을
탔는지도(구간으로 실린다). 돌연변이 둘로 물리는지 확인했다: std::sort 폴백을 끄면 300 칸, 청크 번호를 하나 밀면 512 칸이 어긋난다.
`MathTest.CreateTrsWithoutRotationMatchesIdentityQuaternion` — 지름길을 단위 사원수 경로와 성분마다 정확히(음수 스케일 · -0 각 포함).
...
**한 것.** `Source/App` 14 개 파일과 `CMakeLists.txt` 의 주석을 Core 와 같은 규칙으로 다시 썼다. 사실과 달랐던 것:
- `ModuleHost.h` 머리말이 "RuntimeAPI 레이어에 위치하여" 라고 적고 있었다 → App 에 있고, RuntimeAPI 계약의 수명 주기를 App
  쪽에서 맡는다.
- `drainRenderWorkers` 를 "재웁니다" · "배수" 로 적었다 → 렌더 워커 · GPU · 태스크가 하던 일을 끝낼 때까지 **기다린다**(멈추지 않는다).
- 절 번호 `1) 3) 4) 5)`(`ModuleHost.h`), `2) 3)`(`App/CMakeLists.txt`) → 빠진 번호 없이.
- `LiveReloadManager.cpp` 에 남아 있던 영어 주석 한 블록을 한국어로.

**검증.** Debug · Shipping 빌드 경고 0 · `RunBuildWarnings --preset Ninja-Debug` 0 · `nogpu` + 린트 27/27 ·
`hostgpu`(Shipping) 2/2 · 주석 외 토큰 변화 0(파일마다 대조).
...
**같은 날, 구조로 바꿨다 — 묶임이 리스트보다 오래 살 수 없게.** 위 고침은 "풀어 주는 자리(제출 · 파괴 · 재설정)를 빠뜨리지 않는다" 에
기댄다 — 스레드 로컬이 **컨텍스트 포인터**를 드는 한, 새 경로 하나가 빠지면 같은 부류가 다시 난다. 그래서 포인터를 스레드 로컬에서
없앴다. 스레드 로컬은 **(디바이스 일련번호 · 슬롯 · 기록 세대) 토큰**만 들고, 컨텍스트는 디바이스가 소유한 슬롯 표
(`D3D11RHIDevice::_arrRecordingSlot`, 64 칸)에서 **세대가 맞을 때만** 나온다(`resolveRecordingContext`). 리스트가 태어나면 슬롯을
CAS 로 집고(`acquireRecordingSlot`, 잠금 없음), `beginCommandList` 가 새 세대(홀수)를 열며 토큰을 묶고, `endCommandList` 가 세대를
짝수로 올린다 — **닫는 순간 어느 스레드의 토큰이든 무효**다. 리스트를 놓으면 세대를 한 번 더 올리고 슬롯을 비운다(`releaseRecordingSlot`,
디바이스 종료의 detach 안에서도 불리므로 잠그지 않는다). 디바이스 일련번호는 전역 카운터라 같은 주소에 새 디바이스가 서도 옛 토큰이
맞아떨어지지 않는다. 표는 디바이스와 수명이 같으므로 토큰이 죽은 메모리를 가리킬 길이 없고, 세 자리의 되짚어 풀기는 걷어냈다
(`unbindRecordingContextIf` 삭제). 기록하는 스레드가 패스 시작에서 자기 토큰을 묶는 것(`bindRecordingThread`, 기록 중인 슬롯만)은
그대로다. 갱신 한 번의 비용은 스레드 로컬 비교 하나와 원자 읽기 둘이다. 검증: 회귀 테스트 통과 · 같은 명령 12 회 연속 통과 · BackendSmoke 네 백엔드 오류 0 · 에디터 ON 네 백엔드 오류 0 · Debug · Shipping · ASan 빌드 경고 0 · nogpu · hostgpu · 린트 전부 통과. 슬롯 표가 찬 적은 없다(64 칸, 경고 0).
아직 남은 더 큰 모양 하나: `updateConstantBuffer` 가 스트림을 모르는 `IRHIResource` 에 있는 것 자체가 이 장치가 필요한 이유다 —
갱신을 `IRHICommandList` 로 올리면 DX11 의 스레드 로컬이 통째로 사라진다. RHI 모듈 ABI(가상 표)가 바뀌는 일이라 별도 커밋감이고,
그때까지는 이 토큰이 그 자리를 안전하게 메운다.

**다른 백엔드에도 필요한가 — 아니다, 그리고 그 전제를 테스트가 못박는다.** DX12 의 `updateConstantBuffer` 는 버퍼마다 영구 매핑된
프레임 링 슬롯에 memcpy 하고(`_mapCbMapped`), Vulkan 은 그 버퍼의 메모리를 `vkMapMemory` 로 쓴다 — 둘 다 **컨텍스트를 거치지
않으므로** 어느 스트림으로 갈지 정할 것이 없고, 드로우별 값의 분리는 `PassConstantRing` 의 드로우별 버퍼가 한다. 리스트도 자기
것을 든다(DX12 는 리스트마다 얼로케이터 쌍, Vulkan 은 리스트마다 커맨드 풀 + 디스크립터 풀 묶음)라 begin 과 end 가 스레드를 넘어도
API 계약(동시 사용만 금지) 안이다. GL 은 `_bParallelCommandRecording = 0` 이라 리스트가 스레드를 넘지 않는다. 회귀 테스트를
DX11 · DX12 · Vulkan 셋으로 넓혀 같은 손 넘김(연 스레드 ≠ 닫은 스레드 → 즉시 제출 → 파괴 → 기록 밖 갱신)을 세 백엔드에서 돌린다.
...
개념이 아니라 "현재 잡 시스템 위에서의 그 구현" 이고, 순서가 틀렸던 것이다. 상용 엔진이 씬 갱신을
잡으로 돌려 이득을 보는 전제 셋(디스패치 바닥이 수 us · 워커가 깨어 있어도 싸다 · 우선순위 레인)이
여기엔 없다. 그래서 잡 시스템부터 쟀다.

**행렬** (큐브 100 · 2000 × 워커 14/16 × 스핀 2/50 us × 깨우기 all/one, `RT.Graph.executeParallel` us):

| | 스핀 2 · one | 스핀 2 · all | 스핀 50 · one | 스핀 50 · all |
|---|---|---|---|---|
| 워커 16 | 161~192 | 340~380 | 229~242 | 324~329 |
| 워커 14 | **150~158** | 333~350 | 214~229 | 258~295 |

- **`notify_all` 은 금지.** 워커 열넷이 일감 여섯을 다투는 천둥 무리 — 두 배 느리다. 넣은 태스크
  수만큼 `notify_one`(`wakeSleepingWorkers( n )`)이 가장 빠르다. 앞 회차에 "그룹당 한 번 깨우기가
  132 us 로 좋아졌다" 고 적은 것은 **편차였다** — 같은 설정 반복이 ±40% 흔들린다(150 vs 235).
- **스핀 늘리기는 손해.** `_workEpoch` 한 줄만 읽어 큐 경합을 없앤 스핀이어도 50 us 면 SMT 형제
  코어를 점유해 GT·RT 가 느려진다. 소수(2·4)만 길게 돌리는 hot pool 도 이득이 없었다.
- 워커 14 vs 16 은 중립 — 그래도 **하드웨어 − 2(GT·RT 몫)** 로 둔다(`kReservedThreadCount`).
- 남긴 것: 에포크 게이트 스핀(경합 없음, 예산 2 us), 그룹 단위 `submitWithoutWake` + 필요한 수만
  깨우기(렌더 그래프 웨이브도), `TaskTest.ParallelGroupWakesWorkersOnce`(서브태스크마다 깨우는 변이에
  실패한다), 깨운 횟수 통계(`getWakeSignalCount`).

**결론을 고쳐 적는다.** 디스패치 바닥(~50 us)은 잠든 워커의 OS 웨이크 지연이고 이 풀 구조로는 더
못 내린다. 이 벤치의 프레임은 **0.7 ms** 라 그 바닥이 7% 이고, 그래서 GT 병렬 씬 갱신이 안 남는다.
상용 엔진의 8~16 ms 프레임에서는 같은 바닥이 <1% 라 답이 뒤집힌다 — **"지금 불필요" 이지 "영영
불필요" 가 아니다.** 프레임당 일이 열 배쯤 커지는 시점(실제 게임 씬)에 다시 걸 순서:
...
**→ 2026-09-23 에 다시 고쳤다:** "이 풀 구조로는 더 못 내린다" 가 틀렸다 — 바닥의 대부분이 뮤텍스 호송과 청크 노드였고,
주소 대기 + 티켓으로 잠든 풀의 포크-조인이 67~118 → 34 us 다(1절 (B) 열여섯째).

### 2026-09-20 (GPU 프레임 전체를 재니 "기다림" 은 처리량이 아니라 히치였다 — 정렬 커널 고정 512 루프 · 잡 풀 경합 · 기각 셋)

**1. 계측을 한 칸 더 넓혔다 — `GPU.Frame` · `GPU.Compute`.** 패스 합(270 us)과 펜스 대기(400 us)가 안
맞았다. 타임스탬프 칸 29~31 을 프레임 시작·컴퓨트 프리패스에 예약하고(`FrameRendererUtil::kGpuTimestampSlot*`,
패스는 14 개까지), 네 백엔드의 기준점을 "번호가 낮은 칸" 에서 **"가장 이른 시각"** 으로 바꿨다 —
안 그러면 뒤쪽 칸의 값이 음수가 되어 0 으로 잘린다.

**2. 정렬 커널이 개수와 무관하게 512 칸을 돌고 있었다.** 바이토닉 정렬이 `count` 를 보지 않고 늘
`SW_SORT_MAX_ELEMENTS` 만큼 채우고 비교했다 — 큐브 100 개 씬에서 컬링·정렬 프리패스가 **약 110 us**
(컬링 끄면 −60, 투명 없애면 −97). 실제 개수의 다음 2의 거듭제곱까지만 일한다: DX12 `GPU.Compute`
**21 us**, DX11 **10 us**. FXC 함정 하나 — `count` 는 UAV 에서 읽은 값이라 그 값에 걸린 흐름 안의
배리어는 X4026 으로 거부된다(그룹 안에서 같은 값이지만 증명할 수 없다). 그래서 배리어는 상수 루프에
그대로 두고 **비교·교환만** n 안으로 줄였다. 배리어 45 번은 남지만 싸다.

**3. `RT.BeginFrame` 400 us 는 GPU 처리량이 아니다.** 씬 크기와 무관했고(큐브 100: 398 · 2000: 430),
`GPU.Frame` 은 231~353 us 인데 프레임 주기는 626~828 us — **GPU 가 절반 넘게 논다.** 임시 히스토그램으로
분포를 보니 평균이 거짓말이었다 (600 프레임 · 큐브 100 · DX12):

| <50us | <200 | <500 | <1ms | <2ms | <5ms | <10ms | ≥10ms |
|---|---|---|---|---|---|---|---|
| 262 | 230 | 59 | 9 | 29 | 8 | 1 | 2 |

82% 가 200 us 미만이고 **40 프레임이 1~18 ms 히치**다. Vulkan 도 같은 꼬리이고 DX11 은 598/600 이
...
`PrimitiveRegistry`(원자 플래그)까지 만들어 쟀다.

| | 직렬 | 병렬 1차 | 원인 |
|---|---|---|---|
| 8000 플러시 | 368 | 262 | 이것만 이득 |
| 8000 수집 | 137 | 197 | 디스패치 비용 |
| 2000 수집 | 28 | **271** | 일 28 us 에 디스패치 ~250 us |

디스패치를 쪼개 보니 **분배 125 us(대기 30)** — 서브태스크 32 개마다 잠든 워커를 `notify_one` 으로
깨우는 시스템콜이었다. 그룹당 한 번 깨우기(`wakeSleepingWorkers`)로 **~50 us**. 그러고도 8000 플러시가
304 us 로 직렬과 같았고, **RT 의 그래프 병렬 기록이 170 → 261 us 로 느려졌다** — 게임 스레드의 잡이
렌더 스레드의 잡과 **같은 워커 풀**을 다툰다. 워커 스핀을 2 us → 50 us 로 늘리는 것도 재 봤다: 분배는
줄지만 워커 열다섯이 전역 큐를 두드리며 GT·RT 코어를 빼앗아 프레임 전체가 느려진다(GT 889 → 1305).
**전부 되돌리고 "그룹당 한 번 깨우기" 만 남겼다** — 렌더 그래프 병렬 기록이 큐브 100 에서 150~166 →
132~134 us. 교훈: 이 풀은 분배 바닥이 ~50 us 이고 GT·RT 가 나눠 쓰므로, **노는 코어가 있을 때만**
나눠야 하고 그것은 재 봐야 안다.

**5. 이름 둘.** `SW_LOG_LEVEL_COMPILED( 2 )` 의 2 가 무엇인지 읽는 사람이 알 수 없었다 —
`SW_LOG_VERBOSITY_ERROR/WARNING/INFO/TRACE` 로 이름을 붙이고 `static_assert` 로 `LogLevel` 열거자와
묶었다. 계측 게이트는 `SW_PROFILE_COMPILED` 하나로 부른다("왜 Info 인가" 를 호출부가 몰라도 된다).

**6. 씬 경로가 주광을 채우지 않았다.** `FrameRenderer::execute( device, &scene )` 는 라이트 버퍼만
채우고 `_frameLight`(그림자 행렬·앰비언트·폴백)는 패킷 경로만 채웠다 — 테스트가 씬에 방향광을
세게 두어도 어두웠던 이유의 절반이다. 나머지 절반은 머티리얼이 없어 알베도가 0 이었던 것.
둘을 고치니 `FusedPostChainMatchesStaged` 가 블룸 뒤 `saturate` 를 빼는 변이에 **최대 차이 31 · 픽셀 7%**
로 실패한다 — 앞 회차에 "덮지 못했다" 고 적은 구멍이 닫혔다.

### 2026-09-20 (CI 가 죽던 것 — 락프리 풀의 "이중 반납" 단언이 느린 기계에서 잘못 터졌다)

`CoreTest` 가 리눅스 CI 에서 `ILLEGAL` 로 죽고 있었다. Debug 의 `SW_LOG_ASSERT` 는
`SW_DEBUG_BREAK()` 까지 하므로 디버거가 없는 CI 에서는 **프로세스가 죽는다.**

```
DataStructureTest.LockFreeObjectPoolLosesNoBlockUnderContention
LockFreeObjectPool::release: free queue stayed full — the pointer is not this pool's, or was already released.
```

**끝을 횟수로 잡은 것이 원인이다.** 앞선 회차(`4d11dcff`)가 "가득 참은 이중 반납이 아니다" 를
알고 재시도를 넣었는데, 상한이 `yield()` **1024 번**이었다. Vyukov 큐는 소비자가 칸을 집어간 뒤
순번을 공개하기 전 찰나에도 생산자에게 가득 찼다고 답한다. 그 창은 소비자가 선점되면 스케줄러
퀀텀(밀리초) 동안 열려 있는데, `yield()` 1024 번은 마이크로초면 끝난다 — 코어가 적은 러너에서
**정상 반납이 이중 반납으로 몰렸다.**

고친 방식은 둘이다.
1. **남의 포인터는 기다리지 않고 가린다** — `owns()` 가 스토리지 범위와 슬롯 간격으로 즉시 판정한다.
   예전에는 이것도 "자리가 안 난다" 는 증상으로 알아냈고, 그 증상이 정상 반납과 구별되지 않았다.
2. **남은 기다림은 시간으로 끊는다** — 1 초. 이 풀의 블록이 자리를 못 얻는 이유는 이중 반납뿐이고,
   그건 버그 경로라 1 초를 써도 된다. 느린 기계와 진짜 버그가 이제 갈린다.

`DataStructureTest.LockFreeObjectPoolOwnsOnlyItsOwnBlocks` 가 `owns()` 를 검사한다 — 범위 검사나
간격 검사를 빼는 변이 둘 다 실패한다. **재현은 로컬에서 못 했다**(128 스레드·용량 2·단일 코어
고정까지 해도 안 터졌다) — 코어가 많은 Windows 에서는 그 창이 너무 짧다. 그래서 고친 근거는
재현이 아니라 구조다: 끝을 횟수로 잡으면 기계 속도에 따라 답이 달라진다.

### 2026-09-20 (후처리 셋을 Present 한 패스로 합쳤다 — GPU 프레임 -25~41%)

앞 회차가 "합치면 -125 us 인데 조립이 깨진다" 로 남겨 둔 것을 닫았다. **조립은 안 깨진다.**

**어떻게.** 효과를 패스가 아니라 **함수**로 뺐다(`postbloom.hlsli` · `postoutline.hlsli`).
`postchain.hlsl` 은 퍼뮤테이션으로 부를 것만 부르고, 무엇을 켤지는 여전히 파이프라인 XML 한 줄이다.

```xml
<_listPermutation>
    <item>SW_POST_BLOOM=1</item>
    <item>SW_POST_OUTLINE=1</item>
    <item>SW_POST_TONEMAP=1</item>
</_listPermutation>
```

엔진 쪽 변경은 둘뿐이다 — Present 계약이 깊이·AO 를 **선택 입력**으로 받고, Present 실행이
선언된 입력을 전부 건다. PSO 는 원래부터 XML 의 `_shaderPath`·`_listPermutation` 을 읽고 있었다.

**결과** (720p · 벤치 2000 · 200 프레임, GPU 프레임 합):

| | DX11 | DX12 | Vulkan | OpenGL |
|---|---|---|---|---|
| 앞 회차 | 261 | 380 | 492 | 668 |
| 합친 뒤 | **155** | **263** | **335** | **501** |
| | -41% | -31% | -32% | -25% |

그림은 같다 — 나눈 판과 **최대 차이 1**(중간 타깃 UNORM8 반올림이 한 번 줄어든 것).

**합치다가 드러난 것 — 스크린샷은 최종 화면을 본 적이 없다.** `-gv_screenshot` 은 백버퍼가 아니라
Present 가 **읽는** 첨부를 찍는다(백버퍼는 핸들이 없어 읽을 수 없다). 그래서 톤맵은 한 번도
스크린샷에 들어간 적이 없고, 후처리를 Present 로 옮기자 후처리 전체가 안 보이게 됐다.
`FrameRenderer::setPresentCaptureEnabled` 가 그 구멍을 닫는다 — 스크린샷 실행에서만 Present 를
텍스처로 받고 백버퍼로 복사한다(전체화면 복사 한 번). 이제 찍히는 그림이 실제로 나간 그림이다.

**검증.** `RenderPassGpuTest.FusedPostChainMatchesStaged` 가 같은 씬을 `forwardpipeline.xml`(합친 판)과
`forwardpipelinestaged.xml`(나눈 판)로 그려 네 백엔드에서 픽셀을 맞춘다. 외곽선 단계를 빼는 변이를
걸면 네 백엔드 모두 실패한다(최대 차이 7).

**(같은 날 닫았다)** 블룸 뒤 `saturate` 는 처음엔 테스트가 못 덮었다 — 씬 경로가 주광을 안 채우고
머티리얼이 없어 씬이 검었다. 둘을 고쳐 변이가 물린다(아래 회차 6 번).

**남은 여지.** 이제 Present 가 GPU 프레임의 절반 가까이다(DX12 125/263). 그중 **백버퍼 쓰기만으로
62 us** 이고 이건 누가 해도 내야 하는 값이다. 더 줄이려면 후처리를 반해상도로 돌리는 쪽이다.

### 2026-09-20 (타임스탬프로 다시 재니 기각했던 가설 하나가 살아났다 — GPU 프레임 −10%, DX11 은 −20%)

바로 앞 회차에서 패스별 GPU 시간이 생겼다. 그것으로 **예전에 `RT.BeginFrame` 으로 재고 기각했던
가설들을 다시 걸었다.** 하나는 정말 틀렸고, 하나는 계측이 나빠서 기각됐던 것이었다.

**1. 무엇이 비싼지부터 다시 물었다.** (720p · DX12 · 벤치 2000)

| 프로브 | 결과 | 읽은 것 |
|---|---|---|
| 해상도 1/4 (640x360) | Outline 122 -> 33 · PostBloom 69 -> 19 | 비용은 **픽셀당**이다. 패스 고정비는 3 us 남짓 |
| 깊이 탭 5 -> 1 | Outline 121 -> 91 | 여분의 깊이 탭 넷이 **30 us** |
| Present 가 아무것도 안 읽게 | 87 -> 62 | **720p 백버퍼 쓰기만으로 62 us**, 읽기가 25 us |

**2. 살아난 가설 — `BloomColor` FP16 -> RGBA8.** 예전엔 "355~365 vs 347, 차이 없음" 으로 기각했는데,
그 숫자는 vsync·백프레셔가 섞인 `RT.BeginFrame` 이었다. 패스 시간으로 보면 **PostBloom 68 -> 53,
Outline 122 -> 111** 로 분명하다(재실행 편차 ±1 us). 애초에 이 파이프라인에서 FP16 은 값을 지키지
못한다 — 바로 다음 패스인 Outline 이 `R8G8B8A8_UNORM` 타깃이라 거기서 어차피 잘린다.

**3. 깊이 십자 다섯 칸을 게더 두 번으로.** `GatherRed` 는 필터 없이 텍셀 넷을 돌려주고 값은
바이리니어 샘플 하나 수준이다. **1/4 텍셀** 비낀 두 게더의 2x2 가 십자 다섯 칸을 정확히 덮는다
(반 텍셀은 안 된다 — 정확히 텍셀 모서리라 어느 2x2 를 잡을지가 반올림에 달린다).
`binding.hlsli` 의 `SampleDepthCross` 가 그것이고, 네 이웃은 **순서를 약속하지 않는다**
(쓰는 쪽이 합만 보므로 게더 성분 순서가 백엔드마다 달라도 결과가 같다).

**4. 다시 기각한 것 둘.** 둘 다 이번엔 좋은 계측으로 쟀는데도 안 움직였다.
- **점 샘플러로 바꾸기.** 전체화면 패스는 텍셀 중심에 정확히 떨어져 선형 필터가 섞을 것이 없으니
  값이 쌀 줄 알았다. **122 -> 121.** 이 GPU 에서 바이리니어는 사실상 공짜다. (헬퍼
  `SampleDepthPoint`/`SampleSourcePoint` 는 의미가 맞아 남겼다 — 가장자리 WRAP 을 CLAMP 로 바꾼다.)
- **전체화면 타깃의 클리어 제거.** 예전 기각이 그대로 유효하다 — 패스 시간으로 봐도 **0 us** 다.

**결과** (720p · 벤치 2000 · 200 프레임, us):

| 패스 | DX11 | DX12 | Vulkan | OpenGL |
|---|---|---|---|---|
| PostBloom | 113 -> **94** | 68 -> **54** | 77 -> **68** | 84 -> **77** |
| Outline | 120 -> **70** | 123 -> **95** | 141 -> **120** | 271 -> **249** |
| GPU 프레임 합 | 327 -> **261** (-20%) | 420 -> **380** (-10%) | 533 -> **492** (-8%) | 709 -> **668** (-6%) |

그림은 그대로다. 4 백엔드 모두 **276 만 바이트 중 3~4 바이트, 최대 차이 1** 이다.

### 2026-09-20 (기하가 있는 씬을 픽셀로 비교할 수 있게 됐다 — `-gv_benchAnimate=0`)

위 작업 중에 **하마터면 맞는 최적화를 "그림이 달라졌다" 며 버릴 뻔했다.** 게더 판을 5 탭 판과
비교하니 0.5~1% 가 달랐는데, 손대지 않은 대조군(같은 빌드 두 실행)이 **1.5%** 움직이고 있었다.

벤치는 두 가지를 벽시계 시간으로 움직인다 — 컴퓨트가 만드는 회전과, `BenchScene::update` 의
사인파가 만드는 상하 이동·스케일이다. **회전만 멈춰서는 부족했다**(여전히 0.05% 흔들렸다).
`-gv_benchAnimate=0` 은 둘 다 멈춘다. 그 뒤 네 백엔드 모두 같은 프레임이 **바이트까지 동일**하고,
게더 판과 5 탭 판도 **완전히 같다**(0 바이트 차이) — 애초에 같은 텍셀을 읽으니 당연한 결과였다.

```powershell
cd build/Ninja-Release/Bin
./App.exe -gv_rhiBackend=1 -gv_benchMeshes=200 -gv_benchAnimate=0 `
  -gv_screenshot=a.ppm -gv_screenshotFrame=30 -gv_profileFrames=40
```

**셰이더를 고쳤으면 `App.exe --bake-shaders` 를 먼저 돌릴 것.** 구운 바이너리는 저장소에 들어
있고 빌드가 다시 굽지 않는다 — 이번에도 옛 바이너리로 한 번 재고 "효과 없음" 이라 읽었다.

**다음 사람에게.** 이제 골든 이미지 회귀 테스트의 전제가 생겼다. 지금 렌더 변경의 검증은 손으로
찍어 비교하는 것뿐이라, 멈춘 벤치의 스크린샷을 기준으로 삼는 테스트가 있으면 이 회차에서 한
것이 자동으로 지켜진다.

**아직 안 한 큰 것 — 전체화면 패스 셋을 하나로 합치기.** 숫자는 이제 있다. PostBloom 53 +
Outline 95 + Present 87 = **235 us 가 GPU 프레임의 62%** 이고, 그중 백버퍼 쓰기 62 us 만이
피할 수 없는 값이다. 셋을 한 패스로 합치면 중간 타깃 두 번의 쓰기·읽기가 사라져 **대략 -125 us
(프레임의 33%)** 가 나온다. 하지만 블룸·외곽선·톤맵이 셰이더 하나로 묶여 파이프라인 XML 로
패스를 끼우고 빼는 조립이 깨진다. **이득은 이제 추정이 아니라 측정값이니**, 조립 가능성을 얼마나
중요하게 볼지만 정하면 되는 문제다.

### 2026-09-20 (패스별 GPU 시간을 네 백엔드 전부에서 잰다 — 패스를 지워 가며 추측하던 것을 끝냈다)

**패스를 지워서 비용을 나누는 방법은 틀린 답을 준다.** 바로 위 회차(블룸)에서 그렇게 구한 값이
`Outline ~38 us` · `PostBloom ~145 us` 였다. 진짜 타임스탬프를 붙여 보니 **정반대**다.

| 패스 | 지워서 추정 | 실측(DX12) |
|---|---|---|
| Outline | ~38 us | **123 us** |
| PostBloom | ~145 us | **68 us** |

패스를 지우면 그 패스만 사라지는 것이 아니라 **타깃 사슬이 같이 바뀌기 때문**이다. 앞으로 GPU
비용을 나눌 때는 `GPU.<패스>` 를 볼 것 — `-gv_profileFrames` 를 주면 프로파일 표에 같이 나온다.

**네 백엔드 전부에 넣었다** (720p · 벤치 2000 · 200 프레임, 단위 us):

| 패스 | DX11 | DX12 | Vulkan | GL |
|---|---|---|---|---|
| Shadow | 2 | 24 | 27 | 38 |
| ForwardOpaque | 40 | 102 | 95 | 241 |
| Transparent | 11 | 16 | 18 | 17 |
| PostBloom | 113 | 68 | 77 | 84 |
| Outline | 120 | 123 | 141 | 271 |
| Present | 41 | 87 | 175 | 58 |

**계약은 셋이고, 셋째가 이 일의 어려운 부분이었다.**
1. 칸은 **패스 인덱스로 고정**이다(패스당 begin/end 두 칸). 패스는 병렬로 기록되므로 흐르는
   카운터를 쓰면 그 자체가 경쟁이 된다.
2. **기다리지 않는다.** 읽기는 그 링 슬롯이 펜스를 통과한 직후, 즉 다시 쓰기 직전에만 한다.
   기다려서 최신 값을 받으면 재려던 그 파이프라인을 멈춰 세워 숫자가 거짓이 된다 — 값이 몇
   프레임 늦는 쪽을 골랐다.
3. **안 적은 칸은 음수로 온다.** 안 적은 칸에 무엇이 남는지가 백엔드마다 다르기 때문이다 —
   DX12·DX11 은 쿼리를 리셋하지 않아 **지난 사이클 값**이 그대로 남고(그래서 건너뛴 패스가
   0 us 나 남의 값으로 보고된다), Vulkan·GL 은 "아직 준비 안 됨" 이다. Vulkan 은 그것 하나
   때문에 범위 전체가 `VK_NOT_READY` 가 되어 **아무 값도 못 받는다** — `VK_QUERY_RESULT_WITH_AVAILABILITY_BIT`
   로 칸마다 따로 물어야 한다. DX12·DX11 은 적은 칸을 원자 비트마스크로 기록해 가린다.

`RHIDeviceTest.GpuTimestampsMarkUnwrittenSlotsAllBackends` 가 이 셋을 네 백엔드에서 검사한다.
0·1 번만 적고 나머지 30 칸이 전부 음수인지 본다 — 백엔드 넷의 음수 표시를 하나씩 `0.0f` 로
바꾸는 변이를 걸면 넷 다 그 백엔드 이름으로 실패한다. (Present 가 없으면 DX11 의
`GetData(DONOTFLUSH)` 도 GL 의 `QUERY_RESULT_AVAILABLE` 도 스스로 큐를 밀지 않아 쿼리가 영영
안 끝난다 — 테스트가 프레임마다 `waitIdle()` 을 부르는 이유다.)

**계측 비용은 두 겹으로 막았다.** (1) `SW_LOG_LEVEL_COMPILED( 2 )` — Shipping 에서는 패스의
타임스탬프 기록과 보고가 통째로 사라진다. (2) `IRHIDevice::setTimestampEnabled` — Dev 에서도
프로파일러가 꺼져 있으면 **백엔드가 쿼리 힙·풀을 만들지조차 않는다.** 백엔드는 별도 모듈이라
엔진 전역(프로파일러)을 볼 수 없으므로, 정책은 `FrameRenderer` 가 쥐고 여기는 메커니즘만 둔다.

`constant::kMaxGpuTimestampSlot`(32) 은 네 백엔드가 같은 값으로 힙·풀·쿼리 배열을 잡아야 하므로
`RHITypes.h` 의 계약 상수 블록에 있다. **패스 16 개까지**다 — 렌더 그래프가 그보다 길어지면
뒤쪽 패스는 조용히 빠진다. ABI v6 / `rhi-cl-v16-2026-09`.

### 2026-09-20 (더티 구동을 채우기·제자리 갱신까지 — 10 개 이동 시 build 495 -> 77 us)

앞 회차에서 수집만 더티 구동으로 바꿨더니 `build` 495 -> 202 us 였고, 남은 것을 쪼개 재니
**전부 "8000 개를 다 훑기"** 였다 (8000 큐브 중 10 개 이동 · DX12):

| 단계 | us | 하는 일 |
|---|---|---|
| `refresh.restore` | 74 | 발행본을 되복사(800 KB) |
| `fill` | 60 | 후보 -> raw 전체 복사 |
| refresh 루프 | 47 | 인스턴스 8000 순회 |
| `refresh.stamp` | 38 | 머티리얼 원소 도장 |

**고친 것 셋.**
1. **`fill` 을 더티 구동으로.** raw 는 후보에서 1:1 로 나오므로 손대지 않은 후보의 raw 는 지난
   프레임 것이 그대로 맞다. 60 -> 0 us.
2. **제자리 갱신을 더티 구동으로.** `buildBatches` 에서 후보 -> 인스턴스 **역매핑**을 같이 만들어
   (`_listCandidateToInstance`), 더티 후보의 인스턴스 자리만 고친다. 47 -> 0 us.
   회전 인스턴스 수는 전부 훑지 않으므로 **증감으로 유지**한다 — 240 회 빌드에서 전수 계산과
   불일치 0 회임을 프로브로 확인했다.
3. **부분 프레임에는 회수 시계를 멈춘다.** 시계를 돌리면서 더티 원소만 도장을 찍으면, 손대지
   않았지만 여전히 쓰이는 원소가 낡은 것으로 보여 **회수돼 버린다.** 시계를 멈추면 아무것도
   낡지 않아 잘못된 회수가 없다 — 회수가 전체 훑기 프레임으로 미뤄질 뿐이다. 38 -> 0 us.

**결과** (8000 큐브 · 투명 0%)

| 움직이는 큐브 | build 처음 | 수집만 고친 뒤 | 지금 |
|---|---|---|---|
| 10 | 495 us | 202 | **77** |
| 100 | 485 | 231 | **89** |
| 8000 | 467 | 433 | 411 (전체 경로) |

**재서 버린 것 하나 — 발행 배열 풀.** 남은 74 us 는 발행본 되복사(800 KB)다. `shared_ptr` 배열을
풀에 돌려 써 할당을 없애 봤는데 **더 느렸다**: 풀 2/3/8 에서 발행 85/78/76 us 로, 갓 할당한
블록보다 차가웠다(8 개면 작업 집합이 6.4 MB 다). **800 KB 복사 자체가 ~75 us 이고 할당은 그 안에서
작다.** 되돌렸고, 그 숫자를 코드 주석에 남겼다.

**남은 것.** 그 74 us 복사는 "RT 가 읽는 동안 GT 가 못 고친다" 는 제약에서 나온다. 없애려면
발행본에 **지난 프레임 이후의 변경 구간만 덧칠**해야 하는데, 버퍼마다 "어느 세대까지 반영했나" 를
들고 구간을 재생해야 한다. 지금 이득(74 us)에 비해 복잡해서 하지 않았다.

### 2026-09-20 (바뀐 프리미티브만 다시 모은다 - 8000 개 중 10 개가 움직일 때 수집 244 -> 1 us)

**앞서 "영속 렌더 씬(FScene 모델)은 기각" 이라고 적은 것은 축이 틀렸다.** 그때는 `-gv_benchMeshVariants`
로 **종류**만 늘려 재고 "평평하니 이득 없다" 고 결론냈는데, 이 모델의 진짜 사용처는 **큰 씬에서 몇 개만
움직이는 경우**다. 그 축으로 재니:

| 움직이는 큐브 | build | collect |
|---|---|---|
| 0 | 0 us | (건너뜀) |
| 10 | **495** | **244** |
| 8000 | 467 | 185 |

**10 개만 움직여도 8000 개 전부 움직일 때와 같았다.** 매 프레임 모든 프리미티브의 후보를 다시
만들고 있었기 때문이다.

**이미 있던 것 — 등록부는 더티를 알고 있었다.** `PrimitiveRegistry` 가 `_listDirty`(바뀐 프리미티브
인덱스)와 `_setGeneration` 을 들고 있는데, 빌더는 `clearDirty()` 로 **지우기만 하고 목록은 버렸다.**

- `consumeDirty()` 로 목록을 받아 온다.
- 수집 본문을 `fillCandidateFromPrimitive()` 로 빼서 **전체 수집과 부분 수집이 같은 함수**를 쓴다
  (규칙이 갈리면 부분 갱신만 낡은 필드를 남기고, 그 화면은 대부분의 프레임에서 멀쩡해 보인다).
- 집합 세대가 그대로면 후보 자리 배치도 그대로다 -> 더티 프리미티브의 자리만 새로 채운다.
  `_listPrimitiveToCandidate` 가 등록부 인덱스 -> 후보 인덱스를 들고 있다.
- **조건이 하나라도 어긋나면 전체 수집으로 떨어진다**: 집합 세대·퍼뮤테이션 세대가 달라졌거나,
  더티가 전체의 1/4 을 넘거나, 더티 프리미티브의 **실릴지 말지가 바뀌었을 때**(자리 배치가 달라진다).
- 이중 버퍼는 시작에서 한 번 더 swap 해 "지난 프레임 후보" 를 작업 자리로 가져오는 것으로 맞췄다.
  부분 경로에서는 배열 통째 비교(`compare`)도 배치 키 전수 비교도 하지 않는다 — **무엇이 바뀌었는지
  이미 알기 때문**이다.

**결과** (8000 큐브 · 투명 0% · DX12 · 120 프레임)

| 움직이는 큐브 | collect 전 | collect 후 | build 전 | build 후 |
|---|---|---|---|---|
| 10 | 244 us | **1** | 495 | **202** |
| 100 | 228 | **5** | 485 | **231** |
| 8000 | 185 | 150 | 467 | 433 |

**왜 안전한가 — 더티 커버리지가 이미 완전하다.** 트랜스폼은 `onWorldTransformUpdated`,
메시·머티리얼·가시성은 세터가 `markRenderStateDirty` 를 찍고, **GameObject 활성 상태는 집합 세대를
올린다**(`refreshActiveInHierarchy` -> `markPrimitiveSetDirtyOnManager`) — 그 셋이 수집 필터의 세
...
### 2026-09-20 (간접 드로우는 돌고 있었지만, 셰이더가 **쓰지도 않는 값**이 병합을 막고 있었다)

"generate command + indirect draw 가 잘 되고 있나" 를 재서 답한 회차.

**먼저 프로파일 표를 잘못 읽고 있었다.** `RT.Draw.indirectCalls` 가 0 으로 보여 경로가 죽은 줄
알았는데, **카운터 값은 시간 열이 아니라 `per_frame` 열에 실린다**(시간이 없는 구간이 곧
`SW_PROFILE_COUNT` 다 — `FrameProfiler.cpp` 주석에 적혀 있다). 제대로 읽으니:

| 도형 종류 | 배치/프레임 | indirect 호출/프레임 |
|---|---|---|
| 1 | 3 | 3 |
| 8 | 1795 | **1206** |
| 64 | 2105 | **1335** |

경로는 돌고 있었지만 **1795 배치가 1206 호출로밖에 안 묶였다.** 병합 실패 사유를 세어 보니
`failMaterial` 이 1204 회 — 정점 버퍼도 PSO 도 아니었다.

**원인: `_materialCb` 가 병합 키에 들어 있었다.** 그런데 GPUScene 경로의 머티리얼은 구조버퍼
(`g_SwMaterials`, t9)에서 인스턴스의 `_materialIndex` 로 읽는다 — 그런 셰이더에는 머티리얼 CB
슬롯이 아예 없어서 `bindForDraw` 가 그 값을 **걸지도 않는다.** 그리기에 아무 영향이 없는 값이
깊이순으로 섞인 투명 배치를 하나씩 따로 그리게 하고 있었다.

`MaterialCB` 를 실제로 선언하는 것은 `fullscreentriangle.hlsl`(비-GPUScene 드로우용 b1 픽스처)
뿐이다. 그래서 **레이아웃이 그 슬롯을 가질 때만** 키에 넣도록 고쳤다.

| | gpuBatches(RT CPU) | indirect 호출 | RT.Frame |
|---|---|---|---|
| 전 | 539 us | 1207 | ~2190 |
| 후 | **180 us** | **3** | ~2190 |

**렌더 스레드 CPU 작업이 3 배 줄었다.** 프레임은 GPU-bound 라 그대로고, 줄어든 것은 여유다.
화면은 같다(표본 차이 0.04% · 평균 R 34.99 vs 34.99), 4 백엔드 모두 그려진다.

**테스트로 못 덮었다 — 다음 사람을 위해 이유를 적어 둔다.** `MergedSceneDrawsMatchPerBatch` 에
오브젝트마다 `MaterialInstance` 를 붙여 이 경로를 태우려 했는데, 변이(키에 무조건 포함)를 걸어도
테스트가 통과했다. `MaterialInstance::getDescriptorIndex()` 는 **자기 상수버퍼가 없으면 부모
머티리얼 것으로 떨어지고**, 오프라인 테스트 경로에서는 인스턴스 CB 가 만들어지지 않아 배치의
`_materialCb` 가 전부 같아진다. 값을 덮어써도(`setVectorParameter`) 마찬가지였다. 근거는 벤치
실측(위 표)이다. 테스트로 덮으려면 인스턴스 CB 가 실제로 만들어지는 경로부터 세워야 한다.

### 2026-09-20 (GPU 를 재 보니 기하가 아니라 전체화면 패스였다 - 블룸 블러를 바이리니어 2탭으로)

**GPU 타임스탬프가 없다.** 그래서 `RT.BeginFrame`(GPU 백프레셔)을 GPU 시간의 대리값으로 쓰고,
**패스를 빼 가며** 차이로 패스별 비용을 구했다. 파이프라인이 데이터(XML)라 가능한 방법이다.

**1. 기하가 아니라 고정 비용이 GPU 를 먹고 있었다.** (720p · DX12 · 120 프레임)
...
메시 8000 개가 더하는 것은 290 us 뿐이고 **712 us 는 씬과 무관한 고정 비용**이다.

**2. 패스를 빼서 나눈 GPU 비용** (빈 씬 기준, `RT.BeginFrame`)

| 구성 | BeginFrame |
|---|---|
| 전부 | 348 us |
| Outline 없음 | 310 (**Outline ~38**) |
| Outline + PostBloom 없음 | 165 (**PostBloom ~145**) |

**3. 재서 기각한 것 둘.** 둘 다 "당연히 이것이겠지" 였는데 숫자가 안 움직였다.
- **전체화면 패스의 클리어를 `DontCare` 로.** 전체화면 패스는 모든 픽셀을 덮어쓰니 클리어가
  낭비라고 보고 바꿨는데 **344~369 vs 기준 347** 로 차이가 없었다(DX12 데스크톱 드라이버가
  이미 지워 주는 듯하다). 이득 없는 변경은 넣지 않았다. *타일 GPU 로 가면 다시 볼 것.*
- **`BloomColor` 를 FP16 에서 RGBA8 로.** 픽셀당 8 -> 4 바이트인데 **355~365 vs 347** 로
  차이가 없었다. 이 패스는 대역폭 병목이 아니다.
  > **이 기각은 틀렸다 — 계측이 나빴다.** `RT.BeginFrame` 에는 vsync·백프레셔가 섞여 있다.
  > 패스별 타임스탬프로 다시 재니 PostBloom 68 -> 53 · Outline 122 -> 111 로 분명했다(같은 날 회차).

**4. 실제로 먹힌 것 - 블러 4 점탭을 바이리니어 2 탭으로.** 샘플러가 `SW_SAMPLER_LINEAR_WRAP`
이므로 텍셀 중심에서 **반 칸 비낀 곳**을 찍으면 이웃 네 텍셀이 공짜로 평균된다 — 두 번이면
여덟 텍셀이다. **GPU 프레임 355 -> 317 us (-11%)**, 화면 차이는 표본 0.51% · 평균 0.16 으로
보이지 않는다. (point 샘플러였다면 성립하지 않는 최적화다 — 셰이더만 보고 옮기지 말 것.)

> **이 절의 2 번 표(패스를 빼서 나눈 비용)는 틀렸다.** 같은 날 GPU 타임스탬프를 붙여 다시 재니
> Outline 123 us · PostBloom 68 us 로 **뒤바뀐다** — 패스를 빼면 타깃 사슬도 같이 바뀌기 때문이다.
> 위 4 번(바이리니어 2 탭)의 이득은 전후 프레임 실측이라 그대로 유효하다.

**남은 것.** PostBloom 은 아직 ~110 us 로 GPU 의 3 분의 1 이다. 클리어도 포맷도 아니었으니
남은 후보는 **해상도**(반해상도 블룸 — 트랜지언트 풀이 지금은 스왑체인 해상도 하나로 고정이라
부착물별 배율이 필요하다)와 **패스 합치기**(PostBloom + Outline + Present 가 각각 720p 를
읽고 쓴다 - 다만 합치면 데이터 구동 조립이 깨진다). 둘 다 재고 시작할 것.

### 2026-09-20 (씬에서 온 메시가 하나도 안 묶이고 있었다 - 배치 8000 -> 6, 프레임 2469 -> 1462 us)

"쿠킹 말고 런타임도 더 되나" 를 재다가 나온 회차. **벤치에는 한 번도 안 보이던 결함**이 나왔다.

**1. 먼저 잰 것 - 정적 씬에서 게임 스레드는 이미 할 일이 없다.** (Release · 정적 8002 엔티티)

| scope | 값 |
|---|---|
| `GT.GpuScene.build` | **0 us** (캐시 경로가 완벽히 먹는다) |
| `GT.Scene.tick.flushTransforms` | **0 us** (세대 가드) |
| `GT.Packet.export` | **441 us** ← 유일한 실제 작업 |

**2. 백로그의 "GpuScene.build 는 초선형 -> SoA 재배치" 는 기각한다.** 구조체를 일부러 78% 키워도
(`DrawCandidate` 144 -> 256 B) 숫자가 안 움직였다. 촘촘히 재니 메시가 2 배될 때마다 2.1~2.4 배로
거의 선형이다 - 예전의 "28~62 배" 는 낮은 개수에서 게임 스레드가 GPU 를 기다리느라 한가했던 것이
만든 착시였다. **작업 집합 가설도 SoA 도 근거가 없다.**

**3. 진짜 결함 - `createPrimitive` 가 부를 때마다 새 `Mesh` 를 만들고 있었다.**
씬에서 온 `MeshComponent` 는 전부 `_meshId` 로 도형을 해석하는데, 그때마다 **자기 Mesh 객체**를
새로 지었다. 배치 키가 메시 **포인터**라 같은 큐브 8000 개가 배치 8000 개로 갈렸고, GPU 정점
버퍼도 8000 벌이었다.

**벤치에는 왜 안 보였나.** 벤치는 메시 하나를 8000 개 컴포넌트가 나눠 쓴다 - 배치 2 개다.
즉 성능 검증 워크로드가 이 결함을 정확히 가리고 있었다. 디스크에서 로드한 씬만 무너졌다.

고친 뒤 (정적 8002 엔티티 · DX12 · 120 프레임):

| | 전 | 후 |
|---|---|---|
| 배치 수 | 8000 | **6** |
| `GT.Packet.export` | 441 us | **3 us** |
| `GT.Frame` | 2469 us | **1462 us** |
| `RT.Frame` | 2469 us | **1465 us** |

**헤드룸이 아니라 실제 프레임이 1.7 배**다 - GPU 가 8000 드로우 대신 6 을 한다.

**그리고 백엔드가 서로 맞기 시작했다.** 같은 씬을 세 백엔드로 찍어 비교한 결과:

| | 전 | 후 |
|---|---|---|
| dx12 vs vk | 15.6% 불일치 | **0.1%** |
| dx12 vs dx11 | 29.3% | **0.7%** |
| vk vs dx11 | 30.5% | **0.8%** |

버퍼 8000 벌이 백엔드마다 다르게 무너지고 있었다는 뜻이다. 즉 이 변경의 픽셀 차이는 **개선**이다.
(전/후 이미지가 32.7% 다른 것을 보고 회귀를 의심했는데, 백엔드 사이가 이미 그만큼 어긋나
있어서 "전" 은 애초에 기준이 못 됐다. 판단은 **백엔드 일치도**로 했다.)

**4. 인스턴스 배열은 값이 아니라 공유한다.** RT 는 그 배열을 읽기만 하므로(배치 목록은 GPU
핸들을 덧칠하니 그대로 값이다) `shared_ptr<const vector<GpuInstance>>` 로 바꿨다. 빌더는 다 지은
배열을 **옮겨서** 발행하고, 제자리 갱신이 필요할 때만 발행본에서 되돌려 받는다 - 전체 재구축
경로에는 복사가 아예 없고, 내용이 그대로인 프레임에는 인스턴스가 한 번도 복사되지 않는다.
배치가 잘 묶이는 벤치에서 export 76 -> 4 us.

**지키는 불변식은 "발행한 배열은 다시 고치지 않는다" 하나다** - 이미 넘긴 배열을 제자리에서
갱신하면 RT 가 그 프레임에 읽는 데이터가 밑에서 바뀐다. `InstancesStayLiveAfterPublishWhenObjectsMove`
가 그것을 잡는다(변이로 확인).

**5. 되돌린 것 하나 - "안 바뀌면 안 보내기".** `_bCpuDirty` 가 false 면 패킷에 스냅샷을 안 싣고
RT 가 지난 것을 쓰게 했더니 export 가 0 이 됐지만, 그 설계의 불변식은 "RT 가 스냅샷을 잃는 모든
자리에서 GT 도 무효화한다" 이고 **앱에서만 우연히 성립한다**(테스트·임베더는 자기 빌더를 따로
든다). `hostgpu` 가 바로 잡아냈다. 위 4 번은 패킷이 여전히 자기완결이라 그 함정이 없다.

**6. 부분 업로드(언리얼 GPUScene dirty-primitive) 를 했다.** 인스턴스 버퍼는 뭐 하나라도 dirty 면
**전체**를 다시 올리고 있었다. 벤치에 "움직이는 개수" 스위치를 임시로 넣어 재 보니 10 개만 움직여도
800 개와 같은 값이었다 - 전제는 사실이었다.

**처음엔 더 느려졌다.** 구간마다 `updateStructuredBufferRange` 를 부르게 했더니 800 개 이동이
101 -> 425 us 가 됐다. 재 보니 **호출당 ~3.3 us 의 고정비**가 있었다 (DX12 · 8000 인스턴스 ·
구간 1/2/5/20/64 = 17/34/45/113/209 us). 원인은 바이트가 아니라 **호출마다 붙는 스테이징 확보와
큐 제출**이었다(`submitUploadSlot` 이 호출마다 커맨드 리스트를 제출한다).

그래서 API 를 구간 **배열**을 받는 것으로 바꿨다(`updateStructuredBufferRegions`) - 스테이징 한 번 ·
배리어 한 쌍 · 영역 N 개. `vkCmdCopyBuffer` 는 원래 영역 배열을 받고, DX12 는 배리어 한 쌍 안에서
`CopyBufferRegion` 을 N 번 부르면 된다. GL·DX11 은 제출이 없어 그냥 돈다.

| 움직이는 큐브 | 전(전체 업로드) | 구간마다 호출 | **구간 배열 한 번** |
|---|---|---|---|
| 1 | ~100 us | 17 | **16** |
| 5 | ~100 | 47 | **18** |
| 20 | ~100 | 113 | **18** |
...
**3~6 배 빨라졌고 나빠지는 구간이 없다.** (8000 개가 전부 움직이면 구간이 하나로 이어져 결국
통째로 한 번 올리는 것과 같다.)

건드린 것: `IRHIResource::updateStructuredBufferRegions` 신설(기존 두 이름은 영역 1 개짜리 헬퍼로
남는다), 4 백엔드 구현, **ABI v4 -> v5 · 스탬프 v15**. `GpuSceneBuilder` 가 제자리 갱신에서 바뀐
슬롯을 구간으로 적고(`_listDirtyInstanceRun`), `GpuScene::upload` 가 그것을 한 번에 넘긴다.
버퍼가 **새로 만들어진 프레임은 전체**로 취급한다 - 새 버퍼에는 아직 아무것도 없다.

검증: 4 백엔드에서 물체가 실제로 움직이고(프레임 30 vs 90 이 다르다) 서로 일치한다(0.3~1.1%,
잘 묶인 씬의 기준선과 같은 수준). 백엔드 교체 · 에디터(창 15 개 · 빈 패널 0 개)도 통과.

**7. "영속 렌더 씬 + 변경 이벤트"(언리얼 `FScene` 모델)는 기각한다 - 종류가 늘어도 비용이 안 는다.**
이 모델의 값어치는 "매 프레임 씬에서 후보를 다시 수집하는 대신 변경만 밀어 넣는다" 인데, 그러려면
수집 비용이 씬의 **종류**(배치 수)에 따라 자라야 의미가 있다. `-gv_benchMeshVariants` 로 8000
인스턴스의 종류만 바꿔 쟀다(DX12 · 120 프레임):

| 종류 | export | build | collect | batches | GT.Frame |
|---|---|---|---|---|---|
| 1 | 3 us | 481 | 141 | 217 | 1136 |
| 8 | 66 | 925 | 196 | 543 | 1637 |
| 64 | 66 | 935 | 184 | 573 | 1893 |
| 256 | 79 | 1037 | 227 | 617 | 1992 |
| 1024 | 103 | 916 | 206 | 574 | 1854 |

**8 -> 1024 는 128 배인데 build 는 925 -> 916 으로 평평하다.** 종류는 비용을 끌지 않는다 - 이미
(1) 내용이 그대로면 build 가 통째로 건너뛰고, (2) 물체만 움직이면 제자리 갱신을 타고, (3) 인스턴스
배열은 공유하고 바뀐 구간만 올리기 때문이다. 영속 씬으로 얻을 것이 남아 있지 않다.

**1 -> 8 의 점프는 종류가 아니라 투명이었다.** 투명 0% 로 두면 export 66 -> 4, GT.Frame
1637 -> 1179 다. 원인은 `refreshInstancesInPlace` 가 **매 프레임** 투명 정렬 순서가 바뀌어
빠져나가는 것이다(프로브로 확인: 120 프레임 중 120 회).

**그런데 이것은 결함이 아니다.** 투명 배치는 **정렬된 순서에서 연속 동일 키를 묶어** 만든다 - 그래야
배치 순서가 곧 깊이 순서가 되어 뒤에서 앞으로 그리기와 배칭이 양립한다. 순서가 바뀌면 배치 구성
자체가 달라지므로 재구축이 맞다. (GPU 바이토닉 정렬은 **배치 안**만 정렬한다 - 배치 사이 순서는
여전히 CPU 몫이다.) 게다가 절벽이 아니라 비율에 비례한다:

| 투명 비율 | export | build | batches |
|---|---|---|---|
| 0% | 4 us | 480 | 205 |
| 1% | 8 | 482 | 210 |
| 5% | 20 | 490 | 225 |
...
현실적인 비율(<= 5%)에서는 기준선 대비 ~10 us 다. **여기를 "최적화" 하겠다는 제안이 나오면 위
구조를 먼저 읽을 것** - 순서를 무시하고 키로만 묶으면 빨라지지만 투명이 잘못 겹쳐 보인다.
...
**1. 먼저 벤치가 틀렸다.** 처음에는 `GameObjectManager` 하나를 여러 단계가 돌려 쓰게 짰다.
단계마다 오브젝트와 죽은 컴포넌트가 쌓여 **나중 단계일수록 느려졌고**, 그 상태에서 읽은
수치로 "바이너리가 XML 보다 3.3 배 느리다" 는 결론을 냈다. **건드리지도 않은 XML 경로가
7 us 에서 33 us 로 튀는 것을 보고서야** 하니스를 의심했다. 라운드마다 매니저를 새로 만들자
수치가 재현 가능해졌다(같은 값이 세 번 연속). **누적 상태를 공유하는 벤치는 단계 순서를
재고 있을 뿐이다.**

**2. 제대로 재니 바이너리는 9 배 빨랐다.** (Release · best-of-5 · 2000 회 · 엔티티당 ns)

| | XML | Binary |
|---|---|---|
| 컴포넌트 하나 역직렬화 | 3070 ns | **332 ns** |
| 오브젝트 상태 크기 | 454 B | **265 B** |

**3. 그런데 `BinarySerializer` 는 다형 소유 포인터를 아예 못 실었다.** XML·JSON 은 진작
`ctx.createOwnedPointer` / `getRuntimeTypeInfo` 로 `vector<Component*>` 를 다루는데, 바이너리만
빠져 있어 `serializeValueBinary` 로 흘러 들어가 **0 바이트 하나를 적고 조용히 버렸다.**
성능 문제가 아니라 데이터 유실이었고, `ObjectStateSerializer` 가 바이너리만 손으로 쓴
세 번째 구현을 들고 있던 이유이기도 하다.

원소마다 `[이름][본문크기][본문]` 으로 싣도록 `SerializerUtil` 에 추가했다. **본문 크기를 같이
적는 이유는 모르는 타입을 건너뛰기 위해서다** - 없으면 낯선 컴포넌트 하나가 그 뒤 스트림을
통째로 어긋낸다(XML 은 태그 중첩으로 공짜로 얻는 성질이다).

**4. 그 위에서 손으로 쓴 쌍을 지웠다.** `saveToBinaryBuffer`/`loadFromBinaryBuffer` 가 이름·부모·
활성·태그·컴포넌트·부착표를 한 줄씩 적던 ~120 줄이, XML·JSON 과 같은 세 줄짜리 리플렉션
경로가 되었다. 부수 효과로 **태그를 두 번 적던 것**(`TagComponent` 가 이미 상태에 실리는데
태그 배열을 또 적었다)이 사라졌다. 바깥에 남는 것은 부모 이름 하나뿐이다 - 오브젝트 사이의
부모 관계만 리플렉션 상태에 없고 씬이 나중에 rebind 한다.

**5. 컨텍스트를 엔티티마다 통째로 복사하고 있었다.** `SerializeContext` 는 `unordered_map` 을
여섯 개 들고 있는데 `makeGameObjectXmlContext` 가 `= getDefault()` 로 **전부 복사**했다.
**659 ns**, 오브젝트 XML 경로의 13% 다. 표는 만들어진 뒤 바뀌지 않으므로 복사할 이유가 없다.
`deriveFromDefault()` 로 표를 가리키기만 하게 했다(조회는 자기 것 먼저, 없으면 빌려온 쪽).

**전/후 (Release · 같은 하니스 · 엔티티당):**

| | 전 | 후 |
|---|---|---|
| 오브젝트 XML (씬 로드) | 4970 ns | **4254 ns** |
| 오브젝트 Binary (에디터 undo·플레이 스냅샷·세이브게임) | 1696 ns | **1602 ns** |

바이너리 경로가 리플렉션으로 바뀌었는데도 **손으로 쓴 옛 경로보다 빠르다.** 중간에 한 번
2325 ns 로 느려진 적이 있었는데, 그 620 ns 가 정확히 위 컨텍스트 복사였다.

**6. 그리고 테스트가 엔진 무한 재귀를 하나 끌어냈다.** 부착 계층을 검사하려고 한 오브젝트
안에서 `MeshComponent` 를 `SceneComponent` 에 붙였더니 **XML 경로도 같이** 죽었다. 원인은
직렬화가 아니라 `GameObject::getChildren()` 이었다 - 자식 GameObject 를 primary SceneComponent 의
**자식 컴포넌트들의 owner** 로 구하는데, 같은 오브젝트 안의 부착이면 그 owner 는 자기 자신이라
**자신이 자기 자식으로** 나왔다. `refreshActiveInHierarchy` 가 그대로 무한 재귀해 스택을 넘겼다.
부착 없이는 멀쩡하고 부착하는 순간 죽어서, 부착을 쓰는 경로에서만 터졌다.
...
**먼저 잴 것을 만들었다.** 이 저장소의 씬은 5 KB 하나뿐이라 로드를 잴 수가 없었다.
`Scripts/dev/GenerateStressScene.py` 로 엔티티 4002 개(2.1 MB, 도형 6종 섞음) 씬을 만들어 쟀다.
도형을 섞는 이유는 `_meshId` 마다 정점 수가 달라서다 — 큐브만 N 개면 실제 씬과 다른 그림이 된다.

**첫 숫자: 파싱 8 ms + 생성 247 ms = 255 ms.** 파싱은 3% 뿐이었다. 단계를 쪼개 들어가니
`ObjectStateSerializer::loadFromXmlString` → `applyTypeDefaults` 가 **207 ms** 였다.

원인은 `ComponentDefaults::ensureDefaultsLoaded` 다. 깃발이 **"읽었는가" 하나뿐**이라, 로드가
실패하면 false 로 남고 **다음 컴포넌트가 또 파일을 열었다.** 기본값 파일(`data/gamedata.xml`)은
있어도 되고 없어도 되는 것이고 이 게임에는 **없다** — 그래서 씬 로드가
`컴포넌트 수 x 파일 열기 실패` 가 됐다.

"시도했는가" 깃발을 따로 두어 성공·실패 모두 한 번으로 끝낸다. `reload()` · `setPath()` 는 그
깃발도 내리므로 개발 중 파일을 만들어 넣는 길은 그대로다.

측정은 저장소에 이미 있는 **`ScopeCpuTimer`** 로 한다(`Scene.load.parse` ·
`Scene.load.instantiate`). 처음에는 `CpuTimer` 를 손수 돌리고 `[[maybe_unused]]` 로 Shipping
경고를 막았는데, `ScopeCpuTimer` 가 그 둘을 이미 한다 — 스코프 동안 재고, 로그가 사라지는
빌드에서는 경과 계산도 함께 사라진다. **있는 도구를 안 쓰고 있었다.**

| | Scene.load.parse | Scene.load.instantiate |
|---|---|---|
| 전 | 8.35 / 8.27 / 8.62 ms | **249.7 / 242.7 / 246.7 ms** |
| 후 | 8.15 / 8.21 / 8.35 ms | **31.1 / 29.5 / 31.3 ms** |

생성 단계가 **8.0 배**(246 → 30.6 ms), 로드 전체가 **6.5 배**(255 → 39 ms)다. 엔티티가 많을수록
차이가 커진다 — 컴포넌트 수에 비례하던 비용이 사라지기 때문이다.

(처음 커밋에는 정수로 자른 값으로 "5.8 배" 라고 적었다. `ScopeCpuTimer` 로 소수점까지 다시
재고 **같은 도구로 전/후를 각각 3회** 돌린 것이 위 표다.)

곁들여 넣은 것 둘.

- **타입별 해석 캐시.** `apply` 가 컴포넌트마다 상속 체인을 모으고(레지스트리 조회 N 번) 단계마다
  조회 이름 문자열을 만들어 XML 을 훑었다. 그 계산은 `TypeInfo` 와 문서만으로 정해진다.
  기본값 파일을 **일부러 만들어 놓고** 재니 `defaults` 3~4 ms → 2 ms 로, 작지만 일관된다.
  문서를 다시 읽으면(`reload`/`setPath`) 캐시는 통째로 버린다 — `XmlNode` 가 그 문서를 가리킨다.
- **씬 로드 한 줄 요약 로그.** 로드는 프레임 밖에서 일어나 `SW_PROFILE_SCOPE` 의 프레임 집계에
  안 잡힌다. 이제 `[SceneLoad] '<경로>' 엔티티 N개 — 파싱 A ms + 생성 B ms = C ms` 가 남는다.

**헛짚은 순서도 적어 둔다.** 처음에 `applyTypeDefaults` 를 의심해 캐시를 넣었는데 **숫자가 안
움직였다** — `apply` 가 캐시보다 **앞에서** `ensureDefaultsLoaded()` 를 부르고 거기서 되돌아
나가고 있었기 때문이다. 그 뒤 `PrimitiveRegistry::add`(O(1) 이었다) 와
`getOrCreateComponentPool` 을 차례로 의심했다가 전부 빗나갔고, 단계를 직접 쪼개 재고 나서야
자리가 나왔다. **읽고 추측하는 것보다 쪼개서 재는 것이 빨랐다.**

회귀 테스트는 시간이 아니라 **열어 본 횟수**로 본다(`getLoadAttemptCount`) — 시간으로 재면
흔들린다. 없는 경로를 주고 컴포넌트 32 개를 만든 뒤 시도 횟수가 1 이하인지 본다.

### 2026-09-20 (병목은 씬 크기에 따라 뒤집힌다 — 측정만 하고 고치지 않은 회차)

커밋 `TBD`. **코드 변경 없음.** (B) 패스의 남은 방향을 정하려고 잰 결과와, 재서 기각한 것.

**1. 병목이 뒤집히는 지점을 찾았다.** 앞서 "프레임은 GPU 대기가 정한다" 고 적었는데 그것은
큐브 2000 개 기준이었다. 8000 개에서는 반대다.

| 큐브 | GT.Packet.submit | RT.BeginFrame | GT.Frame | 누가 기다리나 |
|---|---|---|---|---|
| 2000 | 722us | 516us | 909us | GPU |
| 8000 | **98us** | **135us** | 996us | **게임 스레드** |

8000 에서 임계 경로를 채우는 것은 `GT.Scene.tick.flushTransforms`(315us) 와
`GT.GpuScene.build`(490us) 다. 0순위 절의 결론을 그 범위까지 적어 고쳤다.

**2. `GpuScene.build` 는 초선형이다.** 메시 500 → 8000 (16배)에 단계별로 28~62배다.
...
**→ 2026-09-20 에 기각됐다.** 구조체를 78% 키워도 숫자가 안 움직였고(작업 집합 아님), 촘촘히
재니 2 배마다 2.1~2.4 배로 거의 선형이었다. 위 표의 큰 배율은 낮은 개수에서 게임 스레드가 GPU 를
기다리느라 한가했던 것이 만든 착시다. **SoA 재배치는 하지 않는다.**

**3. 재서 기각한 것 하나.** `collect` 이 월드 행렬을 지역 변수로 한 번, 후보로 또 한 번
복사하고 있어서 한 번으로 줄였다 — **숫자가 안 움직였다**(위 기각 목록에 적었다). 컴파일러가
이미 없애고 있었다. 되돌렸다.

**4. 사실이 아니었던 가설 하나.** "Core 의 문자열·컨테이너 유틸이 다른 레이어에서 손수 다시
구현돼 있을 것" 이라고 보고 훑었는데, `find( x ) != npos` 15자리는 대부분 정당한 용법이고
(대소문자를 일부러 가리는 곳, 문자 하나 찾기) sort+unique 관용구도 6자리뿐이다. 일괄 치환은
diff 만 늘리고 나아지는 것이 없어서 **하지 않았다.**

**5. 잴 수 없는 것도 적어 둔다.** 씬 로드 경로에는 프로파일 스코프가 하나도 없다. 다만 이
저장소에 있는 씬은 5 KB 짜리 하나뿐이라 **잴 만한 로드 워크로드가 없다** — 계측을 넣어도
의미 있는 숫자가 안 나온다. 큰 씬이 생기면 그때 넣을 것.
**→ 바로 다음 회차에 큰 씬을 만들어 쟀고, 로드 시간의 81% 를 먹는 결함이 나왔다(아래).**

### 2026-09-20 (이중 해제 가드가 종류마다 달랐고 GL 의 UAV 만 없었다)

커밋 `TBD`. (B) 패스, RHI 백엔드 넷 사이의 중복을 훑다가 나온 **실제 결함**.

bindless 인덱스를 반납하는 자리의 "이미 빈 슬롯인가" 가드가 백엔드·종류마다 달랐다.

| | buffer | texture | uav |
|---|---|---|---|
| DX11 | 가드 + 에러 로그 | 가드 + 에러 로그 | 가드(조용한 반환) |
| GL | 가드 + 에러 로그 | 가드 + 에러 로그 | **없음** |
| Vulkan | 가드 | 가드 | 가드(두 목록을 같이 본다) |

**GL 의 `unregisterBindlessUav` 만 가드가 없었다.** 공유 헬퍼 `releaseFreeListIndex` 는 범위만
보고 무조건 프리리스트에 넣으므로, 이중 해제하면 **같은 인덱스가 프리리스트에 두 번** 들어가고
다음 두 번의 할당이 그 인덱스를 **서로 다른 리소스에 발급한다.** 그 뒤로는 한쪽이 다른 쪽의
디스크립터를 덮어쓴다. 다른 자리들의 주석이 정확히 그 위험을 적어 두고 있었는데, 정작 한 자리만
그 가드가 없었다.

가드를 **공유 헬퍼 한 자리**로 옮겼다(종류 이름은 로그용으로 받는다). 부르는 쪽의 중복 가드
넷을 걷었고, GL 의 UAV 는 그 결과로 막혔다 — 앞으로 이 헬퍼를 쓰는 곳도 같이 막힌다.
헬퍼가 "비었다" 를 알아야 하므로 GL 의 레코드 둘에 `operator==` 를 줬다.

검증: 단위 테스트 `RHIIndexFreeListTest.DoubleReleaseIsRejected`(가드를 빼면 "같은 인덱스가
두 리소스에 발급됐습니다" 로 깨진다) + **네 백엔드 실행**. 각 백엔드로 벤치를 띄워 스크린샷을
찍고 R 채널을 견줬다 — 고유값 216~221 · 최대 255 · 평균 35.5~35.8 로 넷이 일치하고 에러 로그 0.

**남은 중복은 손대지 않았다.** DX12 의 bindless 인덱스 할당은 벡터가 아니라 **디스크립터 힙
카운터 + 용량 상한**이라 이 헬퍼가 맞지 않는다(헬퍼 주석이 DX12 를 일부러 뺀 이유다). 한 군데서만
쓸 변형을 만드는 것은 재사용이 아니라 간접층이라 두었다.

### 2026-09-20 (리소스 읽기 두 벌 · 못 찾을 때 경로를 두 번 풀었다)

커밋 `TBD`. (B) 패스, clang-tidy 가 못 잡는 종류.

`ResourceUtil::readTextResource` 와 `readBinaryResource` 는 **같은 네 단계를 두 벌** 적고 있었다
(절대경로 → 낱개 파일 → 팩 → 낱개 폴백). 다른 것은 "어떻게 읽는가" 뿐이다. 한쪽만 고치면 같은
키로 텍스트와 바이너리가 **서로 다른 파일**을 읽게 된다.

그리고 둘 다 **못 찾을 때 `getResourcePath` 와 `fileExists` 를 두 번씩** 했다 — 1단계가 이미
"그 경로에 없다" 를 확인했는데 3단계가 같은 경로를 다시 물었다. 없는 파일을 찾는 일은 드물지
않다(선택적 파일 탐색, 확장자 추정). 경로를 한 번만 풀고, 폴백은 **1단계가 경로를 못 푼
경우에만** 의미가 있으므로 그때만 돈다.

`readResourceCommon` 하나로 모았다 — 순서는 한 자리에 있고 두 함수는 읽는 방법만 넘긴다.

**테스트가 덮는 것과 아닌 것을 분명히 해 둔다.**
`TextAndBinaryReadsPickTheSameSource` 는 "둘 중 하나가 어떤 소스를 아예 안 보게 되는" 변이를
잡는다(팩을 건너뛰게 만들면 깨진다). 그러나 **낱개 파일과 팩이 경쟁할 때의 우선순위는 못
덮는다** — 처음에 그 순서를 못 박았다고 적었다가 변이(바이너리만 팩 우선)가 안 물려서 확인했다.
같은 상대 키로 디스크와 팩에 다른 내용을 두려면 리소스 루트 안에 파일을 심어야 하는데, 검색
폴더를 테스트에서 더할 창구가 없다(`initialize()` 가 리소스 루트에서만 채운다). 그 창구가
생기면 그때 우선순위 케이스를 붙인다.

### 2026-09-20 (광선 질의만 좁힘이 없었다 · 폴더 기록이 무한히 늘었다)

커밋 `TBD`. (B) 패스, 백로그에 적어 둔 마지막 두 항목.

**1. `SpatialHashGrid2D::queryRay` 만 좁힘 판정이 없었다.**
형제 둘은 처음부터 각자의 판정을 거친다 — `queryAabb` 는 `intersects`, `queryCircle` 은 가장
가까운 점까지의 거리. `queryRay` 는 광선이 **지나간 셀 안의 핸들을 전부** 담았다. 이름이
`queryRay` 인데 실제로는 후보 목록이었으므로, 걸러 주지 않는 호출부는 틀린 답을 받았다.
슬랩(slab) 판정을 넣었고, **셀에 못 들어간 큰 핸들도 같은 판정을 거치게** 했다(그쪽은 크다는
이유만으로 무조건 결과에 들어가고 있었다).

테스트 `SpatialHashGrid2DRayIgnoresBoxesItNeverTouches` — 셀 크기(64)보다 작은 상자 둘을
**같은 셀**에 넣고 하나만 지나는 광선을 쏜다. 좁힘이 없으면 같은 셀이라는 이유로 둘 다 나온다.
사거리 밖과 광선 뒤쪽도 같이 본다. 변이(좁힘 제거)로 깨지는 것을 확인했다.

**2. `ContentBrowserPanel::_listHistory` 에 상한이 없었다.**
폴더를 옮길 때마다 문자열 둘이 붙고 지워지지 않아서, 에디터를 오래 켜 둘수록 단조 증가했다.
브라우저의 뒤로 가기와 같은 규칙으로 `kMaxHistoryCount = 64` 를 두고 오래된 것부터 버린다.

이것으로 **(B) 패스의 백로그 목록은 비었다.**
