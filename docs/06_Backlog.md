...
**모든 단계가 고르게** 초선형인 것은 알고리즘이 아니라 **작업 집합** 신호다. `DrawCandidate`
는 약 144바이트(`float4x4` 64 + `shared_ptr` 셋 48 + 나머지)이고 8000 개면 1.15 MB 로 L2 를
넘는다. 고치려면 SoA 재배치인데 규모가 크고 위험하므로 **숫자를 더 모으기 전에는 시작하지
않는다.** 다음에 여기를 열 사람을 위해 표를 남긴다.
