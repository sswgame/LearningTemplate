...
DX12·DX11·Vulkan 은 픽셀이 일치하는데 GL 만 기하가 어긋나 `_bGpuMeshMorph = SW_FALSE` 로 꺼 두었다.
배제한 것 셋(모프 수식·std430 정렬·폴백 경로)과 남은 의심 둘(정점 스테이지 SSBO 바인딩,
`ARB_gl_spirv` 의 `gl_VertexID` 의미)은 3절의 구현 항목에 적어 두었다.

다음에 볼 것 둘:
1. GL 의 `GL_MAX_VERTEX_SHADER_STORAGE_BLOCKS` 를 실제로 질의한다 — `t11` 은 이 스테이지의 **네 번째**
   SSBO 다(인스턴스 t4 · 머티리얼 t9 · 가시 목록 t10 다음). 한도를 넘으면 링크는 통과해도 읽기가 어긋난다.
2. 컴퓨트가 쓴 버퍼를 `glGetBufferSubData` 로 되읽어 **어느 쪽이 틀렸는지** 가른다 — 컴퓨트 쓰기가
   틀린 것인지, 정점 셰이더 읽기가 틀린 것인지. 지금은 둘을 구분하지 못한 채 그림만 보고 있다.

### 1-5. 정점에 **노멀이 없다** — 셰이더가 위치로 지어내고 있다

`RHIVertex` 는 위치와 색뿐이다. 그래서 모든 셰이더가 `DemoCubeNormal( localPos )` 로 노멀을 **지어낸다**
— 원점 중심 박스형 도형에만 맞는 함수다. 증상은 조용하다: 바닥 평면을 `y = 0` 에 두면 `|y|` 가 0 이라
±X/±Z 노멀을 받아 **바닥이 옆을 보는 것처럼** 칠해진다(그래서 `MeshUtil::createPlane` 은 면을
큐브 윗면 자리인 `y = +0.5` 에 둔다 — 회피이지 해결이 아니다). 구·캡슐도 정확한 노멀이 아니다.

손댈 자리가 한 번에 여럿이라 따로 잡아야 한다:
- `RHIVertex` 레이아웃과 **네 백엔드의 입력 레이아웃**(stride 가 박힌 자리 포함 — `setVertexBuffer( …, sizeof( RHIVertex ), … )`)
- `forwardlit` · `gbuffer` · `shadowdepth` 의 VS 입력과 `DemoCubeNormal` 호출부
- 모프 풀 원소(`GpuMorphVertex` / `SwVertexData`)와 `meshmorph.hlsl` — 변형하면 노멀도 다시 만들어야 한다
- `MeshUtil` 의 도형 여섯 개가 노멀을 함께 낸다
