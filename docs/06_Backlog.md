...
**다음에 볼 곳 (재지 않았다).** 인스턴스 버퍼는 뭐 하나라도 dirty 면 **전체**를 다시 올린다
(`GpuScene::upload`). 8000 개 중 10 개가 움직여도 8000 개를 올린다 - 언리얼의 GPUScene
dirty-primitive 리스트가 그 자리다. 재려면 "일부만 움직이는" 워크로드가 필요한데 저장소에 없다
(벤치는 전부 움직이고 정적 씬은 업로드를 통째로 건너뛴다).
