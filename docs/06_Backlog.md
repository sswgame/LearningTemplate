...
>
> 마지막 갱신: 2026-09-19 · 기준 커밋 `baefc99f`
...
평균 · **목적지 전체가 채워지는가**(좌상단만 그리던 증상의 축소판) · 확대 · 빈 입력. 변이 확인:
평균을 최근접으로 바꾸면 `DownscaleAveragesTheCoveredPixels` 가 `Expected [75], Actual [0]` 로 진다.
