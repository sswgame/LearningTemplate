...
>
> 마지막 갱신: 2026-09-21 · 기준 커밋 `64f06f62` (+ DX12 업로드 제출 한 번 커밋)
...
- 남은 길은 **웨이브 사이의 프레임 스트림 조각을 없애는 것**(웨이브 배리어를 프레임 스트림이 아니라 그 웨이브의 첫 패스
  리스트 앞머리에 기록) — 프레임당 12 → ~7 리스트, RT ~35 us. 그래프의 병렬 기록 계약(배리어는 렌더 스레드가 미리 정한다)을
  건드리므로 그 계약을 지키는 설계가 먼저다. 그리고 지금은 GT 가 RT 보다 길어 벽시계가 안 움직인다 — GT 다음.
