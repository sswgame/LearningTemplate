...
### 1-4. GPU 메시 모프의 **OpenGL** 경로 — 컴퓨트의 **읽기**만 틀린다

`_bGpuMeshMorph = SW_FALSE` 로 GL 만 꺼 둔 상태다. 폴백은 깨끗하다(모프 켬/끔 51,916 vs 51,880 · 평균 동일).
측정은 전부 진짜 백엔드(`-gl`)로 했다.

**여기까지 좁혔다 — 컴퓨트의 쓰기는 맞고, 레스트 버퍼 읽기가 틀린다.**

검증 방법: 기하는 **레스트로 그리고**(그래야 무너지지 않아 모든 정점을 볼 수 있다) 모프 버퍼 내용만
색으로 판정했다. 초록 = 기대값과 같음.

| 컴퓨트가 쓰는 값 | DX12 | OpenGL |
|---|---|---|
| **상수**(레스트를 안 읽음) | 초록 52,356 · 빨강 0 | 초록 51,386 · 빨강 **925** (98.2% 정확) |
| **레스트 복사**(항등) | 초록 52,289 · 빨강 0 | 초록 2,770 · 빨강 **49,332** |

즉 `g_MorphVerticesRW`(u0 → SSBO binding 16) **쓰기**와 정점 셰이더의 `g_SwMorphVertices`(t11 → binding 11)
**읽기**는 GL 에서도 정상이다. `g_RestVertices`(t0 → binding 0) **읽기**만 틀린다.

**배제한 것 (전부 실측)**
1. ~~정점 스테이지 SSBO 한도~~ — 질의값 정점 96 · 프래그먼트 96 · 컴퓨트 96.
2. ~~`gl_VertexID` 의미~~ — `SV_VertexID` 를 색으로 찍어 DX12·Vulkan·GL 동일.
3. ~~엔진 바인딩~~ — 실제 `glBindBufferBase` 호출을 찍어 확인했다: 레스트 → binding 0, 모프 쓰기 →
   binding 16, 그리기 읽기 → binding 11. 전부 계약대로다.
4. ~~리플렉션 누락~~ — 레이아웃에 `SwMorphVertices`(reg 11) 가 있고 레지스트리도 찾는다(DX12 와 동일).
5. ~~컴퓨트 상수버퍼~~ — CB 를 한 줄도 안 쓰는 항등 복사로도 틀린다.
6. ~~동기화~~ — 드로우 직전에 `glMemoryBarrier( GL_ALL_BARRIER_BITS )` 를 걸어도 그대로다.
7. ~~인덱스 공간 추측~~ · ~~`glBufferData` 초기 데이터 OOB 읽기~~ — 둘 다 **실재 결함이라 고쳤지만**
   이 증상의 원인은 아니었다(3절 참고).

**다음에 볼 것** — 범위가 "컴퓨트 셰이더의 `StructuredBuffer` 읽기" 하나로 좁혀졌다:
- `gpucull.hlsl` 도 `g_Instances` 를 `t0`(binding 0)에서 읽고 **잘 돈다**. 두 선언의 차이를 SPIR-V
  수준에서 대조할 것 — 원소 구조체(`SwVertexData` = float4 둘 vs `GpuInstance`), `-fvk-use-dx-layout`
  적용 결과, `OpDecorate` 의 `ArrayStride`. stride 가 어긋나면 **읽기만** 어긋난다(쓰기는 같은 stride 로
  일관되게 틀려도 상수 쓰기 검증은 통과한다) — 위 표의 모양과 정확히 맞는다.
- `glGetBufferSubData` 로 레스트 버퍼를 CPU 로 되읽어 **업로드된 바이트 자체**를 확인할 것.
  버퍼 되읽기 API 가 없어 지금은 셰이더로만 보고 있다.

**왜 우선순위인가.** 정점 데이터를 입력 어셈블러에서 구조버퍼로 옮기는 전환(언리얼의 manual vertex
fetch)은 **정확히 이 경로**에 기댄다. 지금 전환하면 OpenGL 은 폴백이 "레스트 포즈" 가 아니라
**아무것도 안 그려짐**이 된다.
