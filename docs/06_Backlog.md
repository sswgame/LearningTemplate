...
>
> 마지막 갱신: 2026-09-20 · 기준 커밋 `4a8b4bba`
...
**덮지 못한 것 하나.** `postchain.hlsl` 의 블룸 뒤 `saturate` 는 나눈 판이 중간 타깃에서 자르던
것을 재현하는 것인데, 테스트 씬이 어두워(RGB 평균 약 26) 블룸이 1 을 넘지 않아 **그 자름이 아무
일도 하지 않는다** — 빼는 변이를 걸어도 테스트가 통과한다. 그쪽은 벤치 씬에서 앱으로 확인했다
(자름이 있으면 나눈 판과 최대 차이 1, 빼면 31). 밝은 머티리얼을 쓰는 씬을 테스트에 세우면 닫힌다.
