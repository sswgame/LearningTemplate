...
>
> 마지막 갱신: 2026-09-19 · 기준 커밋 `c700338b`
...
> 증상은 데이터에 따라 나중에 터진다.
> **왜 지금 안 고쳤나**: 올바른 수정은 "제자리 쓰기가 불가능한 컨테이너" 개념을 래퍼 인터페이스에
> 넣고 **세 직렬화기(Binary·Json·Xml)와 인스펙터**의 채우기 경로를 insert 기반으로 바꾸는 설계
> 변경이다. 긴 세션 끝에 시작할 크기가 아니다. 지금 이 저장소에 `set` 프로퍼티를 쓰는 타입은 없다
> (그래서 아무도 몰랐다) — 쓰기 전에 이것부터 고칠 것.
